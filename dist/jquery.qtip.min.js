...
this(q)});r={show:h.show.target,hide:h.hide.target};l={show:String(h.show.event).replace(" ",u+" ")+u,hide:String(h.hide.event).replace(" ",u+" ")+u};r.show.bind(l.show,i);if(h.show.ready||h.prerender)i(c)})};a.each({attr:function(d){var c=a(this),i=c.data("qtip");return arguments.length===1&&d==="title"&&i&&i.rendered===TRUE?c.data("oldtitle"):NULL},remove:a.ui?NULL:function(d,c){a(this).each(function(){if(!c)if(!d||a.filter(d,[this]).length)a("*",this).add(this).each(function(){a(this).triggerHandler("remove")})})}},
