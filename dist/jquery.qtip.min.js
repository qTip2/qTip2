...
e.css(f)},disable:function(e){var f=a.elements.tooltip;if(a.rendered)f.toggleClass("ui-state-disabled",e);else a.cache.disabled=!!e;return a},destroy:function(){var e=a.elements,f=e.target.data("oldtitle");if(a.rendered){e.tooltip.remove();c.each(a.plugins,function(){this.initialize==="render"&&this.destroy()})}clearTimeout(a.timers.show);clearTimeout(a.timers.hide);C(1,1,1,1);d.removeData("qtip");f&&d.attr("title",f);d.removeAttr("aria-describedby");return d}})}function P(d,b){var h,j,l=c(this),
o=c(document.body),w=this===document?o:l;j=l.metadata?l.metadata(b.metadata):NULL;var q=c.extend(TRUE,{},c.fn.qtip.defaults,b,M((j&&b.metadata.type==="html5"?j[b.metadata.name]:NULL)||j));l.removeData("metadata");j=q.position;if("boolean"===typeof q.content.text)if(q.content.attr!==FALSE&&l.attr(q.content.attr))q.content.text=l.attr(q.content.attr);else return FALSE;if(j.container===FALSE)j.container=o;if(j.target===FALSE)j.target=w;if(q.show.target===FALSE)q.show.target=w;if(q.hide.target===FALSE)q.hide.target=
w;j.at=new c.fn.qtip.plugins.Corner(j.at);j.my=new c.fn.qtip.plugins.Corner(j.my);if(l.data("qtip"))if(q.overwrite)l.qtip("destroy");else if(q.overwrite===FALSE)return FALSE;l.attr("title")&&l.data("oldtitle",l.attr("title")).removeAttr("title");h=new O(l,q,d);l.data("qtip",h);l.bind("remove.qtip",function(){h.destroy()});return h}function Q(d){var b=this;b.checks={"^content.ajax":function(){this.plugins.ajax.load(this.options.content.ajax)}};c.extend(b,{init:function(){var h=d.options.content.ajax;
d.elements.tooltip.bind("tooltiprender.ajax",function(){b.load(h);d.elements.tooltip.bind("tooltipshow.ajax",function(){h.once===FALSE&&d.rendered===TRUE&&b.load(h)})})},load:function(h){c.ajax(c.extend(TRUE,{},h,{success:function(j,l){if(c.isFunction(h.success))if(h.success.call(d,j,l)===FALSE)return;d.set("content.text",j)},error:function(j,l,o){var w=l||o;if(c.isFunction(h.error)){j=h.error.call(d,j,l,o);if(j===FALSE)return}d.set("content.text",w)}}));return b},destroy:function(){d.elements.tooltip.unbind("tooltipshow.ajax")}});
b.init()}function N(d,b,h){var j=Math.ceil(b/2),l=Math.ceil(h/2);b={bottomright:[[0,0],[b,h],[b,0]],bottomleft:[[0,0],[b,0],[0,h]],topright:[[0,h],[b,0],[b,h]],topleft:[[0,0],[0,h],[b,h]],topcenter:[[0,h],[j,0],[b,h]],bottomcenter:[[0,0],[b,0],[j,h]],rightcenter:[[0,0],[b,l],[0,h]],leftcenter:[[b,0],[b,h],[0,l]]};b.lefttop=b.bottomright;b.righttop=b.bottomleft;b.leftbottom=b.topright;b.rightbottom=b.topleft;return b[d.string()]}function R(d){function b(a,m,v){if(l.tip){a=c.extend({},h.corner);m=a.precedance===
"y"?["y","top","left","height","x"]:["x","left","top","width","y"];var e=v.adjusted,f=[0,0];if(h.corner.fixed!==TRUE){if(e.left)a.x=a.x==="center"?e.left>0?"left":"right":a.x==="left"?"right":"left";if(e.top)a.y=a.y==="center"?e.top>0?"top":"bottom":a.y==="top"?"bottom":"top";if(a.string()!==q.corner.string()&&(q.top!==e.top||q.left!==e.left))h.update(a)}f[0]=z?parseInt(w.css("border-"+a[m[0]]+"-width"),10)||0:C==="vml"?1:0;f[1]=Math.max(a[m[4]]==="center"?j.offset:0,j.offset);v[m[1]]+=(a[m[0]]===
m[1]?1:-1)*(n[m[3]]-f[0]);v[m[2]]-=(a[m[4]]===m[2]||a[m[4]]==="center"?1:-1)*f[1];q.left=e.left;q.top=e.top;q.corner=a}}var h=this,j=d.options.style.tip,l=d.elements,o=l.tooltip,w=l.wrapper,q={top:0,left:0,corner:{string:function(){}}},n={width:j.width,height:j.height},B={},z=j.border||0,C=j.method||FALSE;h.corner=NULL;h.mimic=NULL;h.checks={"^position.my|style.tip.(corner|mimic|method|border)":function(){z=j.border;if(h.init())d.get("position.target")!=="mouse"&&d.reposition();else h.destroy()},
"^style.tip.(height|width)":function(){n={width:j.width,height:j.height};h.create();h.update();d.reposition()},"^style.(classes|widget)$":function(){h.detectColours();h.update()}};c.extend(h,{init:function(){var a=c.browser.msie,m=h.detectCorner();(h.mimic||h.corner).string().indexOf("center");if(m){if(C!=="polygon")C=c("<canvas />")[0].getContext?"canvas":a?"vml":"polygon";h.create();h.detectColours();h.update();o.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",b)}return m},detectCorner:function(){var a=
j.corner,m=d.options.position,v=m.at;m=m.my.string?m.my.string():m.my;if(a===FALSE||m===FALSE&&v===FALSE)return FALSE;else if(a===TRUE)h.corner=new c.fn.qtip.plugins.Corner(m);else if(!a.string){h.corner=new c.fn.qtip.plugins.Corner(a);h.corner.fixed=TRUE}return h.corner.string()!=="centercenter"},detectColours:function(){var a=l.tip.css({backgroundColor:"",border:""}),m=h.corner,v=h.corner[h.corner.precedance];v="border"+v.charAt(0).toUpperCase()+v.substr(1)+"Color";var e=/rgba?\(0, 0, 0(, 0)?\)|transparent|#000000/i,
f=l.titlebar&&m.y==="top",g=d.options.style.widget;m=g?l.content:f?l.titlebar:l.wrapper;f=!g?l.wrapper:f?l.titlebar:l.content;B.fill=a.css("background-color")||"transparent";B.border=a.css(v)||"transparent";if(e.test(B.fill))B.fill=z?m.css("background-color"):f.css(v);if(!B.border||e.test(B.border))B.border=f.css(v)||B.fill;c("*",a).add(a).css("background-color","transparent").css("border",0)},create:function(){var a=n.width,m=n.height;l.tip&&l.tip.remove();l.tip=c("<div />",{"class":"ui-tooltip-tip"}).css(n).toggleClass("ui-widget-content",
d.options.style.widget).prependTo(o);switch(C){case "canvas":c('<canvas height="'+m+'" width="'+a+'" />').appendTo(l.tip)[0].getContext("2d").save();break;case "vml":a='<vml:shape coordorigin="0,0" coordsize="'+a+" "+m+'" stroked="false"  style="behavior:url(#default#VML); display:inline-block; position:absolute; antialias:false; left: 0; top: 0; width:'+a+"px; height:"+m+'px;"></vml:shape>';l.tip.html(z?a+=a:a);break;case "polygon":l.tip.append('<div class="ui-tooltip-tip-inner" />').append(z?'<div class="ui-tooltip-tip-border" />':
"");break}return h},update:function(a){var m=l.tip.children(),v=n.width,e=n.height,f=z>0?0:1,g=j.mimic,k,i,t,y;if(!a)a=h.corner;if(g===FALSE)g=a;else{g=new c.fn.qtip.plugins.Corner(g);g.precedance=a.precedance;if(g.x==="inherit")g.x=a.x;else if(g.y==="inherit")g.y=a.y;else if(g.x===g.y)g[a.precedance]=a[a.precedance]}t=g.string().indexOf("center")>-1;y=Math[/b|r/.test(g[g.precedance==="y"?"x":"y"])?"ceil":"floor"];switch(C){case "canvas":k=m.get(0).getContext("2d");k.restore&&k.restore();k.clearRect(0,
0,3E3,3E3);for(i=N(g,v,e);f<2;f++){if(f){k.save();k.translate(y((g.x==="left"?1:g.x==="right"?-1:0)*(z+1)*(g.precedance==="y"?0.5:1)),y((g.y==="top"?1:g.y==="bottom"?-1:0)*(z+1)*(g.precedance==="x"?0.5:1)))}k.beginPath();k.moveTo(i[0][0],i[0][1]);k.lineTo(i[1][0],i[1][1]);k.lineTo(i[2][0],i[2][1]);k.closePath();k.fillStyle=B[f?"fill":"border"];k.fill()}break;case "vml":i=N(g,v,e);i="m"+i[0][0]+","+i[0][1]+" l"+i[1][0]+","+i[1][1]+" "+i[2][0]+","+i[2][1]+" xe";m.each(function(A){c(this).attr({path:i,
fillcolor:B[A||!z?"fill":"border"]}).css("antialias",""+t)});break;case "polygon":m.removeAttr("style").css({position:"absolute",left:0,top:0}).each(function(A){var x={x:g.precedance==="x"?g.x==="left"?"right":"left":g.x,y:g.precedance==="y"?g.y==="top"?"bottom":"top":g.y},p=g.x==="center"?["left","right",x.y,e,v]:["top","bottom",x.x,v,e];A=B[!A&&z?"border":"fill"];g.x==="center"||g.y==="center"?c(this).css("border-"+p[2],p[3]+"px solid "+A).css("border-"+p[0],y(p[4]/2)+"px dashed transparent").css("border-"+
p[1],y(p[4]/2)+"px dashed transparent"):c(this).css("border-width",y(e/2)+"px "+y(v/2)+"px").css("border-"+x.x,y(v/2)+"px solid "+A).css("border-"+x.y,y(e/2)+"px solid "+A)});break}if(C!=="canvas"&&z){f=[z*2.75,z];f=g.precedance==="y"?[g.x==="left"?f[1]:g.x==="right"?-f[1]:0,g.y==="bottom"?-f[0]:f[0]]:[g.x==="left"?f[0]:-f[0],g.y==="bottom"?-f[1]:g.y==="top"?f[1]:0];m.eq(1).css({left:f[0],top:f[1]})}h.reposition(a);return h},reposition:function(a){var m=l.tip,v=["left","right"],e=j.offset,f,g;if(j.corner===
FALSE||!m)return FALSE;a=a||h.corner;f=a.precedance;g=a[f==="y"?"x":"y"];v[f==="y"?"push":"unshift"]("top","bottom");e=Math.max(g==="center"?e:0,e);m.css({top:"",bottom:"",left:"",right:"",margin:""});switch(g){case "center":m.css(v[0],"50%").css("margin-"+v[0],-Math.floor(n[f==="y"?"width":"height"]/2)+e);break;case v[0]:m.css(v[0],e);break;case v[1]:m.css(v[1],e);break}e=n[f==="x"?"width":"height"];if(z){o.toggleClass("ui-tooltip-accessible",!o.is(":visible"));e-=parseInt(w.css("border-"+a[f]+"-width"),
10)||0;o.removeClass("ui-tooltip-accessible")}m.css(a[f],-e)},destroy:function(){l.tip&&l.tip.remove();o.unbind(".qtip-tip")}});h.init()}function S(d){var b=this,h=d.options.show.modal,j=d.elements,l=j.tooltip;b.checks={"^show.modal.(on|blur)$":function(){b.init();j.overlay.toggle(l.is(":visible"))}};c.extend(b,{init:function(){if(h.on){l.unbind(".qtipmodal").bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(o,w,q){o=o.type.replace("tooltip","");c.isFunction(h[o])?h[o].call(j.overlay,q,
w):b[o](q)});b.create();h.blur===TRUE&&j.overlay.unbind(".qtipmodal"+d.id).bind("click.qtipmodal"+d.id,function(){d.hide.call(d)})}},create:function(){var o=c("#qtip-overlay");if(o.length)j.overlay=o;else{j.overlay=c("<div />",{id:"#qtip-overlay".substr(1),css:{position:"absolute",top:0,left:0,display:"none",zIndex:parseInt(l.css("z-index"),10)}}).appendTo(document.body);c(F).bind("resize.qtipmodal",function(){j.overlay.css({height:Math.max(c(F).height(),c(document).height()),width:Math.max(c(F).width(),
c(document).width())})}).trigger("resize")}},toggle:function(o){var w=j.overlay,q=d.options.show.modal.effect,n=o?"show":"hide";w.stop(TRUE,FALSE);if(c.isFunction(q))q.call(w,o);else q===FALSE?w[n]():w.fadeTo(90,o?0.7:0,function(){o||c(this).hide()})},show:function(){b.toggle(TRUE)},hide:function(){b.toggle(FALSE)},destroy:function(){var o=TRUE;c("*").each(function(){var w=c(this).data("qtip");if(w&&w.id!==d.id&&w.options.show.modal)return o=FALSE});if(o){j.overlay.remove();c(F).unbind(".qtipmodal")}else j.overlay.unbind(".qtipmodal"+
d.id);l.unbind("tooltipshow.qtipmodal tooltiphide.qtipmodal")}});b.init()}function T(d){var b=this,h=d.elements,j=h.tooltip,l=".bgiframe-"+d.id,o="tooltipmove"+l+" tooltipshow"+l;c.extend(b,{init:function(){h.bgiframe=c('<iframe class="ui-tooltip-bgiframe" frameborder="0" tabindex="-1" src="javascript:\'\';"  style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=0);"></iframe>');h.bgiframe.appendTo(j);j.bind(o,b.adjust)},adjust:function(){var w=d.get("dimensions"),q=d.plugins.tip,
n=d.elements.tip,B;B=parseInt(j.css("border-left-width"),10)||0;B={left:-B,top:-B};if(q&&n){q=q.corner.precedance==="x"?["width","left"]:["height","top"];B[q[1]]-=n[q[0]]()}h.bgiframe.css(B).css(w)},destroy:function(){b.iframe.remove();j.unbind(o)}});b.init()}c.fn.qtip=function(d,b,h){var j=String(d).toLowerCase(),l=NULL,o=j==="disable"?[TRUE]:c.makeArray(arguments).slice(1,10),w=o[o.length-1],q;if(!arguments.length&&this.data("qtip")||j==="api")return(q=this.data("qtip"))?q:L;else if("string"===
typeof d){this.each(function(){var n=c(this).data("qtip");if(!n)return TRUE;if(/option|set/.test(j)&&b)if(h!==L)n.set(b,h);else l=n.get(b);else{if(!n.rendered&&(j==="show"||j==="toggle")){if(w&&w.timeStamp)n.cache.event=w;n.render(1)}else if(j==="enable"){j="disable";o=[FALSE]}n[j]&&n[j].apply(n[j],o)}});return l!==NULL?l:this}else if("object"===typeof d||!arguments.length){q=M(c.extend(TRUE,{},d));return c.fn.qtip.bind.call(this,q,w)}};c.fn.qtip.bind=function(d,b){return this.each(function(){function h(B){function z(){n.render(typeof B===
"object"||j.show.ready);l.show.unbind(o.show);l.hide.unbind(o.hide)}if(n.cache.disabled)return FALSE;n.cache.event=c.extend({},B);if(j.show.delay>0){clearTimeout(n.timers.show);n.timers.show=setTimeout(z,j.show.delay);o.show!==o.hide&&l.hide.bind(o.hide,function(){clearTimeout(n.timers.show)})}else z()}var j,l,o,w=d.id=!d.id||d.id===FALSE||d.id.length<1||c("#ui-tooltip-"+d.id).length?c.fn.qtip.nextid++:d.id,q=".qtip-"+w+"-create",n=P.call(this,w,d);if(n===FALSE)return TRUE;j=n.options;c.each(c.fn.qtip.plugins,
function(){this.initialize==="initialize"&&this(n)});l={show:j.show.target,hide:j.hide.target};o={show:String(j.show.event).replace(" ",q+" ")+q,hide:String(j.hide.event).replace(" ",q+" ")+q};l.show.bind(o.show,h);if(j.show.ready||j.prerender)h(b)})};c.each({attr:function(d){var b=c(this),h=b.data("qtip");return arguments.length===1&&d==="title"&&h&&h.rendered===TRUE?b.data("oldtitle"):NULL},remove:c.ui?NULL:function(d,b){c(this).each(function(){if(!b)if(!d||c.filter(d,[this]).length)c("*",this).add(this).each(function(){c(this).triggerHandler("remove")})})}},
function(d,b){if(!b)return TRUE;c.fn["Old"+d]=c.fn[d];c.fn[d]=function(){return b.apply(this,arguments)||c.fn["Old"+d].apply(this,arguments)}});c(document.body).attr("role",function(d,b){return!b?"application":b});c(document).bind("mousemove.qtip",function(d){c.fn.qtip.mouse={pageX:d.pageX,pageY:d.pageY}});c.fn.qtip.version="2.0.0pre";c.fn.qtip.nextid=0;c.fn.qtip.inactiveEvents="click dblclick mousedown mouseup mousemove mouseleave mouseenter".split(" ");c.fn.qtip.zindex=15E3;c.fn.qtip.plugins={Corner:function(d){d=
...
widget:FALSE},events:{render:NULL,move:NULL,show:NULL,hide:NULL,toggle:NULL,focus:NULL,blur:NULL}};c.fn.qtip.plugins.ajax=function(d){var b=d.plugins.ajax,h=d.options.content.ajax;if(h&&h.url)if(b)return b;else{d.plugins.ajax=new Q(d);return d.plugins.ajax}};c.fn.qtip.plugins.ajax.initialize="render";c.fn.qtip.plugins.ajax.sanitize=function(d){var b=d.content;if(b&&"ajax"in b){b=b.ajax;if(typeof b!=="object")b=d.content.ajax={url:b};if("boolean"!==typeof b.once&&b.once)b.once=!!b.once}};c.extend(TRUE,
c.fn.qtip.defaults,{content:{ajax:{once:TRUE}}});c.fn.qtip.plugins.tip=function(d){var b=d.plugins.tip;return"object"===typeof b?b:d.plugins.tip=new R(d)};c.fn.qtip.plugins.tip.initialize="render";c.fn.qtip.plugins.tip.sanitize=function(d){var b=d.style;if(b&&"tip"in b){b=d.style.tip;if(typeof b!=="object")d.style.tip={corner:b};if(!/string|boolean/i.test(typeof b.corner))b.corner=true;if(typeof b.method!=="string")b.method=TRUE;if(!/canvas|polygon/i.test(b.method))b.method=TRUE;typeof b.width!==
"number"&&delete b.width;typeof b.height!=="number"&&delete b.height;typeof b.border!=="number"&&delete b.border;typeof b.offset!=="number"&&delete b.offset}};c.extend(TRUE,c.fn.qtip.defaults,{style:{tip:{corner:TRUE,mimic:FALSE,method:TRUE,width:8,height:8,border:0,offset:0}}});c.fn.qtip.plugins.imagemap=function(d,b){function h(C,a){for(var m=0,v=1,e=1,f=0,g=0,k=C.width,i=C.height;k>0&&i>0&&v>0&&e>0;){k=Math.floor(k/2);i=Math.floor(i/2);if(b.x==="left")v=k;else if(b.x==="right")v=C.width-k;else v+=
Math.floor(k/2);if(b.y==="top")e=i;else if(b.y==="bottom")e=C.height-i;else e+=Math.floor(i/2);for(m=a.length;m--;){if(a.length<2)break;f=a[m][0]-C.offset.left;g=a[m][1]-C.offset.top;if(b.x==="left"&&f>=v||b.x==="right"&&f<=v||b.x==="center"&&(f<v||f>C.width-v)||b.y==="top"&&g>=e||b.y==="bottom"&&g<=e||b.y==="center"&&(g<e||g>C.height-e))a.splice(m,1)}}return{left:a[0][0],top:a[0][1]}}var j=d.attr("shape").toLowerCase(),l=d.attr("coords").split(","),o=[],w=c('img[usemap="#'+d.parent("map").attr("name")+
'"]'),q=w.offset(),n={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},B=0,z=0;q.left+=Math.ceil((w.outerWidth()-w.width())/2);q.top+=Math.ceil((w.outerHeight()-w.height())/2);if(j==="poly")for(B=l.length;B--;){z=[parseInt(l[--B],10),parseInt(l[B+1],10)];if(z[0]>n.offset.right)n.offset.right=z[0];if(z[0]<n.offset.left)n.offset.left=z[0];if(z[1]>n.offset.bottom)n.offset.bottom=z[1];if(z[1]<n.offset.top)n.offset.top=z[1];o.push(z)}else o=c.map(l,function(C){return parseInt(C,10)});switch(j){case "rect":n=
{width:Math.abs(o[2]-o[0]),height:Math.abs(o[3]-o[1]),offset:{left:o[0],top:o[1]}};break;case "circle":n={width:o[2]+2,height:o[2]+2,offset:{left:o[0],top:o[1]}};break;case "poly":c.extend(n,{width:Math.abs(n.offset.right-n.offset.left),height:Math.abs(n.offset.bottom-n.offset.top)});n.offset=b.string()==="centercenter"?{left:n.offset.left+n.width/2,top:n.offset.top+n.height/2}:h(n,o.slice());n.width=n.height=0;break}n.offset.left+=q.left;n.offset.top+=q.top;return n};c.fn.qtip.plugins.modal=function(d){var b=
d.plugins.modal;return"object"===typeof b?b:d.plugins.tip=new S(d)};c.fn.qtip.plugins.modal.initialize="render";c.fn.qtip.plugins.modal.sanitize=function(d){if(d.show)if(typeof d.show.modal!=="object")d.show.modal={on:!!d.show.modal};else if(typeof d.show.modal.on==="undefined")d.show.modal.on=TRUE};c.extend(TRUE,c.fn.qtip.defaults,{show:{modal:{on:FALSE,effect:TRUE,blur:TRUE}}});c.fn.qtip.plugins.bgiframe=function(d){if(!(c.browser.msie&&/^6\.[0-9]/.test(c.browser.version)&&c("select, object").length))return FALSE;
var b=d.plugins.bgiframe;if(b)return b;else{d.plugins.bgiframe=new T(d);return d.plugins.bgiframe}};c.fn.qtip.plugins.bgiframe.initialize="render"})(jQuery,window);
