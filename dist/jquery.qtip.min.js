...
a.filter(b,[this]).length)a("*",this).add(this).each(function(){a(this).triggerHandler("remove")})})}},function(b,e){if(!e)return TRUE;var h=a.fn[b];a.fn[b]=function(){return e.apply(this,arguments)||h.apply(this,arguments)}});a.fn.qtip.nextid=0;a.fn.qtip.inactiveEvents="click dblclick mousedown mouseup mousemove mouseleave mouseenter".split(" ");a.fn.qtip.zindex=15E3;a.fn.qtip.plugins={Corner:function(b){this.x=(String(b).replace(/middle/i,"center").match(/left|right|center/i)||["false"])[0].toLowerCase();
this.y=(String(b).replace(/middle/i,"center").match(/top|bottom|center/i)||["false"])[0].toLowerCase();this.precedance=b.charAt(0).search(/^(t|b)/)>-1?"y":"x";this.string=function(){return this.precedance==="y"?this.y+this.x:this.x+this.y};this.abbreviation=function(){var e=this.x.substr(0,1),h=this.y.substr(0,1);return e===h?e:e==="c"||e!=="c"&&h!=="c"?h+e:e+h}}};a.fn.qtip.defaults={prerender:FALSE,id:FALSE,overwrite:TRUE,metadata:{type:"class"},content:{text:TRUE,attr:"title",title:{text:FALSE,
...
b.plugins.ajax,h=b.options.content;if(h.ajax&&h.ajax.url)if(e)return e;else{b.plugins.ajax=new Q(b);return b.plugins.ajax}};a.fn.qtip.plugins.ajax.initialize="render";a.fn.qtip.plugins.ajax.sanitize=function(b){if(b.content!==F)if(b.content.ajax!==F){if(typeof b.content.ajax!=="object")b.content.ajax={url:b.content.ajax};if(b.content.text===FALSE)b.content.text="Loading...";b.content.ajax.once=Boolean(b.content.ajax.once);b.content.ajax.preload=Boolean(b.content.ajax.preload);if(b.content.ajax.preload){b=
b.content.ajax.url;a("#qtip-preload").length||a('<div id="qtip-preload" class="ui-tooltip-accessible" />').appendTo(document.body);K.length||(K=a("<div />").appendTo("#qtip-preload").load(b+" img"))}}};a.fn.qtip.plugins.tip=function(b){var e=b.plugins.tip,h=b.options.style.tip;if(h&&h.corner)if(e)return e;else{b.plugins.tip=new R(b);b.plugins.tip.init();return b.plugins.tip}};a.fn.qtip.plugins.tip.initialize="render";a.fn.qtip.plugins.tip.sanitize=function(b){if(b.style===F)b.style={};if(b.style.tip===
F)b.style.tip={corner:TRUE};if(typeof b.style.tip!=="object")b.style.tip={corner:b.style.tip||TRUE};if(typeof b.style.tip.method!=="string")b.style.tip.method=TRUE;if(!/canvas|polygon/i.test(b.style.tip.method))b.style.tip.method=TRUE;if(typeof b.style.tip.width!=="number")b.style.tip.width=12;if(typeof b.style.tip.height!=="number")b.style.tip.height=12;if(typeof b.style.tip.border!=="number")b.style.tip.border=0};a.fn.qtip.plugins.imagemap=function(b,e){function h(d,s){for(var B=0,f=1,c=1,g=0,j=
0,k=d.width,l=d.height;k>0&&l>0&&f>0&&c>0;){k=Math.floor(k/2);l=Math.floor(l/2);if(e.x==="left")f=k;else if(e.x==="right")f=d.width-k;else f+=Math.floor(k/2);if(e.y==="top")c=l;else if(e.y==="bottom")c=d.height-l;else c+=Math.floor(l/2);for(B=s.length;B--;){if(s.length<2)break;g=s[B][0]-d.offset.left;j=s[B][1]-d.offset.top;if(e.x==="left"&&g>=f||e.x==="right"&&g<=f||e.x==="center"&&(g<f||g>d.width-f)||e.y==="top"&&j>=c||e.y==="bottom"&&j<=c||e.y==="center"&&(j<c||j>d.height-c))s.splice(B,1)}}return{left:s[0][0],
top:s[0][1]}}var i=b.attr("shape").toLowerCase(),o=b.attr("coords").split(","),m=[],w=a('img[usemap="#'+b.parent("map").attr("name")+'"]').offset(),n={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},x=0,r=0;if(i==="poly")for(x=o.length;x--;){r=[parseInt(o[--x],10),parseInt(o[x+1],10)];if(r[0]>n.offset.right)n.offset.right=r[0];if(r[0]<n.offset.left)n.offset.left=r[0];if(r[1]>n.offset.bottom)n.offset.bottom=r[1];if(r[1]<n.offset.top)n.offset.top=r[1];m.push(r)}else m=a.map(o,function(d){return parseInt(d,
10)});switch(i){case "rect":n={width:Math.abs(m[2]-m[0]),height:Math.abs(m[3]-m[1]),offset:{left:m[0],top:m[1]}};break;case "circle":n={width:m[2]+2,height:m[2]+2,offset:{left:m[0],top:m[1]}};break;case "poly":a.extend(n,{width:Math.abs(n.offset.right-n.offset.left),height:Math.abs(n.offset.bottom-n.offset.top)});n.offset=e.string()==="centercenter"?{left:n.offset.left+n.width/2,top:n.offset.top+n.height/2}:h(n,m.slice());n.width=n.height=0;break}n.offset.left+=w.left;n.offset.top+=w.top;return n};
a.fn.qtip.plugins.modal=function(b){var e=b.plugins.modal,h=b.options.show.modal;if(e)return e;else if(typeof h==="object"){b.plugins.modal=new S(b,h);return b.plugins.modal}};a.fn.qtip.plugins.modal.initialize="render";a.fn.qtip.plugins.modal.sanitize=function(b){if(b.show&&b.show.modal!==F)if(typeof b.show.modal!=="object")b.show.modal={}};a.fn.qtip.plugins.modal.defaults={effect:TRUE,blur:TRUE};a.fn.qtip.plugins.bgiframe=function(b){if(!(a.browser.msie&&/^6\.[0-9]/.test(a.browser.version)&&a("select, object").length))return FALSE;
var e=b.plugins.bgiframe;if(e)return e;else{b.plugins.bgiframe=new T(b);return b.plugins.bgiframe}};a.fn.qtip.plugins.bgiframe.initialize="render"})(jQuery,window);
