...
2)+"px solid "+A)});break}c(j);return h},destroy:function(){o.tip&&o.tip.remove();v.unbind("tooltipmove.tip")}})}function Q(b,c){var k=this,h=b.elements,q=h.tooltip;h.overlay=a("#qtip-overlay");a.extend(k,{init:function(){c=a.extend(TRUE,a.fn.qtip.plugins.modal.defaults,c);q.bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(o,v,p){o=o.type.replace("tooltip","");a.isFunction(c[o])?c[o].call(h.overlay,p,v):k[o](p)});h.overlay.length||k.create();c.blur===TRUE&&h.overlay.bind("click.qtipmodal"+
b.id,function(){b.hide.call(b)})},create:function(){h.overlay=a("<div />",{id:"qtip-overlay",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(B).bind("resize.qtipmodal",function(){h.overlay.css({height:Math.max(a(B).height(),a(document).height()),width:Math.max(a(B).width(),a(document).width())})}).trigger("resize")},toggle:function(o){var v=h.overlay,p=b.options.show.modal.effect,w=o?"show":"hide";if(a.isFunction(p))p.call(h.overlay);else p===FALSE?v[w]():v.fadeTo(90,
o?100:0)},show:function(){k.toggle(true)},hide:function(){k.toggle(false)},destroy:function(){var o=TRUE;a("*").each(function(){var v=a(this).data("qtip");if(v&&v.id!==b.id&&v.options.show.modal)return o=FALSE});if(o){h.overlay.remove();a(B).unbind("scroll.qtipmodal resize.qtipmodal")}else h.overlay.unbind("click.qtipmodal"+b.id);q.unbind("tooltipshow.qtipmodal tooltiphide.qtipmodal")}});k.init()}function R(b){var c=this,k=b.elements,h=k.tooltip,q=".bgiframe-"+b.id,o="tooltipmove"+q+" tooltipshow"+
