...
if(f==="mouse"){q={x:"left",y:"top"};d=d.type==="resize"||d.type==="scroll"?e.cache.event:x.mouse||!d||!d.pageX?a.extend({},a.fn.qtip.mouse):d;y={top:d.pageY,left:d.pageX}}else{if(f==="event")f=d&&d.target&&d.type!=="scroll"&&d.type!=="resize"?e.cache.target=a(d.target):e.cache.target;f=a(f).eq(0);if(f.length===0)return e;else if(f[0]===document||f[0]===F){r=f.width();E=f.height();if(f[0]===F)y={top:C?0:w.scrollTop,left:C?0:w.scrollLeft}}else if(f.is("area")&&a.fn.qtip.plugins.imagemap){y=a.fn.qtip.plugins.imagemap(f,
q);r=y.width;E=y.height;y=y.offset}else{r=f.outerWidth();E=f.outerHeight();y=p(f[0],t)}y.left+=q.x==="right"?r:q.x==="center"?r/2:0;y.top+=q.y==="bottom"?E:q.y==="center"?E/2:0}y.left+=x.x+(g.x==="right"?-D:g.x==="center"?-D/2:0);y.top+=x.y+(g.y==="bottom"?-u:g.y==="center"?-u/2:0);y.adjusted=x.screen&&f[0]!==F&&f[0]!==document.body?{left:J.left(y.left),top:J.top(y.top)}:{left:0,top:0};i.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(j+"-pos-"+g.abbreviation());
I.originalEvent=a.extend({},d);i.trigger(I,[e,y,w.elem]);if(I.isDefaultPrevented())return e;delete y.adjusted;if(i.is(":visible")&&a.isFunction(l.effect)){l.effect.call(i,e,y);i.queue(function(){var G=a(this);G.css({opacity:"",height:""});a.browser.msie&&this.style&&this.style.removeAttribute("filter");G.dequeue()})}else isNaN(y.left,y.top)||i.css(y);return e},redraw:function(){if(!e.rendered||!(a.browser.msie&&parseInt(a.browser.version.charAt(0),10)<9))return FALSE;var d=e.elements.tooltip;d.attr("style");
var f;d.css({width:"auto",height:"auto"});f=m("dimensions",1);a.each(["width","height"],function(i,l){var g=parseInt(d.css("max-"+l),10)||0,q=parseInt(d.css("min-"+l),10)||0;f[l]=g+q?Math.min(Math.max(f[l],q),g):f[l]});d.css(f)},disable:function(d){var f=e.elements.tooltip;if(e.rendered)f.toggleClass("ui-state-disabled",d);else e.cache.disabled=!!d;return e},destroy:function(){var d=e.elements,f=d.target.data("oldtitle");e.rendered&&a.each(e.plugins,function(){this.initialize==="render"&&this.destroy()});
z(1,1,1,1);c.removeData("qtip");e.rendered&&d.tooltip.remove();f&&c.attr("title",f);c.removeAttr("aria-describedby");return c}})}function Q(c,b){var k,h,p=a(this);k=a(document.body);var m=this===document?k:p;h=p.metadata?p.metadata(b.metadata):NULL;var s=a.extend(TRUE,{},a.fn.qtip.defaults,b,M((h&&b.metadata.type==="html5"?h[b.metadata.name]:NULL)||h));p.removeData("metadata");h=s.position;if("boolean"===typeof s.content.text)if(s.content.attr!==FALSE&&p.attr(s.content.attr))s.content.text=p.attr(s.content.attr);
else return FALSE;if(h.container===FALSE)h.container=k;if(h.target===FALSE)h.target=m;if(s.show.target===FALSE)s.show.target=m;if(s.hide.target===FALSE)s.hide.target=m;h.at=new a.fn.qtip.plugins.Corner(h.at);h.my=new a.fn.qtip.plugins.Corner(h.my);if(p.data("qtip"))if(s.overwrite)p.qtip("destroy");else if(s.overwrite===FALSE)return FALSE;p.attr("title")&&p.data("oldtitle",p.attr("title")).removeAttr("title");k=new P(p,s,c);p.data("qtip",k);return k}function R(c){var b=this;b.checks={"^content.ajax":function(){this.plugins.ajax.load(this.options.content.ajax)}};
