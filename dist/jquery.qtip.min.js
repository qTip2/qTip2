...
d.originalEvent=A;f.trigger(d,[e,g]);if(!d.isDefaultPrevented())f.addClass(q)[0].style.zIndex=g}return e},reposition:function(d){if(e.rendered===FALSE)return FALSE;var f=b.position.target,i=e.elements.tooltip,l=b.position,g=l.my,q=l.at,A=l.adjust,C=e.elements.tooltip.width(),w=e.elements.tooltip.height(),t=0,r=0,G=a.Event("tooltipmove"),J=i.css("position")==="fixed",v=l.container[0]!==document.body?l.container:a(F),u={left:0,top:0,adjusted:{left:0,top:0}},I={left:function(D){var E=v.scrollLeft,H=
q.x==="left"?t:q.x==="right"?-t:-t/2,K=E-D;E=D+C-v.width-E;H=(g.x==="left"?C:g.x==="right"?-C:-C/2)-(g.precedance==="x"||g.x===g.y?H:0);if(K>0&&(g.x!=="left"||E>0))u.left-=H;else if(E>0&&(g.x!=="right"||K>0))u.left-=(g.x==="center"?-1:1)*H+2*A.x;if(u.left<0&&-u.left>E)u.left=D;return u.left-D},top:function(D){var E=v.scrollTop,H=q.y==="top"?r:q.y==="bottom"?-r:-r/2,K=E-D;E=D+w-v.height-E;H=(g.y==="top"?w:g.y==="bottom"?-w:-w/2)-(g.precedance==="y"||g.x===g.y?H:0);if(K>0&&(g.y!=="top"||E>0))u.top-=
H;else if(E>0&&(g.y!=="bottom"||K>0))u.top-=(g.y==="center"?-1:1)*H+2*A.y;if(u.top<0&&-u.top>E)u.top=D;return u.top-D}};v={elem:v,height:v[(v[0]===F?"h":"outerH")+"eight"](),width:v[(v[0]===F?"w":"outerW")+"idth"](),scrollLeft:v.scrollLeft(),scrollTop:v.scrollTop()};if(f==="mouse"){q={x:"left",y:"top"};d=d&&(d.type==="resize"||d.type==="scroll")?e.cache.event:A.mouse||!d||!d.pageX?a.extend({},a.fn.qtip.mouse):d;u={top:d.pageY,left:d.pageX}}else{if(f==="event")f=d&&d.target&&d.type!=="scroll"&&d.type!==
"resize"?e.cache.target=a(d.target):e.cache.target;f=a(f).eq(0);if(f.length===0)return e;else if(f[0]===document||f[0]===F){t=f.width();r=f.height();if(f[0]===F)u={top:J?0:v.scrollTop,left:J?0:v.scrollLeft}}else if(f.is("area")&&a.fn.qtip.plugins.imagemap){u=a.fn.qtip.plugins.imagemap(f,q);t=u.width;r=u.height;u=u.offset}else{t=f.outerWidth();r=f.outerHeight();u=p(f[0],l.container[0])}u.left+=q.x==="right"?t:q.x==="center"?t/2:0;u.top+=q.y==="bottom"?r:q.y==="center"?r/2:0}u.left+=A.x+(g.x==="right"?
-C:g.x==="center"?-C/2:0);u.top+=A.y+(g.y==="bottom"?-w:g.y==="center"?-w/2:0);if(A.screen&&f[0]!==F&&f[0]!==document.body)u.adjusted={left:I.left(u.left),top:I.top(u.top)};i.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(j+"-pos-"+g.abbreviation());G.originalEvent=a.extend({},d);i.trigger(G,[e,u,v.elem]);if(G.isDefaultPrevented())return e;delete u.adjusted;if(i.is(":visible")&&a.isFunction(l.effect)){l.effect.call(i,e,u);i.queue(function(){var D=
a(this);D.css({opacity:"",height:""});a.browser.msie&&this.style&&this.style.removeAttribute("filter");D.dequeue()})}else isNaN(u.left,u.top)||i.css(u);return e},redraw:function(){if(!e.rendered||!(a.browser.msie&&parseInt(a.browser.version.charAt(0),10)<9))return FALSE;var d=e.elements.tooltip;d.attr("style");var f;d.css({width:"auto",height:"auto"});f=m("dimensions",1);a.each(["width","height"],function(i,l){var g=parseInt(d.css("max-"+l),10)||0,q=parseInt(d.css("min-"+l),10)||0;f[l]=g+q?Math.min(Math.max(f[l],
...
b.blur===TRUE&&h.overlay.bind("click.qtipmodal"+c.id,function(){c.hide.call(c)})},create:function(){h.overlay=a("<div />",{id:"qtip-overlay",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(F).bind("resize.qtipmodal",function(){h.overlay.css({height:Math.max(a(F).height(),a(document).height()),width:Math.max(a(F).width(),a(document).width())})}).trigger("resize")},toggle:function(m){var s=h.overlay,x=c.options.show.modal.effect,o=m?"show":"hide";s.stop(TRUE,FALSE);
