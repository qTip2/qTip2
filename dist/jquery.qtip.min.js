...
adjust:function(){var r=e.get("dimensions"),u=e.plugins.tip,q=e.elements.tip,D;D=parseInt(m.css("border-left-width"),10)||0;D={left:-D,top:-D};if(u&&q){u=u.corner.precedance==="x"?["width","left"]:["height","top"];D[u[1]]-=q[u[0]]()}p.bgiframe.css(D).css(r)},destroy:function(){b.iframe.remove();m.unbind(k)}});b.init()}var x=true,n=false,E=null,L="ui-tooltip",X="ui-widget",R="ui-state-disabled",U="div.qtip."+L,V=L+"-focus",Z="-31000px";d.fn.qtip=function(e,b,p){var m=String(e).toLowerCase(),l=E,k=
m==="disable"?[x]:d.makeArray(arguments).slice(1,10),r=k[k.length-1],u=this[0]?d.data(this[0],"qtip"):E;if(!arguments.length&&u||m==="api")return u;else if("string"===typeof e){this.each(function(){var q=d.data(this,"qtip");if(!q)return x;if(/option|set/.test(m)&&b)if(p!==S)q.set(b,p);else l=q.get(b);else{if(!q.rendered&&(m==="show"||m==="toggle")){if(r&&r.timeStamp)q.cache.event=r;q.render(1)}else if(m==="enable"){m="disable";k=[n]}q[m]&&q[m].apply(q[m],k)}});return l!==E?l:this}else if("object"===
typeof e||!arguments.length){u=W(d.extend(x,{},e));return d.fn.qtip.bind.call(this,u,r)}};d.fn.qtip.bind=function(e,b){return this.each(function(){function p(D){function y(){q.render(typeof D==="object"||m.show.ready);l.show.unbind(k.show);l.hide.unbind(k.hide)}if(q.cache.disabled)return n;q.cache.event=d.extend({},D);if(m.show.delay>0){clearTimeout(q.timers.show);q.timers.show=setTimeout(y,m.show.delay);k.show!==k.hide&&l.hide.bind(k.hide,function(){clearTimeout(q.timers.show)})}else y()}var m,l,
k,r=e.id=!e.id||e.id===n||e.id.length<1||d("#"+L+"-"+e.id).length?d.fn.qtip.nextid++:e.id,u=".qtip-"+r+"-create",q=$.call(this,r,e);if(q===n)return x;m=q.options;d.each(d.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(q)});l={show:m.show.target,hide:m.hide.target};k={show:String(m.show.event).replace(" ",u+" ")+u,hide:String(m.hide.event).replace(" ",u+" ")+u};l.show.bind(k.show,p);if(m.show.ready||m.prerender)p(b)})};d.each({attr:function(e){var b=d.data(this,"qtip");return arguments.length===
1&&e==="title"&&b&&b.rendered===x?d.data(this,"oldtitle"):E},remove:d.ui?E:function(e,b){d(this).each(function(){if(!b)if(!e||d.filter(e,[this]).length)d("*",this).add(this).each(function(){d(this).triggerHandler("remove")})})}},function(e,b){if(!b)return x;d.fn["Old"+e]=d.fn[e];d.fn[e]=function(){return b.apply(this,arguments)||d.fn["Old"+e].apply(this,arguments)}});d(N).load(function(){var e=document,b=e.body;d(e).bind("mousemove.qtip",function(p){d.fn.qtip.mouse={pageX:p.pageX,pageY:p.pageY}});
