...
"render"&&this.destroy()});p(1,1,1,1);b.removeData("qtip");e.rendered&&f.tooltip.remove();c&&b.attr("title",c);return b},hash:function(){var f=a.extend({},e);delete f.cache;delete f.timers;delete f.options;delete f.plugins;delete f.render;delete f.hash;return f}})}function R(b,d){var h;h=a(this).metadata?a(this).metadata(d.metadata):{};h=a.extend(TRUE,{},d,h);var i=h.position,q=a(this)[0]===document?a(document.body):a(this);if("boolean"===typeof h.content.text)if(h.content.attr!==FALSE&&a(this).attr(h.content.attr))h.content.text=
a(this).attr(h.content.attr);else return FALSE;if(i.container===FALSE)i.container=a(document.body);if(i.target===FALSE)i.target=q;if(h.show.target===FALSE)h.show.target=q;if(h.hide.target===FALSE)h.hide.target=q;i.at=new a.fn.qtip.plugins.Corner(i.at);i.my=new a.fn.qtip.plugins.Corner(i.my);if(a(this).data("qtip"))if(h.overwrite)a(this).qtip("destroy");else if(h.overwrite===FALSE)return FALSE;h=new Q(a(this),h,b);a(this).data("qtip",h);return h}function S(b){var d=this;d.checks={"^content.ajax":function(){this.plugins.ajax.load(this.options.content.ajax)}};
a.extend(d,{init:function(){var h=b.options.content.ajax;d.load(h);b.elements.tooltip.bind("tooltipshow.ajax",function(){h.once===FALSE&&b.rendered===TRUE&&d.load(h)})},load:function(h){a.ajax(a.extend(TRUE,{},h,{success:function(i,q){if(a.isFunction(h.success))if(h.success.call(b.hash(),i,q)===FALSE)return;b.set("content.text",i);M.remove()},error:function(i,q,l){var s=q||l;if(a.isFunction(h.error)){i=h.error.call(b.hash(),i,q,l);if(i===FALSE)return}b.set("content.text",s)}}));return d},destroy:function(){b.elements.tooltip.unbind("tooltipshow.ajax")}});
d.init()}function P(b,d,h){var i=Math.floor(d/2),q=Math.floor(h/2);d={bottomright:[[0,0],[d,h],[d,0]],bottomleft:[[0,0],[d,0],[0,h]],topright:[[0,h],[d,0],[d,h]],topleft:[[0,0],[0,h],[d,h]],topcenter:[[0,h],[i,0],[d,h]],bottomcenter:[[0,0],[d,0],[i,h]],rightcenter:[[0,0],[d,q],[0,h]],leftcenter:[[d,0],[d,h],[0,q]]};d.lefttop=d.bottomright;d.righttop=d.bottomleft;d.leftbottom=d.topright;d.rightbottom=d.topleft;return d[b]}function T(b){function d(c){var g=l.tip,k=["left","right"],j={left:0,right:0,
top:0,bottom:0},m=0;if(q.corner===FALSE||!g)return FALSE;c=c||i.corner;g.css({top:"",bottom:"",left:"",right:"",margin:""});k[c.precedance==="y"?"push":"unshift"]("top","bottom");if(a.browser.msie)j={top:c.precedance==="y"?0:0,left:0,bottom:c.precedance==="y"?0:0,right:0};switch(c[c.precedance==="y"?"x":"y"]){case "center":g.css(k[0],"50%").css("margin-"+k[0],-(p[c.precedance==="y"?"width":"height"]/2));break;case k[0]:g.css(k[0],j[k[0]]+B);break;case k[1]:g.css(k[1],j[k[1]]+B);break}m=p[c.precedance===
"x"?"width":"height"];if(u){s.toggleClass("ui-tooltip-accessible",!s.is(":visible"));m-=parseInt(n.css("border-"+c[c.precedance]+"-width"),10)||0;s.removeClass("ui-tooltip-accessible")}g.css(c[c.precedance],-1*j[c[c.precedance]]-m)}function h(c,g,k){if(l.tip){c=a.extend({},i.corner);g=i.mimic.adjust?a.extend({},i.mimic):NULL;var j=c.precedance==="y"?["y","top","left","height"]:["x","left","top","width"],m=k.adjusted,w=parseInt(n.css("border-"+c[j[0]]+"-width"),10)||0;a.each([c,g],function(){if(m.left)this.x=
this.x==="center"?m.left>0?"left":"right":this.x==="left"?"right":"left";if(m.top)this.y=this.y==="center"?m.top>0?"top":"bottom":this.y==="top"?"bottom":"top"});k[j[1]]+=(c[j[0]]===j[1]?1:-1)*(p[j[3]]-w);k[j[2]]-=B;if(c.string()!==x.corner.string()&&(x.top!==m.top||x.left!==m.left))i.update(c,g);x.left=m.left;x.top=m.top;x.corner=c}}var i=this,q=b.options.style.tip,l=b.elements,s=l.tooltip,n=l.wrapper,x={top:0,left:0,corner:{string:function(){}}},p={width:q.width,height:q.height},e={},u=q.border||
0,B=q.adjust||0,f=q.method||FALSE;i.corner=NULL;i.mimic=NULL;i.checks={"^position.my|style.tip.(corner|mimic|method|border)":function(){u=q.border;if(i.detectCorner()){i.create();i.detectColours();i.update()}else i.tip.remove();this.get("position.target")!=="mouse"&&this.reposition()},"^style.tip.(height|width)":function(){p={width:q.width,height:q.height};i.create();i.update();b.reposition()}};a.extend(i,{init:function(){if(f===TRUE)f=a("<canvas />")[0].getContext?"canvas":a.browser.msie&&(i.mimic&&
...
switch(f){case "canvas":l.tip.append('<canvas height="'+g+'" width="'+c+'" />');break;case "vml":l.tip.html('<vml:shape coordorigin="0 0" coordsize="'+c+" "+g+'" stroked="'+!!u+'"  style="behavior:url(#default#VML); display:inline-block; antialias:TRUE; position: absolute;  top:0; left:0; width:'+c+"px; height:"+g+"px; vertical-align:"+i.corner.y+';"><vml:stroke weight="'+(u-2)+'px" joinstyle="miter" miterlimit="10"  style="behavior:url(#default#VML); display:inline-block;" /></vml:shape>');break;
case "polygon":l.tip.append('<div class="ui-tooltip-tip-inner" />').append(u?'<div class="ui-tooltip-tip-border" />':"");break}return i},update:function(c,g){var k=l.tip,j=p.width,m=p.height,w=u>0?0:1,t=Math.ceil(u/2+0.5),v;g||(g=c?c:i.mimic);if(!c)c=i.corner;if(g.x==="false")g.x=c.x;if(g.y==="false")g.y=c.y;k=k.children();switch(f){case "canvas":k=k.get(0).getContext("2d");k.restore();k.clearRect(0,0,3E3,3E3);for(v=P(g.string(),j,m);w<2;w++){if(w){k.save();k.translate(Math.floor((g.x==="left"?1:
g.x==="right"?-1:0)*(u+1)*(g.precedance==="y"?0.5:1)),Math.floor((g.y==="top"?1:g.y==="bottom"?-1:0)*(u+1)*(g.precedance==="x"?0.5:1)))}k.beginPath();k.moveTo(v[0][0],v[0][1]);k.lineTo(v[1][0],v[1][1]);k.lineTo(v[2][0],v[2][1]);k.closePath();k.fillStyle=e[w?"fill":"border"];k.fill()}break;case "vml":v=P(g.string(),j,m);w="m"+v[0][0]+","+v[0][1]+" l"+v[1][0]+","+v[1][1]+" "+v[2][0]+","+v[2][1]+" xe";k.attr({path:w,fillcolor:e.fill});if(u){k.children().attr("color",e.border);if(g.precedance==="y"){k.css("top",
(g.y==="top"?1:-1)*(u-2));k.css("left",g.x==="left"?1:-2)}else{k.css("left",(g.x==="left"?1:-1)*(u-2));k.css("top",g.y==="top"?1:-2)}}break;case "polygon":if(g.precedance==="y"){w=j>m?1.5:j<m?5:2.2;t=[g.x==="left"?t:g.x==="right"?-t:0,Math.floor(w*t*(g.y==="bottom"?-1:1)*(g.x==="center"?0.8:1))]}else{w=j<m?1.5:j>m?5:2.2;t=[Math.floor(w*t*(g.x==="right"?-1:1)*(g.y==="center"?0.9:1)),g.y==="top"?t:g.y==="bottom"?-t:0]}k.removeAttr("style").each(function(o){var z={x:g.precedance==="x"?g.x==="left"?"right":
"left":g.x,y:g.precedance==="y"?g.y==="top"?"bottom":"top":g.y},r=g.x==="center"?["left","right",z.y,m,j]:["top","bottom",z.x,j,m],y=e[!o&&u?"border":"fill"];o&&a(this).css({position:"absolute","z-index":1,left:t[0],top:t[1]});g.x==="center"||g.y==="center"?a(this).css("border-"+r[2],r[3]+"px solid "+y).css("border-"+r[0],Math.floor(r[4]/2)+"px dashed transparent").css("border-"+r[1],Math.floor(r[4]/2)+"px dashed transparent"):a(this).css("border-width",Math.floor(m/2)+"px "+Math.floor(j/2)+"px").css("border-"+
z.x,Math.floor(j/2)+"px solid "+y).css("border-"+z.y,Math.floor(m/2)+"px solid "+y)});break}d(c);return i},destroy:function(){l.tip&&l.tip.remove();s.unbind("tooltipmove.tip")}})}function U(b,d){var h=this,i=b.elements,q=i.tooltip;i.overlay=a("#qtip-overlay");a.extend(h,{init:function(){d=a.extend(TRUE,a.fn.qtip.plugins.modal.defaults,d);q.bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(l,s,n){l=l.type.replace("tooltip","");a.isFunction(d[l])?d[l].call(i.overlay,n,s):h[l](n)});i.overlay.length||
h.create();d.blur===TRUE&&i.overlay.bind("click.qtipmodal"+b.id,function(){b.hide.call(b)})},create:function(){i.overlay=a("<div />",{id:"qtip-overlay",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(D).bind("resize.qtipmodal",function(){i.overlay.css({height:Math.max(a(D).height(),a(document).height()),width:Math.max(a(D).width(),a(document).width())})}).trigger("resize")},toggle:function(l){var s=i.overlay,n=b.options.show.modal.effect,x=l?"show":"hide";s.stop(TRUE,
FALSE);if(a.isFunction(n))n.call(s,l);else n===FALSE?s[x]():s.fadeTo(90,l?0.7:0,function(){l||a(this).hide()})},show:function(){h.toggle(TRUE)},hide:function(){h.toggle(FALSE)},destroy:function(){var l=TRUE;a("*").each(function(){var s=a(this).data("qtip");if(s&&s.id!==b.id&&s.options.show.modal)return l=FALSE});if(l){i.overlay.remove();a(D).unbind("scroll.qtipmodal resize.qtipmodal")}else i.overlay.unbind("click.qtipmodal"+b.id);q.unbind("tooltipshow.qtipmodal tooltiphide.qtipmodal")}});h.init()}
...
top:-p};if(n&&x){n=n.corner.precedance==="x"?["width","left"]:["height","top"];p[n[1]]-=x[n[0]]()}h.bgiframe.css(p).css(s)},destroy:function(){d.iframe.remove();i.unbind(l)}});d.init()}a.fn.qtip=function(b,d,h){var i=String(b).toLowerCase(),q=FALSE,l=i==="disable"?[TRUE]:a.makeArray(arguments).slice(1,10),s=l[l.length-1],n=a.extend(TRUE,{},b),x;if(!l.length&&a(this).data("qtip")||i==="api")return(n=a(this).eq(0).data("qtip"))?n.hash():F;else if("string"===typeof b){a(this).each(function(){var p=a(this).data("qtip");
if(!p)return TRUE;if(/option|set/.test(i)&&d)if(h!==F)p.set(d,h);else q=p.get(d);else{if(!p.rendered&&(i==="show"||i==="toggle")){if(s&&s.timeStamp)p.cache.event=s;p.render()}else if(i==="enable"){i="disable";l=[FALSE]}p[i]&&p[i].apply(p[i],l)}});return q!==FALSE?q:a(this)}else if("object"===typeof b){x=N(n,this);n=a.extend(TRUE,{},a.fn.qtip.defaults,n);return a.fn.qtip.bind.call(x,n,s)}};a.fn.qtip.bind=function(b,d){return a(this).each(function(h){function i(u){function B(){l.render(typeof u==="object"||
s.show.ready);n.show.unbind(x.show);n.hide.unbind(x.hide)}if(l.cache.disabled)return FALSE;l.cache.event=a.extend({},u);if(s.show.delay>0){clearTimeout(l.timers.show);l.timers.show=setTimeout(B,s.show.delay);x.show!==x.hide&&n.hide.bind(x.hide,function(){clearTimeout(l.timers.show)})}else B()}var q,l,s,n,x,p,e=b.content.text;q=b.id===FALSE||b.id.length<1||a("#ui-tooltip-"+b.id).length?a.fn.qtip.nextid++:b.id;p=".qtip-"+q+"-create";l=R.call(a(this),q,b);if(l===FALSE)return TRUE;s=l.options;if(a.isArray(e))s.content.text=
e[h];a(this).attr("title")&&a(this).data("oldtitle",a(this).attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(l)});n={show:s.show.target,hide:s.hide.target};x={show:String(s.show.event).replace(" ",p+" ")+p,hide:String(s.hide.event).replace(" ",p+" ")+p};n.show.bind(x.show,i);if(b.show.ready||b.prerender||b.show.event===FALSE)i(d)})};a.each({attr:function(b){var d=a(this).data("qtip");return arguments.length===1&&b==="title"&&d&&d.rendered===
TRUE?a(this).data("oldtitle"):NULL},remove:a.ui?NULL:function(b,d){this.each(function(){if(!d)if(!b||a.filter(b,[this]).length)a("*",this).add(this).each(function(){a(this).triggerHandler("remove")})})}},function(b,d){if(!d)return TRUE;var h=a.fn[b];a.fn[b]=function(){return d.apply(this,arguments)||h.apply(this,arguments)}});a(document.body).attr("role",function(b,d){return!d?"application":d});a(document).bind("mousemove.qtip",function(b){a.fn.qtip.mouse={pageX:b.pageX,pageY:b.pageY}});a.fn.qtip.nextid=
...
"number")b.style.tip.width=12;if(typeof b.style.tip.height!=="number")b.style.tip.height=12;if(typeof b.style.tip.border!=="number")b.style.tip.border=0};a.fn.qtip.plugins.imagemap=function(b,d){function h(e,u){for(var B=0,f=1,c=1,g=0,k=0,j=e.width,m=e.height;j>0&&m>0&&f>0&&c>0;){j=Math.floor(j/2);m=Math.floor(m/2);if(d.x==="left")f=j;else if(d.x==="right")f=e.width-j;else f+=Math.floor(j/2);if(d.y==="top")c=m;else if(d.y==="bottom")c=e.height-m;else c+=Math.floor(m/2);for(B=u.length;B--;){if(u.length<
2)break;g=u[B][0]-e.offset.left;k=u[B][1]-e.offset.top;if(d.x==="left"&&g>=f||d.x==="right"&&g<=f||d.x==="center"&&(g<f||g>e.width-f)||d.y==="top"&&k>=c||d.y==="bottom"&&k<=c||d.y==="center"&&(k<c||k>e.height-c))u.splice(B,1)}}return{left:u[0][0],top:u[0][1]}}var i=b.attr("shape").toLowerCase(),q=b.attr("coords").split(","),l=[],s=a('img[usemap="#'+b.parent("map").attr("name")+'"]').offset(),n={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},x=0,p=0;if(i==="poly")for(x=q.length;x--;){p=
[parseInt(q[--x],10),parseInt(q[x+1],10)];if(p[0]>n.offset.right)n.offset.right=p[0];if(p[0]<n.offset.left)n.offset.left=p[0];if(p[1]>n.offset.bottom)n.offset.bottom=p[1];if(p[1]<n.offset.top)n.offset.top=p[1];l.push(p)}else l=a.map(q,function(e){return parseInt(e,10)});switch(i){case "rect":n={width:Math.abs(l[2]-l[0]),height:Math.abs(l[3]-l[1]),offset:{left:l[0],top:l[1]}};break;case "circle":n={width:l[2]+2,height:l[2]+2,offset:{left:l[0],top:l[1]}};break;case "poly":a.extend(n,{width:Math.abs(n.offset.right-
