...
r.left-=v.left.charAt?v.option:(v.right?-1:1)*v.left;r.top-=v.top.charAt?v.option:(v.bottom?-1:1)*v.top;s.left=b.left;s.top=b.top;s.corner=k.string()}}function l(k,b,r){b=!b?k[k.precedance]:b;b="border-"+b+"-width";k=parseInt((j.titlebar&&k.y==="top"?j.titlebar:j.content).css(b),10);return(r?k||parseInt(x.css(b),10):k)||0}var h=this,m=e.options.style.tip,j=e.elements,x=j.tooltip,s={top:0,left:0,corner:""},n={width:m.width,height:m.height},z={},B=m.border||0,E=m.method||FALSE;h.corner=NULL;h.mimic=
NULL;h.checks={"^position.my|style.tip.(corner|mimic|method|border)$":function(){h.init()||h.destroy();e.reposition()},"^style.tip.(height|width)$":function(){n={width:m.width,height:m.height};h.create();h.update();e.reposition()},"^content.title.text|style.(classes|widget)$":function(){if(j.tip){h.detectColours();h.update()}}};c.extend(h,{init:function(){var k=h.detectCorner();if(k){if(E!=="polygon")E=c("<canvas />")[0].getContext?"canvas":c.browser.msie?"vml":"polygon";h.create();h.detectColours();
h.update();x.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",a)}return k},detectCorner:function(){var k=m.corner,b=e.options.position,r=b.at;b=b.my.string?b.my.string():b.my;if(k===FALSE||b===FALSE&&r===FALSE)return FALSE;else if(k===TRUE)h.corner=new c.fn.qtip.plugins.Corner(b);else if(!k.string){h.corner=new c.fn.qtip.plugins.Corner(k);h.corner.fixed=TRUE}return h.corner.string()!=="centercenter"},detectColours:function(){var k=j.tip.css({backgroundColor:"",border:""}),b=h.corner,r=b[b.precedance],
v="border-"+r+"-color";r="border"+r.charAt(0)+r.substr(1)+"Color";var p=/rgba?\(0, 0, 0(, 0)?\)|transparent/i;b=j.titlebar&&b.y==="top"?j.titlebar:j.content;z.fill=k.css("background-color")||"transparent";z.border=k[0].style[r];if(!z.fill||p.test(z.fill)){z.fill=b.css("background-color");if(p.test(z.fill))z.fill=x.css("background-color")}if(!z.border||p.test(z.border)){z.border=x.css(v);if(p.test(z.border))z.border=b.css(v)||z.fill}c("*",k).add(k).css("background-color","transparent").css("border",
0)},create:function(){var k=n.width,b=n.height;j.tip&&j.tip.remove();j.tip=c("<div />",{"class":"ui-tooltip-tip"}).css(n).toggleClass("ui-widget-content",e.options.style.widget).prependTo(x);switch(E){case "canvas":c('<canvas height="'+b+'" width="'+k+'" />').appendTo(j.tip)[0].getContext("2d").save();break;case "vml":k='<vml:shape coordorigin="0,0" coordsize="'+k+" "+b+'" stroked="false"  style="behavior:url(#default#VML); display:inline-block; position:absolute; antialias:false; left: 0; top: 0; width:'+
k+"px; height:"+b+'px;"></vml:shape>';j.tip.html(B?k+=k:k);break;case "polygon":j.tip.append('<div class="ui-tooltip-tip-inner" />').append(B?'<div class="ui-tooltip-tip-border" />':"");break}},update:function(k){var b=j.tip.children(),r=n.width,v=n.height,p=m.mimic,d,f,i,o,g;if(!k)k=h.corner;if(p===FALSE)p=k;else{p=new c.fn.qtip.plugins.Corner(p);p.precedance=k.precedance;if(p.x==="inherit")p.x=k.x;else if(p.y==="inherit")p.y=k.y;else if(p.x===p.y)p[k.precedance]=k[k.precedance]}B=m.border===TRUE?
l(k,NULL,TRUE):m.border;d=B>0?0:1;o=p.string().indexOf("center")>-1;g=Math[/b|r/.test(p[p.precedance==="y"?"x":"y"])?"ceil":"floor"];switch(E){case "canvas":f=b[0].getContext("2d");f.restore&&f.restore();f.clearRect(0,0,3E3,3E3);for(i=P(p,r,v);d<2;d++){if(d){f.save();f.translate(g((p.x==="left"?1:p.x==="right"?-1:0)*(B+1)*(p.precedance==="y"?0.5:1)),g((p.y==="top"?1:p.y==="bottom"?-1:0)*(B+1)*(p.precedance==="x"?0.5:1)))}f.beginPath();f.moveTo(i[0][0],i[0][1]);f.lineTo(i[1][0],i[1][1]);f.lineTo(i[2][0],
i[2][1]);f.closePath();f.fillStyle=z[d?"fill":"border"];f.fill()}break;case "vml":i=P(p,r,v);i="m"+i[0][0]+","+i[0][1]+" l"+i[1][0]+","+i[1][1]+" "+i[2][0]+","+i[2][1]+" xe";b.each(function(q){c(this).attr({path:i,fillcolor:z[q||!B?"fill":"border"]}).css("antialias",""+o)});break;case "polygon":b.removeAttr("style").css({position:"absolute",left:0,top:0}).each(function(q){var C={x:p.precedance==="x"?p.x==="left"?"right":"left":p.x,y:p.precedance==="y"?p.y==="top"?"bottom":"top":p.y},D=p.x==="center"?
["left","right",C.y,v,r]:["top","bottom",C.x,r,v];q=z[!q&&B?"border":"fill"];p.x==="center"||p.y==="center"?c(this).css("border-"+D[2],D[3]+"px solid "+q).css("border-"+D[0],g(D[4]/2)+"px dashed transparent").css("border-"+D[1],g(D[4]/2)+"px dashed transparent"):c(this).css("border-width",g(v/2)+"px "+g(r/2)+"px").css("border-"+C.x,g(r/2)+"px solid "+q).css("border-"+C.y,g(v/2)+"px solid "+q)});break}if(E!=="canvas"&&B){d=[B*2.75,B];d=p.precedance==="y"?[p.x==="left"?d[1]:p.x==="right"?-d[1]:0,p.y===
"bottom"?-d[0]:d[0]]:[p.x==="left"?d[0]:-d[0],p.y==="bottom"?-d[1]:p.y==="top"?d[1]:0];b.eq(1).css({left:d[0],top:d[1]})}return h.position(k,1)},position:function(k,b){var r=j.tip,v={},p=Math.max(0,m.offset),d,f;if(m.corner===FALSE||!r)return FALSE;k=k||h.corner;d=k.precedance;f=n[d==="x"?"width":"height"];c.each(d==="y"?[k.x,k.y]:[k.y,k.x],function(i,o){var g;if(o==="center"){g=d==="y"?"left":"top";v[g]="50%";v["margin-"+g]=-Math.floor(f/2)+p}else{g=l(k,o,TRUE);if(i||B===L)g=l(k,o);else{if(g)g=0;
else{g=c.browser.mozilla;var q=k.y+(g?"":"-")+k.x;g=(g?"-moz-":c.browser.webkit?"-webkit-":"")+(g?"border-radius-"+q:"border-"+q+"-radius");g=parseInt((j.titlebar&&k.y==="top"?j.titlebar:j.content).css(g),10)||parseInt(x.css(g),10)||0}g=p+g}v[o]=g}});v[k[d]]-=f;b&&r.css({top:"",bottom:"",left:"",right:"",margin:""}).css(v);return v},destroy:function(){j.tip&&j.tip.remove();x.unbind(".qtip-tip")}});h.init()}function U(e){var a=this,l=e.options.show.modal,h=e.elements,m=h.tooltip;a.checks={"^show.modal.(on|blur)$":function(){a.init();
