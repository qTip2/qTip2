...
"h":"outerH")+"eight"](),width:w[(w[0]===F?"w":"outerW")+"idth"](),scrollLeft:w.scrollLeft(),scrollTop:w.scrollTop()};if(f==="mouse"){r={x:"left",y:"top"};d=d.type==="resize"||d.type==="scroll"?e.cache.event:z.mouse||!d||!d.pageX?a.extend({},a.fn.qtip.mouse):d;x={top:d.pageY,left:d.pageX}}else{if(f==="event")f=d&&d.target&&d.type!=="scroll"&&d.type!=="resize"?e.cache.target=a(d.target):e.cache.target;f=a(f).eq(0);if(f.length===0)return e;else if(f[0]===document||f[0]===F){q=f.width();E=f.height();
