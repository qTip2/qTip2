...
q.border;if(i.init())this.get("position.target")!=="mouse"&&this.reposition();else i.destroy()},"^style.tip.(height|width)":function(){u={width:q.width,height:q.height};i.create();i.update();c.reposition()},"^style.classes$":function(){i.detectColours();i.update()}};a.extend(i,{init:function(){var j=a.browser.msie,m=i.detectCorner(),b=i[i.mimic?"mimic":"corner"].string().indexOf("center")>-1;if(m){if(e===TRUE)e=a("<canvas />")[0].getContext?"canvas":j&&(b||u.height!==u.width)?"vml":"polygon";else if(e===
"canvas")e=j?"vml":!a("<canvas />")[0].getContext?"polygon":"canvas";else if(e==="polygon")e=j&&b?"vml":e;i.create();i.detectColours();i.update();t.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",h)}return m},detectCorner:function(){var j=q.corner,m=c.options.position.at,b=c.options.position.my;if(b.string)b=b.string();if(j===FALSE||b===FALSE&&m===FALSE)return FALSE;else if(j===TRUE)i.corner=new a.fn.qtip.plugins.Corner(b);else if(!j.string){i.corner=new a.fn.qtip.plugins.Corner(j);i.corner.fixed=
...
h.bgiframe.appendTo(i);i.bind(l,d.adjust)},adjust:function(){var t=c.get("dimensions"),s=c.plugins.tip,r=c.elements.tip,u;u=parseInt(i.css("border-left-width"),10)||0;u={left:-u,top:-u};if(s&&r){s=s.corner.precedance==="x"?["width","left"]:["height","top"];u[s[1]]-=r[s[0]]()}h.bgiframe.css(u).css(t)},destroy:function(){d.iframe.remove();i.unbind(l)}});d.init()}a.fn.qtip=function(c,d,h){var i=String(c).toLowerCase(),q=NULL,l=i==="disable"?[TRUE]:a.makeArray(arguments).slice(1,10),t=l[l.length-1],s=
a.extend(TRUE,{},c),r;if(!arguments.length&&this.data("qtip")||i==="api")return(s=this.data("qtip"))?s.hash():K;else if("string"===typeof c){this.each(function(){var u=a(this).data("qtip");if(!u)return TRUE;if(/option|set/.test(i)&&d)if(h!==K)u.set(d,h);else q=u.get(d);else{if(!u.rendered&&(i==="show"||i==="toggle")){if(t&&t.timeStamp)u.cache.event=t;u.render(1)}else if(i==="enable"){i="disable";l=[FALSE]}u[i]&&u[i].apply(u[i],l)}});return q!==NULL?q:this}else if("object"===typeof c||!arguments.length){r=
...
a(this),l=c.id,t=c.content.text,s,r,u,x,w;c.id=l=l===FALSE||l.length<1||a("#ui-tooltip-"+l).length?a.fn.qtip.nextid++:l;w=".qtip-"+l+"-create";s=P.call(this,l,c);if(s===FALSE)return TRUE;r=s.options;if(a.isArray(t))r.content.text=t[h];q.attr("title")&&q.data("oldtitle",q.attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(s)});u={show:r.show.target,hide:r.hide.target};x={show:String(r.show.event).replace(" ",w+" ")+w,hide:String(r.hide.event).replace(" ",
...
2)break;f=e[j][0]-w.offset.left;k=e[j][1]-w.offset.top;if(d.x==="left"&&f>=m||d.x==="right"&&f<=m||d.x==="center"&&(f<m||f>w.width-m)||d.y==="top"&&k>=b||d.y==="bottom"&&k<=b||d.y==="center"&&(k<b||k>w.height-b))e.splice(j,1)}}return{left:e[0][0],top:e[0][1]}}var i=c.attr("shape").toLowerCase(),q=c.attr("coords").split(","),l=[],t=a('img[usemap="#'+c.parent("map").attr("name")+'"]'),s=t.offset(),r={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},u=0,x=0;s.left+=Math.ceil((t.outerWidth()-
t.width())/2);s.top+=Math.ceil((t.outerHeight()-t.height())/2);if(i==="poly")for(u=q.length;u--;){x=[parseInt(q[--u],10),parseInt(q[u+1],10)];if(x[0]>r.offset.right)r.offset.right=x[0];if(x[0]<r.offset.left)r.offset.left=x[0];if(x[1]>r.offset.bottom)r.offset.bottom=x[1];if(x[1]<r.offset.top)r.offset.top=x[1];l.push(x)}else l=a.map(q,function(w){return parseInt(w,10)});switch(i){case "rect":r={width:Math.abs(l[2]-l[0]),height:Math.abs(l[3]-l[1]),offset:{left:l[0],top:l[1]}};break;case "circle":r={width:l[2]+
