...
clearTimeout(f.timers.hide);var A=function(){f.show(C)};if(c.show.delay>0)f.timers.show=setTimeout(A,c.show.delay);else A()}function o(C){if(q.tooltip.hasClass("ui-state-disabled"))return FALSE;var A=a(C.relatedTarget||C.target).parents(n)[0]==q.tooltip[0];clearTimeout(f.timers.show);clearTimeout(f.timers.hide);if(c.hide.fixed&&(c.position.target==="mouse"&&A||/mouse(out|leave|move)/.test(C.type)&&A)){C.stopPropagation();C.preventDefault();return FALSE}q.tooltip.stop(TRUE);if(c.hide.delay>0)f.timers.hide=
setTimeout(function(){f.hide(C)},c.hide.delay);else f.hide(C)}function s(C){if(q.tooltip.hasClass("ui-state-disabled"))return FALSE;clearTimeout(f.timers.inactive);f.timers.inactive=setTimeout(function(){f.hide(C)},c.hide.inactive)}function w(C){f.elements.tooltip.is(":visible")&&f.reposition(C)}var u=".qtip-"+h,q={show:c.show.target,hide:c.hide.target,tooltip:f.elements.tooltip},r={show:String(c.show.event).split(" "),hide:String(c.hide.event).split(" ")},F=a.browser.msie&&/^6\.[0-9]/.test(a.browser.version);
d.bind("remove.qtip",function(){f.destroy()});if(i&&c.hide.fixed){q.hide=q.hide.add(q.tooltip);q.tooltip.bind("mouseover"+u,function(){q.tooltip.hasClass("ui-state-disabled")||clearTimeout(f.timers.hide)})}if(e){if("number"===typeof c.hide.inactive){q.show.bind("qtip-"+h+"-inactive",s);a.each(a.fn.qtip.inactiveEvents,function(C,A){q.hide.add(f.elements.tooltip).bind(A+u+"-inactive",s)})}a.each(r.hide,function(C,A){var H=a.inArray(A,r.show);if(H>-1&&a(q.hide).add(q.show).length===a(q.hide).length||
A==="unfocus"){q.show.bind(A+u,function(D){q.tooltip.is(":visible")?o(D):k(D)});delete r.show[H]}else q.hide.bind(A+u,o)})}if(b){a.each(r.show,function(C,A){q.show.bind(A+u,k)});q.tooltip.bind("mouseover"+u,function(){f.focus()})}if(j){if(c.position.adjust.resize||c.position.adjust.screen)a(E).bind("resize"+u,w);if(c.position.adjust.screen||F&&q.tooltip.css("position")==="fixed")a(document).bind("scroll"+u,w);/unfocus/i.test(c.hide.event)&&a(document).bind("mousedown"+u,function(C){var A=f.elements.tooltip;
...
{show:String(c.show.event).split(" "),hide:String(c.hide.event).split(" ")};if(f.rendered){a.each(e.show,function(s,w){o.show.unbind(w+k)});o.show.unbind("mousemove"+k).unbind("mouseout"+k).unbind("qtip-"+h+"-inactive");a.each(e.hide,function(s,w){o.hide.add(o.tooltip).unbind(w+k)});a.each(a.fn.qtip.inactiveEvents,function(s,w){o.hide.add(i?f.elements.content:NULL).unbind(w+k+"-inactive")});o.hide.unbind("mouseout"+k);o.tooltip.unbind("mouseover"+k);if(j){a(E).unbind("resize"+k);a(document).unbind("mousedown"+
...
0)r.left+=L-G+K;return r.left-D},top:function(D){var I=A.scrollTop(),J=A.height(),G=o.y==="top"?q:o.y==="bottom"?-q:0,K=k.y!==o.y&&o.y!=="center"?q:0,L=G+(k.y==="top"?-w:k.y==="bottom"?w:-w/2)+-2*j.adjust.y;J=D+w-J-I;if(I-D>0)r.top+=(k.y==="center"?-1:1)*(L-G-K);else if(J>0)r.top+=L-G-K;return r.top-D}};if(e==="mouse"){o={x:"left",y:"top"};b=j.adjust.mouse||!b?a.extend({},a.fn.qtip.mouse):b;r={top:b.pageY,left:b.pageX}}else{if(e==="event")e=b&&b.target&&b.type!=="scroll"&&b.type!=="resize"?f.cache.target=
a(b.target):f.cache.target;e=a(e).eq(0);if(e.length===0)return f;else if(e[0]===document||e[0]===E){u=e.width();q=e.height();if(e[0]===E)r={top:C?0:A.scrollTop(),left:C?0:A.scrollLeft()}}else if(e.is("area")&&a.fn.qtip.plugins.imagemap){r=a.fn.qtip.plugins.imagemap(e,o);u=r.width;q=r.height;r=r.offset}else{u=e.outerWidth();q=e.outerHeight();r=p(e)}r.left+=o.x==="right"?u:o.x==="center"?u/2:0;r.top+=o.y==="bottom"?q:o.y==="center"?q/2:0}r.left+=j.adjust.x+(k.x==="right"?-s:k.x==="center"?-s/2:0);r.top+=
j.adjust.y+(k.y==="bottom"?-w:k.y==="center"?-w/2:0);r.adjusted=j.adjust.screen&&e[0]!==E&&e[0]!==document.body?{left:H.left(r.left),top:H.top(r.top)}:{left:0,top:0};i.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(x+"-pos-"+k.abbreviation());F.originalEvent=a.extend({},b);i.trigger(F,[f.hash(),r]);if(F.isDefaultPrevented())return f;delete r.adjusted;if(j.container[0]==document.body){if(r.top+A.scrollTop()<1)r.top=0;if(r.left+A.scrollLeft()<1)r.left=
...
"render"&&this.destroy()});B(1,1,1,1);d.removeData("qtip");f.rendered&&b.tooltip.remove();e&&d.attr("title",e);d.removeAttr("aria-describedby");return d},hash:function(){var b=a.extend({},f);delete b.cache;delete b.timers;delete b.options;delete b.plugins;delete b.render;delete b.hash;return b}})}function R(d,c){var h,g=a(this);h=a(document.body);var p=g.metadata?g.metadata(c.metadata):{};p=a.extend(TRUE,{},c,N(a.extend(TRUE,{},(p&&c.metadata.type==="html5"?p[c.metadata.name]:{})||p)));var l=p.position,
v=this===document?h:g;g.removeData("metadata");if("boolean"===typeof p.content.text)if(p.content.attr!==FALSE&&g.attr(p.content.attr))p.content.text=g.attr(p.content.attr);else return FALSE;if(l.container===FALSE)l.container=h;if(l.target===FALSE)l.target=v;if(p.show.target===FALSE)p.show.target=v;if(p.hide.target===FALSE)p.hide.target=v;l.at=new a.fn.qtip.plugins.Corner(l.at);l.my=new a.fn.qtip.plugins.Corner(l.my);if(g.data("qtip"))if(p.overwrite)g.qtip("destroy");else if(p.overwrite===FALSE)return FALSE;
