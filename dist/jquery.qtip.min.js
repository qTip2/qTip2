...
g=j.at,l=e.elements.tooltip.width(),m=e.elements.tooltip.height(),t=a(j.container)[0],r=0,v=0,s={left:0,top:0},y=a.Event("tooltipmove"),z={left:function(A){var C=i==="mouse"?d.pageX:i.offset().left,F=a(B).scrollLeft(),E=a(B).width(),H=g.x==="left"?r:g.x==="right"?-r:r/2,G=f.x!==g.x&&g.x!=="center"?r:0,L=H+(f.x==="left"?-l:f.x==="right"?l:l/2)+-2*j.adjust.x;E=A+l-E-F;if(F-A>0&&!(A>=C&&A<C+r))s.left+=L-H+G;else if(E>0&&A+l>C)s.left+=(f.x==="center"?-1:1)*(L-H-G);return s.left-A},top:function(A){var C=
a(B).scrollTop(),F=a(B).height(),E=g.y==="top"?v:g.y==="bottom"?-v:0,H=f.y!==g.y&&g.y!=="center"?v:0,G=E+(f.y==="top"?-m:f.y==="bottom"?m:-m/2)+-2*j.adjust.y;F=A+m-F-C;if(C-A>0)s.top+=f.y==="center"?-G+E:G;else if(F>0)s.top+=G-E-H;return s.top-A}};if(i==="mouse"){g={x:"left",y:"top"};if(!d)d=e.cache.event;s={top:d.pageY,left:d.pageX}}else{if(i==="event")i=d&&d.target&&d.type!=="scroll"&&d.type!=="resize"?e.cache.target=a(d.target):e.cache.target;if(i[0]===document||i[0]===B){r=i.width();v=i.height();
s={top:n.css("position")==="fixed"?0:i.scrollTop(),left:i.scrollLeft()}}else if(i.is("area")&&a.fn.qtip.plugins.imagemap){s=a.fn.qtip.plugins.imagemap(i,g);r=s.width;v=s.height;s=s.offset}else{r=i.outerWidth();v=i.outerHeight();s=i.offset();if(j.adjust.offset){do{s.left-=t.offsetLeft-t.scrollLeft;s.top-=t.offsetTop-t.scrollTop}while(t=t.offsetParent)}}s.left+=g.x==="right"?r:g.x==="center"?r/2:0;s.top+=g.y==="bottom"?v:g.y==="center"?v/2:0}s.left+=j.adjust.x+(f.x==="right"?-l:f.x==="center"?-l/2:
...
d.tooltip.remove();i&&b.attr("title",i);return b},hash:function(){var d=a.extend({},e);delete d.cache;delete d.timers;delete d.options;delete d.plugins;delete d.render;delete d.hash;return d}})}function N(b,c){var k;k=a(this).metadata?a(this).metadata(c.metadata):{};k=a.extend(TRUE,{},c,k);var h=k.position,p=a(this)[0]===document?a(document.body):a(this);if("boolean"===typeof k.content.text)if(k.content.attr!==FALSE&&a(this).attr(k.content.attr))k.content.text=a(this).attr(k.content.attr);else return FALSE;
...
a.extend(c,{init:function(){var k=b.options.content.ajax;c.load(k);b.elements.tooltip.bind("tooltipshow.ajax",function(){k.once===FALSE&&b.rendered===TRUE&&c.load(k)})},load:function(k){a.ajax(a.extend(TRUE,{},k,{success:function(h,p){if(a.isFunction(k.success))if(k.success.call(b.hash(),h,p)===FALSE)return;b.set("content.text",h);I.remove()},error:function(h,p,o){var w=p||o;if(a.isFunction(k.error)){h=k.error.call(b.hash(),h,p,o);if(h===FALSE)return}b.set("content.text",w)}}));return c},destroy:function(){b.elements.tooltip.unbind("tooltipshow.ajax")}});
c.init()}function J(b,c,k){var h=Math.floor(c/2),p=Math.floor(k/2);c={bottomright:[[0,0],[c,k],[c,0]],bottomleft:[[0,0],[c,0],[0,k]],topright:[[0,k],[c,0],[c,k]],topleft:[[0,0],[0,k],[c,k]],topcenter:[[0,k],[h,0],[c,k]],bottomcenter:[[0,0],[c,0],[h,k]],rightcenter:[[0,0],[c,p],[0,k]],leftcenter:[[c,0],[c,k],[0,p]]};c.lefttop=c.bottomright;c.righttop=c.bottomleft;c.leftbottom=c.topright;c.rightbottom=c.topleft;return c[b]}function P(b){function c(j){var f=o.tip,g=["left","right"],l={left:0,right:0,
top:0,bottom:0},m=0;if(p.corner===FALSE||!f)return FALSE;j=j||h.corner;f.css({top:"",bottom:"",left:"",right:"",margin:""});g[j.precedance==="y"?"push":"unshift"]("top","bottom");if(a.browser.msie)l={top:j.precedance==="y"?0:0,left:0,bottom:j.precedance==="y"?0:0,right:0};switch(j[j.precedance==="y"?"x":"y"]){case "center":f.css(g[0],"50%").css("margin-"+g[0],-(u[j.precedance==="y"?"width":"height"]/2));break;case g[0]:f.css(g[0],l[g[0]]+i);break;case g[1]:f.css(g[1],l[g[1]]+i);break}m=u[j.precedance===
"x"?"width":"height"];if(d)m-=parseInt(q.css("border-"+j[j.precedance]+"-width"),10);j[j.precedance]===g[2]?f.css(g[2],-l[g[2]]-m):f.css(g[3],l[g[3]]-m)}function k(j,f,g){if(o.tip){j=a.extend({},h.corner);f=h.mimic.adjust?a.extend({},h.mimic):null;var l=j.precedance==="y"?["y","top","left","height"]:["x","left","top","width"],m=g.adjusted,t=parseInt(q.css("border-"+j[l[0]]+"-width"),10);a.each([j,f],function(){if(m.left)this.x=this.x==="center"?m.left>0?"left":"right":this.x==="left"?"right":"left";
if(m.top)this.y=this.y==="center"?m.top>0?"top":"bottom":this.y==="top"?"bottom":"top"});g[l[1]]+=(j[l[0]]===l[1]?1:-1)*(u[l[3]]-t);g[l[2]]-=i;if(j.string()!==x.corner.string()&&(x.top!==m.top||x.left!==m.left))h.update(j,f);x.left=m.left;x.top=m.top;x.corner=j}}var h=this,p=b.options.style.tip,o=b.elements,w=o.tooltip,q=o.wrapper,x={top:0,left:0,corner:{string:function(){}}},u={width:p.width,height:p.height},e={},d=p.border||0,i=p.adjust||0,n=p.method||FALSE;h.corner=NULL;h.mimic=NULL;h.checks={"^position.my|style.tip.(corner|mimic|method|border)":function(){d=
p.border;if(h.detectCorner()){h.create();h.detectColours();h.update()}else h.tip.remove();this.get("position.target")!=="mouse"&&this.reposition()},"^style.tip.(height|width)":function(){u={width:p.width,height:p.height};h.create();h.update();b.reposition()}};a.extend(h,{init:function(){if(n===TRUE)n=a("<canvas />")[0].getContext?"canvas":a.browser.msie&&(h.mimic&&/center/i.test(h.mimic.string())||u.height!==u.width)?"vml":"polygon";else if(n==="canvas")n=a.browser.msie?"vml":!a("<canvas />")[0].getContext?
"polygon":"canvas";else if(n==="polygon")n=a.browser.msie&&/center/i.test(h.mimic.string())?"vml":n;if(h.detectCorner()){h.create();h.detectColours();h.update();w.bind("tooltipmove.tip",k)}return h},detectCorner:function(){var j=p.corner,f=p.mimic||j,g=b.options.position.at,l=b.options.position.my;if(l.string)l=l.string();if(j===FALSE||l===FALSE&&g===FALSE)return FALSE;else{if(j===TRUE)h.corner=new a.fn.qtip.plugins.Corner(l);else if(!j.string)h.corner=new a.fn.qtip.plugins.Corner(j);if(f===TRUE)h.mimic=
...
q.css(d?"background-color":g);if(!e.border||/rgba?\(0, 0, 0(, 0)?\)|transparent/i.test(e.border))e.border=q.css(g)||e.fill;a("*",j).add(j).css("background-color","transparent").css("border",0)},create:function(){var j=u.width,f=u.height;o.tip&&o.tip.remove();o.tip=a('<div class="ui-tooltip-tip ui-widget-content"></div>').css(u).prependTo(w);switch(n){case "canvas":o.tip.append('<canvas height="'+f+'" width="'+j+'" />');break;case "vml":o.tip.html('<vml:shape coordorigin="0 0" coordsize="'+j+" "+f+
...
2)+"px solid "+A)});break}c(j);return h},destroy:function(){o.tip&&o.tip.remove();w.unbind("tooltipmove.tip")}})}function Q(b,c){var k=this,h=b.elements,p=h.tooltip;h.blanket=a("#qtip-blanket");a.extend(k,{init:function(){c=a.extend(TRUE,a.fn.qtip.plugins.modal.defaults,c);p.bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(o,w,q){o=o.type.replace("tooltip","");a.isFunction(c[o])?c[o].call(h.blanket,q,w):k[o](q)});h.blanket.length||k.create();c.blur===TRUE&&h.blanket.bind("click.qtipmodal"+
b.id,function(){b.hide.call(b)})},create:function(){h.blanket=a("<div />",{id:"qtip-blanket",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(B).bind("resize.qtipmodal",function(){h.blanket.css({height:Math.max(a(B).height(),a(document).height()),width:Math.max(a(B).width(),a(document).width())})}).trigger("resize")},show:function(o){h.blanket.fadeIn(o)},hide:function(o){h.blanket.fadeOut(o)},destroy:function(){var o=TRUE;a("*").each(function(){var w=a(this).data("qtip");
...
k.bgiframe.appendTo(h);h.bind(o,c.adjust)},adjust:function(){var w=b.calculate("dimensions"),q=b.plugins.tip,x=b.elements.tip,u;u=parseInt(h.css("border-left-width"),10);u={left:-u,top:-u};if(q&&x){q=q.corner.precedance==="x"?["width","left"]:["height","top"];u[q[1]]-=x[q[0]]()}k.bgiframe.css(u).css(w)},destroy:function(){c.iframe.remove();h.unbind(o)}});c.init()}a.fn.qtip=function(b,c,k){var h=String(b).toLowerCase(),p=FALSE,o=h==="disable"?[TRUE]:a.makeArray(arguments).splice(1),w=o[o.length-1],
q;if(!b&&a(this).data("qtip")||h==="api")return(q=a(this).eq(0).data("qtip"))?q.hash():D;else if("string"===typeof b){a(this).each(function(){var x=a(this).data("qtip");if(!x)return TRUE;if(/option|set/.test(h)&&c)if(k!==D)x.set(c,k);else p=x.get(c);else{if(!x.rendered&&(h==="show"||h==="toggle")){if(w.timeStamp)x.cache.event=w;x.render()}else if(h==="enable"){h="disable";o=[FALSE]}x[h]&&x[h].apply(x[h],o)}});return p!==FALSE?p:a(this)}else if("object"===typeof b){K(b);q=a.extend(TRUE,{},a.fn.qtip.defaults,
b);return a.fn.qtip.bind.call(this,q)}};a.fn.qtip.bind=function(b){return a(this).each(function(){function c(x){function u(){h.render(typeof x==="object"||p.show.ready);o.show.unbind(w.show);o.hide.unbind(w.hide)}if(h.cache.disabled)return FALSE;h.cache.event=a.extend({},x);if(p.show.delay>0){h.timers.show=setTimeout(u,p.show.delay);w.show!==w.hide&&o.hide.bind(w.hide,function(){clearTimeout(h.timers.show)})}else u()}var k,h,p,o,w,q;k=b.id===FALSE||b.id.length<1||a("#ui-tooltip-"+b.id).length?a.fn.qtip.nextid++:
b.id;q=".qtip-"+k+"-create";h=N.call(a(this),k,b);if(h===FALSE)return TRUE;p=h.options;a(this).attr("title")&&a(this).data("oldtitle",a(this).attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(h)});o={show:p.show.target,hide:p.hide.target};w={show:String(p.show.event).replace(" ",q+" ")+q,hide:String(p.hide.event).replace(" ",q+" ")+q};p.show.ready||p.prerender||p.show.event===FALSE?c():o.show.bind(w.show,c)})};a.each({attr:function(b){var c=
...
widget:FALSE},events:{render:a.noop,move:a.noop,show:a.noop,hide:a.noop,focus:a.noop,blur:a.noop}};var I=a();a.fn.qtip.plugins.ajax=function(b){var c=b.plugins.ajax,k=b.options.content;if(k.ajax&&k.ajax.url)if(c)return c;else{b.plugins.ajax=new O(b);return b.plugins.ajax}};a.fn.qtip.plugins.ajax.initialize="render";a.fn.qtip.plugins.ajax.sanitize=function(b){if(b.content!==D)if(b.content.ajax!==D){if(typeof b.content.ajax!=="object")b.content.ajax={url:b.content.ajax};if(b.content.text===FALSE)b.content.text=
"Loading...";b.content.ajax.once=Boolean(b.content.ajax.once);b.content.ajax.preload=Boolean(b.content.ajax.preload);if(b.content.ajax.preload){b=b.content.ajax.url;a("#qtip-preload").length||a('<div id="qtip-preload" class="ui-tooltip-accessible" />').appendTo(document.body);I.length||(I=a("<div />").appendTo("#qtip-preload").load(b+" img"))}}};a.fn.qtip.plugins.tip=function(b){var c=b.plugins.tip,k=b.options.style.tip;if(k&&k.corner)if(c)return c;else{b.plugins.tip=new P(b);b.plugins.tip.init();
...
4}};a.fn.qtip.plugins.imagemap=function(b,c){function k(e,d){for(var i=0,n=1,j=1,f=0,g=0,l=e.width,m=e.height;l>0&&m>0&&n>0&&j>0;){l=Math.floor(l/2);m=Math.floor(m/2);if(c.x==="left")n=l;else if(c.x==="right")n=e.width-l;else n+=Math.floor(l/2);if(c.y==="top")j=m;else if(c.y==="bottom")j=e.height-m;else j+=Math.floor(m/2);for(i=d.length;i--;){if(d.length<2)break;f=d[i][0]-e.offset.left;g=d[i][1]-e.offset.top;if(c.x==="left"&&f>=n||c.x==="right"&&f<=n||c.x==="center"&&(f<n||f>e.width-n)||c.y==="top"&&
g>=j||c.y==="bottom"&&g<=j||c.y==="center"&&(g<j||g>e.height-j))d.splice(i,1)}}return{left:d[0][0],top:d[0][1]}}var h=b.attr("shape").toLowerCase(),p=b.attr("coords").split(","),o=[],w=a('img[usemap="#'+b.parent("map").attr("name")+'"]').offset(),q={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},x=0,u=0;if(h==="poly")for(x=p.length;x--;){u=[parseInt(p[--x],10),parseInt(p[x+1],10)];if(u[0]>q.offset.right)q.offset.right=u[0];if(u[0]<q.offset.left)q.offset.left=u[0];if(u[1]>q.offset.bottom)q.offset.bottom=
u[1];if(u[1]<q.offset.top)q.offset.top=u[1];o.push(u)}else o=a.map(p,function(e){return parseInt(e,10)});switch(h){case "rect":q={width:Math.abs(o[2]-o[0]),height:Math.abs(o[3]-o[1]),offset:{left:o[0],top:o[1]}};break;case "circle":q={width:o[2]+2,height:o[2]+2,offset:{left:o[0],top:o[1]}};break;case "poly":a.extend(q,{width:Math.abs(q.offset.right-q.offset.left),height:Math.abs(q.offset.bottom-q.offset.top)});q.offset=c.string()==="centercenter"?{left:q.offset.left+q.width/2,top:q.offset.top+q.height/
2}:k(q,o.slice());q.width=q.height=0;break}q.offset.left+=w.left;q.offset.top+=w.top;return q};a.fn.qtip.plugins.modal=function(b){var c=b.plugins.modal,k=b.options.show.modal;if(c)return c;else if(typeof k==="object"){b.plugins.modal=new Q(b,k);return b.plugins.modal}};a.fn.qtip.plugins.modal.initialize="render";a.fn.qtip.plugins.modal.sanitize=function(b){if(b.show&&b.show.modal!==D)if(typeof b.show.modal!=="object")b.show.modal={}};a.fn.qtip.plugins.modal.defaults={effects:{show:TRUE,hide:TRUE},
blur:TRUE};a.fn.qtip.plugins.bgiframe=function(b){if(!(a.browser.msie&&/^6\.[0-9]/.test(a.browser.version)&&a("select, object").length))return FALSE;var c=b.plugins.bgiframe;if(c)return c;else{b.plugins.bgiframe=new R(b);return b.plugins.bgiframe}};a.fn.qtip.plugins.bgiframe.initialize="render"})(jQuery,window);
