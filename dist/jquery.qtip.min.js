...
o?0.7:0,function(){o||a(this).hide()})},show:function(){k.toggle(true)},hide:function(){k.toggle(false)},destroy:function(){var o=TRUE;a("*").each(function(){var v=a(this).data("qtip");if(v&&v.id!==b.id&&v.options.show.modal)return o=FALSE});if(o){h.overlay.remove();a(B).unbind("scroll.qtipmodal resize.qtipmodal")}else h.overlay.unbind("click.qtipmodal"+b.id);q.unbind("tooltipshow.qtipmodal tooltiphide.qtipmodal")}});k.init()}function R(b){var c=this,k=b.elements,h=k.tooltip,q=".bgiframe-"+b.id,o=
"tooltipmove"+q+" tooltipshow"+q;a.extend(c,{init:function(){k.bgiframe=a('<iframe class="ui-tooltip-bgiframe" frameborder="0" tabindex="-1" src="javascript:\'\';"  style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=0);"></iframe>');k.bgiframe.appendTo(h);h.bind(o,c.adjust)},adjust:function(){var v=b.calculate("dimensions"),p=b.plugins.tip,w=b.elements.tip,u;u=parseInt(h.css("border-left-width"),10);u={left:-u,top:-u};if(p&&w){p=p.corner.precedance==="x"?["width","left"]:["height",
"top"];u[p[1]]-=w[p[0]]()}k.bgiframe.css(u).css(v)},destroy:function(){c.iframe.remove();h.unbind(o)}});c.init()}a.fn.qtip=function(b,c,k){var h=String(b).toLowerCase(),q=FALSE,o=h==="disable"?[TRUE]:a.makeArray(arguments).splice(1),v=o[o.length-1],p;if(!b&&a(this).data("qtip")||h==="api")return(p=a(this).eq(0).data("qtip"))?p.hash():D;else if("string"===typeof b){a(this).each(function(){var w=a(this).data("qtip");if(!w)return TRUE;if(/option|set/.test(h)&&c)if(k!==D)w.set(c,k);else q=w.get(c);else{if(!w.rendered&&
(h==="show"||h==="toggle")){if(v.timeStamp)w.cache.event=v;w.render()}else if(h==="enable"){h="disable";o=[FALSE]}w[h]&&w[h].apply(w[h],o)}});return q!==FALSE?q:a(this)}else if("object"===typeof b){K(b);p=a.extend(TRUE,{},a.fn.qtip.defaults,b);return a.fn.qtip.bind.call(this,p)}};a.fn.qtip.bind=function(b){return a(this).each(function(){function c(w){function u(){h.render(typeof w==="object"||q.show.ready);o.show.unbind(v.show);o.hide.unbind(v.hide)}if(h.cache.disabled)return FALSE;h.cache.event=
a.extend({},w);if(q.show.delay>0){h.timers.show=setTimeout(u,q.show.delay);v.show!==v.hide&&o.hide.bind(v.hide,function(){clearTimeout(h.timers.show)})}else u()}var k,h,q,o,v,p;k=b.id===FALSE||b.id.length<1||a("#ui-tooltip-"+b.id).length?a.fn.qtip.nextid++:b.id;p=".qtip-"+k+"-create";h=N.call(a(this),k,b);if(h===FALSE)return TRUE;q=h.options;a(this).attr("title")&&a(this).data("oldtitle",a(this).attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&
this(h)});o={show:q.show.target,hide:q.hide.target};v={show:String(q.show.event).replace(" ",p+" ")+p,hide:String(q.hide.event).replace(" ",p+" ")+p};q.show.ready||q.prerender||q.show.event===FALSE?c():o.show.bind(v.show,c)})};a.each({attr:function(b){var c=a(this).data("qtip");return arguments.length===1&&b==="title"&&c&&c.rendered===TRUE?a(this).data("oldtitle"):NULL},remove:a.ui?NULL:function(b,c){this.each(function(){if(!c)if(!b||a.filter(b,[this]).length)a("*",this).add(this).each(function(){a(this).triggerHandler("remove")})})}},
