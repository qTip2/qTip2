...
function(){this.sanitize&&this.sanitize(b)})}function L(b,c,k){function g(e){var i,n;e=e.split(".");i=c[e[0]];for(n=1;n<e.length;n+=1)if(typeof i[e[n]]==="object"&&!i[e[n]].jquery)i=i[e[n]];else break;return[i,e[n]]}function p(e){var i=d.elements.tooltip,n=!i.is(":visible")?TRUE:FALSE,j=FALSE;if(!d.rendered)return FALSE;n&&i.addClass("ui-tooltip-accessible");switch(e){case "dimensions":j={height:i.outerHeight(),width:i.outerWidth()};break;case "position":j=i.offset();break}n&&i.removeClass("ui-tooltip-accessible");
return j}function o(){var e=d.elements;if(e.title){e.titlebar.remove();e.titlebar=e.title=e.button=NULL;e.tooltip.removeAttr("aria-labelledby")}}function v(){var e=d.elements,i=c.content.title.button;e.titlebar&&o();e.titlebar=a("<div />",{"class":"ui-tooltip-titlebar "+(c.style.widget?"ui-widget-header":"")}).append(e.title=a("<div />",{id:"ui-tooltip-"+k+"-title","class":"ui-tooltip-title",html:c.content.title.text})).prependTo(e.wrapper);if(i){e.button=i.jquery?i:"string"===typeof i?a("<a />",
{html:i}):a("<a />",{"class":"ui-state-default"}).append(a("<span />",{"class":"ui-icon ui-icon-close"}));e.button.prependTo(e.titlebar).attr("role","button").addClass("ui-tooltip-"+(i===TRUE?"close":"button")).hover(function(n){a(this).toggleClass("ui-state-hover",n.type==="mouseenter")}).click(function(){e.tooltip.hasClass("ui-state-disabled")||d.hide();return FALSE}).bind("mousedown keydown mouseup keyup mouseout",function(n){a(this).toggleClass("ui-state-active ui-state-focus",/down$/i.test(n.type))})}}
function q(e){if(!d.rendered||!e)return FALSE;e.jquery&&e.length>0?d.elements.content.append(e.css({display:"block"})):d.elements.content.html(e);e=void 0;var i=d.elements.tooltip,n,j;if(d.rendered&&a.browser.msie){i.css({width:"auto",maxWidth:"none"});e=p("dimensions").width;i.css({maxWidth:""});n=parseInt(i.css("max-width"),10)||0;j=parseInt(i.css("min-width"),10)||0;e=Math.min(Math.max(e,j),n);i.width(e)}d.reposition(d.cache.event);if(d.rendered<0){if(c.show.ready||d.rendered===-2)d.show(d.cache.event);
d.rendered=TRUE}return d}function w(e,i,n,j){function f(y){if(r.tooltip.hasClass("ui-state-disabled"))return FALSE;r.show.trigger("qtip-"+k+"-inactive");clearTimeout(d.timers.show);clearTimeout(d.timers.hide);d.timers.show=setTimeout(function(){d.show(y)},c.show.delay)}function h(y){if(r.tooltip.hasClass("ui-state-disabled"))return FALSE;var z=a(y.relatedTarget).parents(".qtip.ui-tooltip")[0]==r.tooltip[0];clearTimeout(d.timers.show);clearTimeout(d.timers.hide);if(c.position.target==="mouse"&&z||
...
x={show:String(c.show.event).split(" "),hide:String(c.hide.event).split(" ")},s=a.browser.msie&&/^6\.[0-9]/.test(a.browser.version);if(n&&c.hide.fixed){r.hide=r.hide.add(r.tooltip);r.tooltip.bind("mouseover"+t,function(){r.tooltip.hasClass("ui-state-disabled")||clearTimeout(d.timers.hide)})}if(i){if("number"===typeof c.hide.inactive){r.show.bind("qtip-"+k+"-inactive",l);a.each(a.fn.qtip.inactiveEvents,function(y,z){r.hide.add(d.elements.tooltip).bind(z+t+"-inactive",l)})}a.each(x.hide,function(y,
z){var A=a.inArray(z,x.show);if(A>-1&&a(r.hide).add(r.show).length===a(r.hide).length||z==="unfocus"){r.show.bind(z+t,function(C){r.tooltip.is(":visible")?h(C):f(C)});delete x.show[A]}else r.hide.bind(z+t,h)})}if(e){a.each(x.show,function(y,z){r.show.bind(z+t,f)});r.tooltip.bind("mouseover"+t,function(){d.focus()})}if(j){if(c.position.adjust.resize||c.position.adjust.screen)a(B).bind("resize"+t,m);if(c.position.adjust.screen||s&&r.tooltip.css("position")==="fixed")a(document).bind("scroll"+t,m);/unfocus/i.test(c.hide.event)&&
a(document).bind("mousedown"+t,function(y){var z=d.elements.tooltip;a(y.target).parents(".qtip.ui-tooltip").length===0&&a(y.target).add(b).length>1&&z.is(":visible")&&!z.hasClass("ui-state-disabled")&&d.hide()});c.position.target==="mouse"&&a(document).bind("mousemove"+t,function(y){c.position.adjust.mouse&&!r.tooltip.hasClass("ui-state-disabled")&&r.tooltip.is(":visible")&&d.reposition(y)})}}function u(e,i,n,j){j=parseInt(j,10)!==0;var f=".qtip-"+k,h={show:e?c.show.target:a("<div/>"),hide:i?c.hide.target:
a("<div/>"),tooltip:n?d.elements.tooltip:a("<div/>")};i={show:String(c.show.event).split(" "),hide:String(c.hide.event).split(" ")};if(d.rendered){a.each(i.show,function(l,m){h.show.unbind(m+f)});h.show.unbind("mousemove"+f).unbind("mouseout"+f).unbind("qtip-"+k+"-inactive");a.each(i.hide,function(l,m){h.hide.add(h.tooltip).unbind(m+f)});a.each(a.fn.qtip.inactiveEvents,function(l,m){h.hide.add(n?d.elements.content:NULL).unbind(m+f+"-inactive")});h.hide.unbind("mouseout"+f);h.tooltip.unbind("mouseover"+
f);if(j){a(B).unbind("resize"+f);a(document).unbind("mousedown"+f+" mousemove"+f)}}else e&&h.show.unbind(i.show+f+"-create")}var d=this;d.id=k;d.rendered=FALSE;d.elements={target:b};d.cache={event:{},target:NULL,disabled:FALSE};d.timers={};d.options=c;d.plugins={};a.extend(d,{render:function(e){var i=d.elements;if(d.rendered)return FALSE;d.rendered=e?-2:-1;i.tooltip=a("<div/>").attr({id:"ui-tooltip-"+k,role:"tooltip"}).addClass("qtip ui-tooltip ui-helper-reset "+c.style.classes).toggleClass("ui-widget",
c.style.widget).toggleClass("ui-state-disabled",d.cache.disabled).css("z-index",a.fn.qtip.zindex+a("div.qtip.ui-tooltip").length).data("qtip",d).appendTo(c.position.container);i.wrapper=a("<div />").addClass("ui-tooltip-wrapper").appendTo(i.tooltip);i.content=a("<div />").addClass("ui-tooltip-content").attr("id","ui-tooltip-"+k+"-content").addClass("ui-tooltip-content").toggleClass("ui-widget-content",c.style.widget).appendTo(i.wrapper);c.content.title.text&&v();a.each(a.fn.qtip.plugins,function(){this.initialize===
"render"&&this(d)});w(1,1,1,1);a.each(c.events,function(n,j){i.tooltip.bind("tooltip"+n,j)});b.bind("remove.qtip",function(){d.destroy()});q(c.content.text);i.tooltip.trigger("tooltiprender",[d.hash()]);return d},get:function(e){switch(e.toLowerCase()){case "offset":e=p("position");break;case "dimensions":e=p("dimensions");break;default:e=g(e.toLowerCase());e=e[0].precedance?e[0].string():e[0].jquery?e[0]:e[0][e[1]];break}return e},set:function(e,i){var n=g(e.toLowerCase()),j,f,h,l={builtin:{"^content.text":function(){q(i)},
"^content.title.text":function(){if(d.rendered)if(!d.elements.title&&i){v();d.reposition()}else i?d.elements.title.html(i):o()},"^position.container$":function(){d.rendered&&d.elements.tooltip.appendTo(i)},"^position.(my|at)$":function(){var m=/my$/i.test(e)?"my":"at";if("string"===typeof i)c.position[m]=new a.fn.qtip.plugins.Corner(i)},"^position.(my|at|adjust|target)":function(){d.rendered&&d.reposition()},"^(show|hide).(event|target|fixed)":function(m,t,r,x){var s=e.search(/fixed/i)>-1?[0,[0,1,
1,1]]:e.search(/hide/i)<0?["show",[1,0,0,0]]:["hide",[0,1,0,0]];if(s[0])m[t]=x;u.apply(d,s[1]);if(s[0])m[t]=r;w.apply(d,s[1])}}};a.each(d.plugins,function(m){if("object"===typeof this.checks)l[m]=this.checks});j=n[0][n[1]];n[0][n[1]]=i;K(c);for(f in l)for(h in l[f])RegExp(h,"i").test(e)&&l[f][h].call(d,n[0],n[1],i,j);return d},toggle:function(e,i){function n(){var t=e?"attr":"removeAttr";a(this).css({opacity:"",height:""});d.elements.title&&b[t]("aria-labelledby","ui-tooltip-"+k+"-title");b[t]("aria-describedby",
"ui-tooltip-"+k+"-content");if(e){if(a.browser.msie&&a(this).get(0).style){m=a(this).get(0).style;m.removeAttribute("filter");m.removeAttribute("opacity")}}else a(this).hide()}if(!d.rendered)return FALSE;var j=e?"show":"hide",f=d.elements.tooltip,h=c[j],l=f.is(":visible"),m;if((typeof e).search("boolean|number"))e=!f.is(":visible");if(l&&e||!l&&!e||f.is(":animated"))return d;if(i){if(d.cache.event&&/over|enter/.test(i.type)&&/out|leave/.test(d.cache.event.type)&&a(i.target).add(c.show.target).length<
2&&a(i.relatedTarget).parents(".qtip.ui-tooltip").length>0)return d;d.cache.event=a.extend({},i)}l=a.Event("tooltip"+j);l.originalEvent=d.cache.event;f.trigger(l,[d.hash(),90]);if(l.isDefaultPrevented())return d;if(e){if(d.rendered===TRUE){d.focus();d.reposition(i)}h.solo&&a(":not(.qtip.ui-tooltip)").qtip("hide")}else{clearTimeout(d.timers.show);f.css({opacity:""})}f.attr("aria-hidden",Boolean(!e));f.stop(TRUE,FALSE);if(a.isFunction(h.effect)){h.effect.call(f);f.queue(function(){n();a(this).dequeue()})}else if(h.effect===
FALSE){f[j]();n()}else f.fadeTo(90,e?100:0,n);e&&h.target.trigger("qtip-"+k+"-inactive");return d},show:function(e){d.toggle(TRUE,e)},hide:function(e){d.toggle(FALSE,e)},focus:function(e){if(!d.rendered)return FALSE;var i=d.elements.tooltip,n=parseInt(i.css("z-index"),10),j=a.fn.qtip.zindex+a(".qtip.ui-tooltip").length,f=a.extend({},e);if(!i.hasClass("ui-tooltip-focus")&&n!==j){a(".qtip.ui-tooltip").each(function(){var h=a(this).qtip(),l=a.Event("tooltipblur"),m,t;if(!h||!h.rendered)return TRUE;m=
h.elements.tooltip;t=parseInt(m.css("z-index"),10);isNaN(t)||m.css({zIndex:t-1});m.removeClass("ui-tooltip-focus");l.originalEvent=f;m.trigger(l,[h,j])});e=a.Event("tooltipfocus");e.originalEvent=f;i.trigger(e,[d.hash(),j]);e.isDefaultPrevented()||i.css({zIndex:j}).addClass("ui-tooltip-focus")}return d},reposition:function(e){if(d.rendered===FALSE)return FALSE;var i=c.position.target,n=d.elements.tooltip,j=c.position,f=j.my,h=j.at,l=d.elements.tooltip.width(),m=d.elements.tooltip.height(),t=j.container,
r=0,x=0,s={left:0,top:0},y=a.Event("tooltipmove"),z={left:function(A){var C=i==="mouse"?e.pageX:i.offset().left,G=a(B).scrollLeft(),F=a(B).width(),D=h.x==="left"?r:h.x==="right"?-r:r/2,H=D+(f.x==="left"?-l:f.x==="right"?l:l/2)+-2*j.adjust.x;F=A+l-F-G;if(G-A>0&&!(A>=C&&A<C+r))s.left+=H-D;else if(F>0&&A+l>C)s.left+=f.x==="center"?-H+D:H-D;return s.left-A},top:function(A){var C=a(B).scrollTop(),G=a(B).height(),F=h.y==="top"?x:h.y==="bottom"?-x:0,D=F+(f.y==="top"?-m:f.y==="bottom"?m:-m/2)+-2*j.adjust.y;
G=A+m-G-C;if(C-A>0)s.top+=f.y==="center"?-D+F:D;else if(G>0)s.top+=D-F;return s.top-A}};if(i==="mouse"){h={x:"left",y:"top"};if(!e)e=d.cache.event;s={top:e.pageY,left:e.pageX}}else{if(i==="event")i=e&&e.target&&e.type!=="scroll"&&e.type!=="resize"?d.cache.target=a(e.target):d.cache.target;if(i[0]===document||i[0]===B){r=i.width();x=i.height();s={top:n.css("position")==="fixed"?0:i.scrollTop(),left:i.scrollLeft()}}else if(i.is("area")&&a.fn.qtip.plugins.imagemap){s=a.fn.qtip.plugins.imagemap(i,h);
r=s.width;x=s.height;s=s.offset}else{r=i.outerWidth();x=i.outerHeight();if(!j.adjust.offset||t[0]===document.body)s=i.offset();else{s=i.position();s.top+=t.scrollTop()-t.offset().top;s.left+=t.scrollLeft()-t.offset().left}}s.left+=h.x==="right"?r:h.x==="center"?r/2:0;s.top+=h.y==="bottom"?x:h.y==="center"?x/2:0}s.left+=j.adjust.x+(f.x==="right"?-l:f.x==="center"?-l/2:0);s.top+=j.adjust.y+(f.y==="bottom"?-m:f.y==="center"?-m/2:0);s.adjusted=j.adjust.screen?{left:z.left(s.left),top:z.top(s.top)}:{left:0,
...
a(this).dequeue()})}else n.css(s);return d},disable:function(e){var i=d.elements.tooltip;if(d.rendered)i.toggleClass("ui-state-disabled",e);else d.cache.disabled=!!e;return d},destroy:function(){var e=d.elements,i=e.target.data("oldtitle");d.rendered&&a.each(d.plugins,function(){this.initialize==="render"&&this.destroy()});u(1,1,1,1);b.removeData("qtip");d.rendered&&e.tooltip.remove();i&&b.attr("title",i);return b},hash:function(){var e=a.extend({},d);delete e.cache;delete e.timers;delete e.options;
...
Math.floor(k/2);c={bottomright:[[0,0],[c,k],[c,0]],bottomleft:[[0,0],[c,0],[0,k]],topright:[[0,k],[c,0],[c,k]],topleft:[[0,0],[0,k],[c,k]],topcenter:[[0,k],[g,0],[c,k]],bottomcenter:[[0,0],[c,0],[g,k]],rightcenter:[[0,0],[c,p],[0,k]],leftcenter:[[c,0],[c,k],[0,p]]};c.lefttop=c.bottomright;c.righttop=c.bottomleft;c.leftbottom=c.topright;c.rightbottom=c.topleft;return c[b]}function O(b){function c(j){var f=o.tip,h=["left","right"],l={left:0,right:0,top:0,bottom:0},m=0;if(p.corner===FALSE||!f)return FALSE;
j=j||g.corner;f.css({top:"",bottom:"",left:"",right:"",margin:""});h[j.precedance==="y"?"push":"unshift"]("top","bottom");if(a.browser.msie)l={top:j.precedance==="y"?0:0,left:0,bottom:j.precedance==="y"?0:0,right:0};switch(j[j.precedance==="y"?"x":"y"]){case "center":f.css(h[0],"50%").css("margin-"+h[0],-(u[j.precedance==="y"?"width":"height"]/2));break;case h[0]:f.css(h[0],l[h[0]]+i);break;case h[1]:f.css(h[1],l[h[1]]+i);break}m=u[j.precedance==="x"?"width":"height"];if(e)m-=parseInt(q.css("border-"+
j[j.precedance]+"-width"),10);j[j.precedance]===h[2]?f.css(h[2],-l[h[2]]-m):f.css(h[3],l[h[3]]-m)}function k(j,f,h){if(o.tip){j=a.extend({},g.corner);f=g.mimic.adjust?a.extend({},g.mimic):null;var l=j.precedance==="y"?["y","top","left","height"]:["x","left","top","width"],m=h.adjusted,t=parseInt(q.css("border-"+j[l[0]]+"-width"),10);a.each([j,f],function(){if(m.left)this.x=this.x==="center"?m.left>0?"left":"right":this.x==="left"?"right":"left";if(m.top)this.y=this.y==="center"?m.top>0?"top":"bottom":
this.y==="top"?"bottom":"top"});h[l[1]]+=(j[l[0]]===l[1]?1:-1)*(u[l[3]]-t);h[l[2]]-=i;if(j.string()!==w.corner.string()&&(w.top!==m.top||w.left!==m.left))g.update(j,f);w.left=m.left;w.top=m.top;w.corner=j}}var g=this,p=b.options.style.tip,o=b.elements,v=o.tooltip,q=o.wrapper,w={top:0,left:0,corner:{string:function(){}}},u={width:p.width,height:p.height},d={},e=p.border||0,i=p.adjust||0,n=p.method||FALSE;g.corner=NULL;g.mimic=NULL;g.checks={"^position.my|style.tip.(corner|mimic|method|border)":function(){e=
p.border;if(g.detectCorner()){g.create();g.detectColours();g.update()}else g.tip.remove();this.get("position.target")!=="mouse"&&this.reposition()},"^style.tip.(height|width)":function(){u={width:p.width,height:p.height};g.create();g.update();b.reposition()}};a.extend(g,{init:function(){if(n===TRUE)n=a("<canvas />")[0].getContext?"canvas":a.browser.msie&&(/center/i.test(g.mimic.string())||u.height!==u.width)?"vml":"polygon";else if(n==="canvas")n=a.browser.msie?"vml":!a("<canvas />")[0].getContext?
"polygon":"canvas";else if(n==="polygon")n=a.browser.msie&&/center/i.test(g.mimic.string())?"vml":n;if(g.detectCorner()){g.create();g.detectColours();g.update();v.bind("tooltipmove.tip",k)}return g},detectCorner:function(){var j=p.corner,f=p.mimic||j,h=b.options.position.at,l=b.options.position.my;if(l.string)l=l.string();if(j===FALSE||l===FALSE&&h===FALSE)return FALSE;else{if(j===TRUE)g.corner=new a.fn.qtip.plugins.Corner(l);else if(!j.string)g.corner=new a.fn.qtip.plugins.Corner(j);if(f===TRUE)g.mimic=
new a.fn.qtip.plugins.Corner(l);else if(!f.string){g.mimic=new a.fn.qtip.plugins.Corner(f);g.mimic.precedance=g.corner.precedance}}return g.corner.string()!=="centercenter"},detectColours:function(){var j=o.tip,f=g.mimic[g.mimic.precedance],h="border-"+f+"-color";d.fill=j.css("background-color","").css("border","").css("background-color")||"transparent";d.border=j.get(0).style?j.get(0).style["border"+f.charAt(0)+f.substr(1)+"Color"]:j.css(h)||"transparent";if(/rgba?\(0, 0, 0(, 0)?\)|transparent/i.test(d.fill))d.fill=
q.css(e?"background-color":h);if(!d.border||/rgba?\(0, 0, 0(, 0)?\)|transparent/i.test(d.border))d.border=q.css(h)||d.fill;a("*",j).add(j).css("background-color","transparent").css("border",0)},create:function(){var j=u.width,f=u.height;o.tip&&o.tip.remove();o.tip=a('<div class="ui-tooltip-tip ui-widget-content"></div>').css(u).prependTo(v);switch(n){case "canvas":o.tip.append('<canvas height="'+f+'" width="'+j+'" />');break;case "vml":o.tip.html('<vml:shape coordorigin="0 0" coordsize="'+j+" "+f+
'" stroked="'+!!e+'"  style="behavior:url(#default#VML); display:inline-block; antialias:TRUE; position: absolute;  top:0; left:0; width:'+j+"px; height:"+f+"px; vertical-align:"+g.corner.y+';"><vml:stroke weight="'+(e-2)+'px" joinstyle="miter" miterlimit="10"  style="behavior:url(#default#VML); display:inline-block;" /></vml:shape>');break;case "polygon":o.tip.append('<div class="ui-tooltip-tip-inner" />').append(e?'<div class="ui-tooltip-tip-border" />':"");break}return g},update:function(j,f){var h=
o.tip,l=u.width,m=u.height,t=e>0?0:1,r=Math.ceil(e/2+0.5),x;f||(f=j?j:g.mimic);if(!j)j=g.corner;if(f.x==="false")f.x=j.x;if(f.y==="false")f.y=j.y;h=h.children();switch(n){case "canvas":h=h.get(0).getContext("2d");h.clearRect(0,0,3E3,3E3);h.restore();if(e){x=J(f.string(),l*2,m*2);h.strokeStyle=d.border;h.lineWidth=e+1;h.lineJoin="miter";h.miterLimit=100;h.save();h.translate(f.x==="left"?0:f.x==="right"?-l:-l/2,f.y==="top"?0:f.y==="bottom"?-m:-m/2)}else x=J(f.string(),l,m);h.fillStyle=d.fill;for(h.miterLimit=
0;t<2;t++){h.globalCompositeOperation=t&&e?"destination-in":"source-over";h.beginPath();h.moveTo(x[0][0],x[0][1]);h.lineTo(x[1][0],x[1][1]);h.lineTo(x[2][0],x[2][1]);h.closePath();h.fill();t||h.stroke()}break;case "vml":x=J(f.string(),l,m);t="m"+x[0][0]+","+x[0][1]+" l"+x[1][0]+","+x[1][1]+" "+x[2][0]+","+x[2][1]+" xe";h.attr({path:t,fillcolor:d.fill});if(e){h.children().attr("color",d.border);if(f.precedance==="y"){h.css("top",(f.y==="top"?1:-1)*(e-2));h.css("left",f.x==="left"?1:-2)}else{h.css("left",
(f.x==="left"?1:-1)*(e-2));h.css("top",f.y==="top"?1:-2)}}break;case "polygon":if(f.precedance==="y"){t=l>m?1.5:l<m?5:2.2;r=[f.x==="left"?r:f.x==="right"?-r:0,Math.floor(t*r*(f.y==="bottom"?-1:1)*(f.x==="center"?0.8:1))]}else{t=l<m?1.5:l>m?5:2.2;r=[Math.floor(t*r*(f.x==="right"?-1:1)*(f.y==="center"?0.9:1)),f.y==="top"?r:f.y==="bottom"?-r:0]}h.removeAttr("style").each(function(s){var y={x:f.precedance==="x"?f.x==="left"?"right":"left":f.x,y:f.precedance==="y"?f.y==="top"?"bottom":"top":f.y},z=f.x===
