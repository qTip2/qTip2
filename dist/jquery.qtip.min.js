...
J=i.css("position")==="fixed",u=j.viewport.jquery?j.viewport:FALSE,r={left:0,top:0},I={left:function(D){var E=u.scrollLeft,H=s.x==="left"?p:s.x==="right"?-p:-p/2,K=E-D;E=D+A-u.width-E;H=(g.x==="left"?A:g.x==="right"?-A:-A/2)-(g.precedance==="x"||g.x===g.y?H:0);if(K>0&&(g.x!=="left"||E>0))r.left-=H;else if(E>0&&(g.x!=="right"||K>0))r.left-=(g.x==="center"?-1:1)*H+2*x.x;if(r.left<0&&-r.left>E)r.left=D;return r.left-D},top:function(D){var E=u.scrollTop,H=s.y==="top"?t:s.y==="bottom"?-t:-t/2,K=E-D;E=
D+y-u.height-E;H=(g.y==="top"?y:g.y==="bottom"?-y:-y/2)-(g.precedance==="y"||g.x===g.y?H:0);if(K>0&&(g.y!=="top"||E>0))r.top-=H;else if(E>0&&(g.y!=="bottom"||K>0))r.top-=(g.y==="center"?-1:1)*H+2*x.y;if(r.top<0&&-r.top>E)r.top=D;return r.top-D}};u=!u?FALSE:{elem:u,height:u[(u[0]===G?"h":"outerH")+"eight"](),width:u[(u[0]===G?"w":"outerW")+"idth"](),scrollLeft:u.scrollLeft(),scrollTop:u.scrollTop()};if(e==="mouse"){s={x:"left",y:"top"};d=d&&(d.type==="resize"||d.type==="scroll")?a.cache.event:x.mouse||
!d||!d.pageX?c.extend({},c.fn.qtip.mouse):d;r={top:d.pageY,left:d.pageX}}else{if(e==="event")e=d&&d.target&&d.type!=="scroll"&&d.type!=="resize"?a.cache.target=c(d.target):a.cache.target;e=c(e).eq(0);if(e.length===0)return a;else if(e[0]===document||e[0]===G){p=e.width();t=e.height();if(e[0]===G)r={top:J?0:u.scrollTop,left:J?0:u.scrollLeft}}else if(e.is("area")&&c.fn.qtip.plugins.imagemap){r=c.fn.qtip.plugins.imagemap(e,s);p=r.width;t=r.height;r=r.offset}else{p=e.outerWidth();t=e.outerHeight();r=
m(e[0],j.container[0])}r.left+=s.x==="right"?p:s.x==="center"?p/2:0;r.top+=s.y==="bottom"?t:s.y==="center"?t/2:0}r.left+=x.x+(g.x==="right"?-A:g.x==="center"?-A/2:0);r.top+=x.y+(g.y==="bottom"?-y:g.y==="center"?-y/2:0);r.adjusted=u&&e[0]!==G&&e[0]!==document.body?{left:I.left(r.left),top:I.top(r.top)}:{left:0,top:0};i.attr("class",function(){return c(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(l+"-pos-"+g.abbreviation());F.originalEvent=c.extend({},d);i.trigger(F,[a,r,u.elem]);
if(F.isDefaultPrevented())return a;delete r.adjusted;if(i.is(":visible")&&c.isFunction(j.effect)){j.effect.call(i,a,r);i.queue(function(){var D=c(this);D.css({opacity:"",height:""});c.browser.msie&&this.style&&this.style.removeAttribute("filter");D.dequeue()})}else isNaN(r.left,r.top)||i.css(r);return a},redraw:function(){if(!a.rendered||!(c.browser.msie&&parseInt(c.browser.version.charAt(0),10)<9))return FALSE;var d=a.elements.tooltip;d.attr("style");var e;d.css({width:"auto",height:"auto"});e=k("dimensions",
1);c.each(["width","height"],function(i,j){var g=parseInt(d.css("max-"+j),10)||0,s=parseInt(d.css("min-"+j),10)||0;e[j]=g+s?Math.min(Math.max(e[j],s),g):e[j]});d.css(e)},disable:function(d){var e=a.elements.tooltip;if("boolean"!==typeof d)d=!(e.hasClass("ui-state-disabled")||a.cache.disabled);if(a.rendered)e.toggleClass("ui-state-disabled",d);else a.cache.disabled=!!d;return a},destroy:function(){var d=a.elements,e=d.target.data("oldtitle");if(a.rendered){d.tooltip.remove();c.each(a.plugins,function(){this.initialize===
"render"&&this.destroy()})}clearTimeout(a.timers.show);clearTimeout(a.timers.hide);C(1,1,1,1);f.removeData("qtip");e&&f.attr("title",e);f.removeAttr("aria-describedby");return f}})}function P(f,b){var o,h,m=c(this),k=c(document.body),v=this===document?k:m;h=m.metadata?m.metadata(b.metadata):NULL;var q=c.extend(TRUE,{},c.fn.qtip.defaults,b,M((h&&b.metadata.type==="html5"?h[b.metadata.name]:NULL)||h));m.removeData("metadata");h=q.position;if("boolean"===typeof q.content.text)if(q.content.attr!==FALSE&&
m.attr(q.content.attr))q.content.text=m.attr(q.content.attr);else return FALSE;if(h.container===FALSE)h.container=k;if(h.target===FALSE)h.target=v;if(q.show.target===FALSE)q.show.target=v;if(q.hide.target===FALSE)q.hide.target=v;h.at=new c.fn.qtip.plugins.Corner(h.at);h.my=new c.fn.qtip.plugins.Corner(h.my);if(m.data("qtip"))if(q.overwrite)m.qtip("destroy");else if(q.overwrite===FALSE)return FALSE;m.attr("title")&&m.data("oldtitle",m.attr("title")).removeAttr("title");o=new O(m,q,f);m.data("qtip",
...
"mouse"&&f.reposition();else h.destroy()},"^style.tip.(height|width)":function(){n={width:m.width,height:m.height};h.create();h.update();f.reposition()},"^style.(classes|widget)$":function(){h.detectColours();h.update()}};c.extend(h,{init:function(){var a=h.detectCorner();if(a){if(C!=="polygon")C=c("<canvas />")[0].getContext?"canvas":c.browser.msie?"vml":"polygon";h.create();h.detectColours();B=m.border===TRUE?o(h.corner):m.border;h.update();v.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",b)}return a},
detectCorner:function(){var a=m.corner,l=f.options.position,w=l.at;l=l.my.string?l.my.string():l.my;if(a===FALSE||l===FALSE&&w===FALSE)return FALSE;else if(a===TRUE)h.corner=new c.fn.qtip.plugins.Corner(l);else if(!a.string){h.corner=new c.fn.qtip.plugins.Corner(a);h.corner.fixed=TRUE}return h.corner.string()!=="centercenter"},detectColours:function(){var a=k.tip.css({backgroundColor:"",border:""}),l=h.corner,w=l[l.precedance],d="border-"+w+"-color";w="border"+w.charAt(0)+w.substr(1)+"Color";var e=
/rgba?\(0, 0, 0(, 0)?\)|transparent/i;l=k.titlebar&&l.y==="top"?k.titlebar:k.content;z.fill=a.css("background-color")||"transparent";z.border=a[0].style[w];if(!z.fill||e.test(z.fill)){z.fill=l.css("background-color");if(e.test(z.fill))z.fill=v.css("background-color")}if(!z.border||e.test(z.border)){z.border=l.css(d);if(e.test(z.border))z.border=v.css(d)||z.fill}c("*",a).add(a).css("background-color","transparent").css("border",0)},create:function(){var a=n.width,l=n.height;k.tip&&k.tip.remove();k.tip=
c("<div />",{"class":"ui-tooltip-tip"}).css(n).toggleClass("ui-widget-content",f.options.style.widget).prependTo(v);switch(C){case "canvas":c('<canvas height="'+l+'" width="'+a+'" />').appendTo(k.tip)[0].getContext("2d").save();break;case "vml":a='<vml:shape coordorigin="0,0" coordsize="'+a+" "+l+'" stroked="false"  style="behavior:url(#default#VML); display:inline-block; position:absolute; antialias:false; left: 0; top: 0; width:'+a+"px; height:"+l+'px;"></vml:shape>';k.tip.html(B?a+=a:a);break;
case "polygon":k.tip.append('<div class="ui-tooltip-tip-inner" />').append(B?'<div class="ui-tooltip-tip-border" />':"");break}},update:function(a){var l=k.tip.children(),w=n.width,d=n.height,e=m.mimic,i,j,g,s,x;if(!a)a=h.corner;if(e===FALSE)e=a;else{e=new c.fn.qtip.plugins.Corner(e);e.precedance=a.precedance;if(e.x==="inherit")e.x=a.x;else if(e.y==="inherit")e.y=a.y;else if(e.x===e.y)e[a.precedance]=a[a.precedance]}B=m.border===TRUE?o(a):m.border;i=B>0?0:1;s=e.string().indexOf("center")>-1;x=Math[/b|r/.test(e[e.precedance===
"y"?"x":"y"])?"ceil":"floor"];switch(C){case "canvas":j=l.get(0).getContext("2d");j.restore&&j.restore();j.clearRect(0,0,3E3,3E3);for(g=N(e,w,d);i<2;i++){if(i){j.save();j.translate(x((e.x==="left"?1:e.x==="right"?-1:0)*(B+1)*(e.precedance==="y"?0.5:1)),x((e.y==="top"?1:e.y==="bottom"?-1:0)*(B+1)*(e.precedance==="x"?0.5:1)))}j.beginPath();j.moveTo(g[0][0],g[0][1]);j.lineTo(g[1][0],g[1][1]);j.lineTo(g[2][0],g[2][1]);j.closePath();j.fillStyle=z[i?"fill":"border"];j.fill()}break;case "vml":g=N(e,w,d);
g="m"+g[0][0]+","+g[0][1]+" l"+g[1][0]+","+g[1][1]+" "+g[2][0]+","+g[2][1]+" xe";l.each(function(A){c(this).attr({path:g,fillcolor:z[A||!B?"fill":"border"]}).css("antialias",""+s)});break;case "polygon":l.removeAttr("style").css({position:"absolute",left:0,top:0}).each(function(A){var y={x:e.precedance==="x"?e.x==="left"?"right":"left":e.x,y:e.precedance==="y"?e.y==="top"?"bottom":"top":e.y},p=e.x==="center"?["left","right",y.y,d,w]:["top","bottom",y.x,w,d];A=z[!A&&B?"border":"fill"];e.x==="center"||
e.y==="center"?c(this).css("border-"+p[2],p[3]+"px solid "+A).css("border-"+p[0],x(p[4]/2)+"px dashed transparent").css("border-"+p[1],x(p[4]/2)+"px dashed transparent"):c(this).css("border-width",x(d/2)+"px "+x(w/2)+"px").css("border-"+y.x,x(w/2)+"px solid "+A).css("border-"+y.y,x(d/2)+"px solid "+A)});break}if(C!=="canvas"&&B){i=[B*2.75,B];i=e.precedance==="y"?[e.x==="left"?i[1]:e.x==="right"?-i[1]:0,e.y==="bottom"?-i[0]:i[0]]:[e.x==="left"?i[0]:-i[0],e.y==="bottom"?-i[1]:e.y==="top"?i[1]:0];l.eq(1).css({left:i[0],
top:i[1]})}return h.position(a,1)},position:function(a,l){var w=k.tip,d={},e=Math.max(0,m.offset),i,j;if(m.corner===FALSE||!w)return FALSE;a=a||h.corner;i=a.precedance;j=n[i==="x"?"width":"height"];c.each(i==="y"?[a.x,a.y]:[a.y,a.x],function(g,s){if(s==="center"){var x=i==="y"?"left":"top";d[x]="50%";d["margin-"+x]=-Math.floor(j/2)+e}else d[s]=g||!B?o(a,s):e});d[a[i]]-=j;l&&w.css({top:"",bottom:"",left:"",right:"",margin:""}).css(d);return d},destroy:function(){k.tip&&k.tip.remove();v.unbind(".qtip-tip")}});
h.init()}function S(f){var b=this,o=f.options.show.modal,h=f.elements,m=h.tooltip;b.checks={"^show.modal.(on|blur)$":function(){b.init();h.overlay.toggle(m.is(":visible"))}};c.extend(b,{init:function(){if(o.on){m.unbind(".qtipmodal").bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(k,v,q){k=k.type.replace("tooltip","");c.isFunction(o[k])?o[k].call(h.overlay,q,v):b[k](q)});b.create();o.blur===TRUE&&h.overlay.unbind(".qtipmodal"+f.id).bind("click.qtipmodal"+f.id,function(){f.hide.call(f)})}},
create:function(){var k=c("#qtip-overlay");if(k.length)h.overlay=k;else{h.overlay=c("<div />",{id:"#qtip-overlay".substr(1),css:{position:"absolute",top:0,left:0,display:"none",zIndex:parseInt(m.css("z-index"),10)-1}}).appendTo(document.body);c(G).bind("resize.qtipmodal",function(){h.overlay.css({height:Math.max(c(G).height(),c(document).height()),width:Math.max(c(G).width(),c(document).width())})}).trigger("resize")}},toggle:function(k){var v=h.overlay,q=f.options.show.modal.effect,n=k?"show":"hide";
v.stop(TRUE,FALSE);if(c.isFunction(q))q.call(v,k);else q===FALSE?v[n]():v.fadeTo(90,k?0.7:0,function(){k||c(this).hide()})},show:function(){b.toggle(TRUE)},hide:function(){b.toggle(FALSE)},destroy:function(){var k=TRUE;c("*").each(function(){var v=c(this).data("qtip");if(v&&v.id!==f.id&&v.options.show.modal)return k=FALSE});if(k){h.overlay.remove();c(G).unbind(".qtipmodal")}else h.overlay.unbind(".qtipmodal"+f.id);m.unbind("tooltipshow.qtipmodal tooltiphide.qtipmodal")}});b.init()}function T(f){var b=
...
q.corner.precedance==="x"?["width","left"]:["height","top"];z[q[1]]-=n[q[0]]()}o.bgiframe.css(z).css(v)},destroy:function(){b.iframe.remove();h.unbind(k)}});b.init()}c.fn.qtip=function(f,b,o){var h=String(f).toLowerCase(),m=NULL,k=h==="disable"?[TRUE]:c.makeArray(arguments).slice(1,10),v=k[k.length-1],q;if(!arguments.length&&this.data("qtip")||h==="api")return(q=this.data("qtip"))?q:L;else if("string"===typeof f){this.each(function(){var n=c(this).data("qtip");if(!n)return TRUE;if(/option|set/.test(h)&&
b)if(o!==L)n.set(b,o);else m=n.get(b);else{if(!n.rendered&&(h==="show"||h==="toggle")){if(v&&v.timeStamp)n.cache.event=v;n.render(1)}else if(h==="enable"){h="disable";k=[FALSE]}n[h]&&n[h].apply(n[h],k)}});return m!==NULL?m:this}else if("object"===typeof f||!arguments.length){q=M(c.extend(TRUE,{},f));return c.fn.qtip.bind.call(this,q,v)}};c.fn.qtip.bind=function(f,b){return this.each(function(){function o(z){function B(){n.render(typeof z==="object"||h.show.ready);m.show.unbind(k.show);m.hide.unbind(k.hide)}
if(n.cache.disabled)return FALSE;n.cache.event=c.extend({},z);if(h.show.delay>0){clearTimeout(n.timers.show);n.timers.show=setTimeout(B,h.show.delay);k.show!==k.hide&&m.hide.bind(k.hide,function(){clearTimeout(n.timers.show)})}else B()}var h,m,k,v=f.id=!f.id||f.id===FALSE||f.id.length<1||c("#ui-tooltip-"+f.id).length?c.fn.qtip.nextid++:f.id,q=".qtip-"+v+"-create",n=P.call(this,v,f);if(n===FALSE)return TRUE;h=n.options;c.each(c.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(n)});m=
