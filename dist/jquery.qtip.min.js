...
"aria-atomic":w}));c.rendered=w;if(p){y();z(p)}N(i);x();a.each(a.fn.qtip.plugins,function(){this.initialize==="render"&&this(c)});h(1,1,1,1);a.each(b.events,function(t,D){if(D)g.bind(t==="toggle"?"tooltipshow tooltiphide":"tooltip"+t,D)});g.css("visibility","hidden").queue("fx",function(t){r.originalEvent=c.cache.event;g.trigger(r,[c]);if(b.show.ready||d)c.show(c.cache.event);t()});return c},get:function(d){switch(d.toLowerCase()){case "dimensions":d={height:g.outerHeight(),width:g.outerWidth()};
