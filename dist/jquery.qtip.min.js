...
(function(a,O,T){function V(e){var b;if(!e||"object"!==typeof e)return n;if("object"!==typeof e.metadata)e.metadata={type:e.metadata};if("content"in e){if("object"!==typeof e.content||e.content.jquery)e.content={text:e.content};b=e.content.text||n;if(!a.isFunction(b)&&(!b&&!b.attr||b.length<1||"object"===typeof b&&!b.jquery))e.content.text=n;if("title"in e.content){if("object"!==typeof e.content.title)e.content.title={text:e.content.title};b=e.content.title.text||n;if(!a.isFunction(b)&&(!b&&!b.attr||
...
return e}})}function ba(e,b){var m,l,q,j=a(this),s=a(document.body),x=this===document?s:j;l=j.metadata?j.metadata(b.metadata):F;var k=b.metadata.type==="html5"&&l?l[b.metadata.name]:F,E=j.data(b.metadata.name||"qtipopts");k=a.extend(w,{},a.fn.qtip.defaults,b,V(E),V(k||l));l&&a.removeData(this,"metadata");l=k.position;if("boolean"===typeof k.content.text){q=j.attr(k.content.attr);if(k.content.attr!==n&&q)k.content.text=q;else return n}if(l.container===n)l.container=s;if(l.target===n)l.target=x;if(k.show.target===
n)k.show.target=x;if(k.hide.target===n)k.hide.target=x;if(k.position.viewport===w)k.position.viewport=l.container;l.at=new a.fn.qtip.plugins.Corner(l.at);l.my=new a.fn.qtip.plugins.Corner(l.my);if(a.data(this,"qtip"))if(k.overwrite)j.qtip("destroy");else if(k.overwrite===n)return n;if(a.attr(this,"title")){a.data(this,Y,a.attr(this,"title"));j.removeAttr("title")}m=new $(j,k,e,!!q);a.data(this,"qtip",m);j.bind("remove.qtip",function(){m.destroy()});return m}function ca(e){var b=this,m=e.elements.tooltip,
...
w){if(o.left)h.x=h.x==="center"?o.left>0?"left":"right":h.x==="left"?"right":"left";if(o.top)h.y=h.y==="center"?o.top>0?"top":"bottom":h.y==="top"?"bottom":"top";if(h.string()!==k.corner&&(k.top!==o.top||k.left!==o.left))v=q.update(h)}v||(v=q.position(h,0));if(v.right!==T)v.left=v.right;if(v.bottom!==T)v.top=v.bottom;v.option=Math.max(0,j.offset);c.left-=v.left.charAt?v.option:(v.right?-1:1)*v.left;c.top-=v.top.charAt?v.option:(v.bottom?-1:1)*v.top;k.left=o.left;k.top=o.top;k.corner=h.string()}}function m(h,
o,c){o=!o?h[h.precedance]:o;o="border-"+o+"-width";h=parseInt((s.titlebar&&h.y==="top"?s.titlebar:s.content).css(o),10);return(c?h||parseInt(x.css(o),10):h)||0}function l(h){var o=h.precedance==="y",c=E[o?"width":"height"],v=E[o?"height":"width"],B=h.string().indexOf("center")>-1,g=c*(B?0.5:1),d=Math.pow;h=Math.round;var f=Math.sqrt(d(g,2)+d(v,2));g=[z/g*f,z/v*f];g[2]=Math.sqrt(d(g[0],2)-d(z,2));g[3]=Math.sqrt(d(g[1],2)-d(z,2));B=(f+g[2]+g[3]+(B?0:g[0]))/f;c=[h(B*v),h(B*c)];return{height:c[o?0:1],
width:c[o?1:0]}}var q=this,j=e.options.style.tip,s=e.elements,x=s.tooltip,k={top:0,left:0,corner:""},E={width:j.width,height:j.height},y={},z=j.border||0,N=a("<canvas />")[0].getContext;q.corner=F;q.mimic=F;q.checks={"^position.my|style.tip.(corner|mimic|border)$":function(){q.init()||q.destroy();e.reposition()},"^style.tip.(height|width)$":function(){E={width:j.width,height:j.height};q.create();q.update();e.reposition()},"^content.title.text|style.(classes|widget)$":function(){s.tip&&q.update()}};
a.extend(q,{init:function(){var h=q.detectCorner()&&(N||a.browser.msie);if(h){q.create();q.update();x.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",b)}return h},detectCorner:function(){var h=j.corner,o=e.options.position,c=o.at;o=o.my.string?o.my.string():o.my;if(h===n||o===n&&c===n)return n;else if(h===w)q.corner=new a.fn.qtip.plugins.Corner(o);else if(!h.string){q.corner=new a.fn.qtip.plugins.Corner(h);q.corner.fixed=w}return q.corner.string()!=="centercenter"},detectColours:function(){var h=
s.tip.css({backgroundColor:"",border:""}),o=q.corner,c=o[o.precedance],v="border-"+c+"-color";c="border"+c.charAt(0)+c.substr(1)+"Color";var B=/rgba?\(0, 0, 0(, 0)?\)|transparent/i;o=s.titlebar&&(o.y==="top"||o.y==="center"&&h.position().top+E.height/2+j.offset<s.titlebar.outerHeight(1))?s.titlebar:s.content;y.fill=h.css("background-color")||"transparent";y.border=h[0].style[c];if(!y.fill||B.test(y.fill)){y.fill=o.css("background-color");if(B.test(y.fill))y.fill=x.css("background-color")}if(!y.border||
...
s.tip.html(z?h+=h:h)}},update:function(h){var o=s.tip,c=o.children(),v=E.width,B=E.height,g=j.mimic,d=Math.round,f,i,p;if(!h)h=q.corner;if(g===n)g=h;else{g=new a.fn.qtip.plugins.Corner(g);g.precedance=h.precedance;if(g.x==="inherit")g.x=h.x;else if(g.y==="inherit")g.y=h.y;else if(g.x===g.y)g[h.precedance]=h[h.precedance]}f=g.precedance;q.detectColours();z=y.border==="transparent"||y.border==="#123456"?0:j.border===w?m(h,F,w):j.border;i=da(g,v,B);p=l(h);o.css(p);o=h.precedance==="y"?[d(g.x==="left"?
...
1)},position:function(h,o){var c=s.tip,v={},B=Math.max(0,j.offset),g,d,f;if(j.corner===n||!c)return n;h=h||q.corner;g=h.precedance;d=l(h)[g==="x"?"width":"height"];f=a.browser.msie&&z&&/^(b|r)/i.test(h.string())?1:0;a.each(g==="y"?[h.x,h.y]:[h.y,h.x],function(i,p){var r,t;if(p==="center"){r=g==="y"?"left":"top";v[r]="50%";v["margin-"+r]=-Math.round(d/2)+B}else{r=m(h,p,w);t=a.browser.mozilla;var D=h.y+(t?"":"-")+h.x;t=(t?"-moz-":a.browser.webkit?"-webkit-":"")+(t?"border-radius-"+D:"border-"+D+"-radius");
t=parseInt((s.titlebar&&h.y==="top"?s.titlebar:s.content).css(t),10)||parseInt(x.css(t),10)||0;v[p]=i||!z?m(h,p):B+(t>r?t:0)}});v[h[g]]-=d+f;o&&c.css({top:"",bottom:"",left:"",right:"",margin:""}).css(v);return v},destroy:function(){s.tip&&s.tip.remove();x.unbind(".qtip-tip")}});q.init()}function fa(e){var b=this,m=e.options.show.modal,l=e.elements,q=l.tooltip;b.checks={"^show.modal.(on|blur)$":function(){b.init();l.overlay.toggle(q.is(":visible"))}};a.extend(b,{init:function(){if(m.on){q.unbind(".qtipmodal").bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",
...
a(document).height()),width:Math.max(a(O).width(),a(document).width())})}).trigger("resize")}},toggle:function(j){var s=l.overlay,x=e.options.show.modal.effect,k=j?"show":"hide";s.stop(w,n);if(a.isFunction(x))x.call(s,j);else x===n?s[k]():s.fadeTo(90,j?0.7:0,function(){j||a(this).hide()})},show:function(){b.toggle(w)},hide:function(){b.toggle(n)},destroy:function(){var j=w;a("*").each(function(){var s=a(this).data("qtip");if(s&&s.id!==e.id&&s.options.show.modal)return j=n});if(j){l.overlay.remove();
...
adjust:function(){var s=e.get("dimensions"),x=e.plugins.tip,k=e.elements.tip,E;E=parseInt(l.css("border-left-width"),10)||0;E={left:-E,top:-E};if(x&&k){x=x.corner.precedance==="x"?["width","left"]:["height","top"];E[x[1]]-=k[x[0]]()}m.bgiframe.css(E).css(s)},destroy:function(){b.iframe.remove();l.unbind(j)}});b.init()}var w=true,n=false,F=null,M="ui-tooltip",Z="ui-widget",S="ui-state-disabled",W="div.qtip."+M,X=M+"-focus",aa="-31000px",Y="oldtitle";a.fn.qtip=function(e,b,m){var l=String(e).toLowerCase(),
q=F,j=l==="disable"?[w]:a.makeArray(arguments).slice(1,10),s=j[j.length-1],x=this[0]?a.data(this[0],"qtip"):F;if(!arguments.length&&x||l==="api")return x;else if("string"===typeof e){this.each(function(){var k=a.data(this,"qtip");if(!k)return w;if(/option|set/.test(l)&&b)if(m!==T)k.set(b,m);else q=k.get(b);else{if(!k.rendered&&(l==="show"||l==="toggle")){if(s&&s.timeStamp)k.cache.event=s;k.render(1)}else if(l==="enable"){l="disable";j=[n]}k[l]&&k[l].apply(k[l],j)}});return q!==F?q:this}else if("object"===
typeof e||!arguments.length){x=V(a.extend(w,{},e));return a.fn.qtip.bind.call(this,x,s)}};a.fn.qtip.bind=function(e,b){return this.each(function(){function m(E){function y(){k.render(typeof E==="object"||l.show.ready);q.show.unbind(j.show);q.hide.unbind(j.hide)}if(k.cache.disabled)return n;k.cache.event=a.extend({},E);if(l.show.delay>0){clearTimeout(k.timers.show);k.timers.show=setTimeout(y,l.show.delay);j.show!==j.hide&&q.hide.bind(j.hide,function(){clearTimeout(k.timers.show)})}else y()}var l,q,
j,s=e.id=!e.id||e.id===n||e.id.length<1||a("#"+M+"-"+e.id).length?a.fn.qtip.nextid++:e.id,x=".qtip-"+s+"-create",k=ba.call(this,s,e);if(k===n)return w;l=k.options;a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(k)});q={show:l.show.target,hide:l.hide.target};j={show:String(l.show.event).replace(" ",x+" ")+x,hide:String(l.hide.event).replace(" ",x+" ")+x};q.show.bind(j.show,m);if(l.show.ready||l.prerender)m(b)})};a.each({attr:function(e,b){if(this.length){var m=this[0],l=a.data(m,
"qtip");if(e==="title")if(arguments.length===1)return a.data(m,Y);else if(l){l.rendered&&l.options.content.attr==="title"&&l.cache.attr&&l.set("content.text",b);a.fn.attr_replacedByqTip.apply(this,arguments);a.data(m,Y,a.attr(m,"title"));return this.removeAttr("title")}}},clone:function(){var e=a([]),b;a("*",this).add(this).each(function(){var m=a.data(this,Y);if(m){a.attr(this,"title",m);e=e.add(this)}});b=a.fn.clone_replacedByqTip.apply(this,arguments);e.removeAttr("title");return b},remove:a.ui?
F:function(e,b){a(this).each(function(){if(!b)if(!e||a.filter(e,[this]).length)a("*",this).add(this).each(function(){a(this).triggerHandler("remove")})})}},function(e,b){if(!b)return w;var m=a.fn[e+"_replacedByqTip"]=a.fn[e];a.fn[e]=function(){return b.apply(this,arguments)||m.apply(this,arguments)}});a(O).load(function(){var e=document,b=e.body;a(e).bind("mousemove.qtip",function(m){a.fn.qtip.mouse={pageX:m.pageX,pageY:m.pageY}});e.activeElement===T&&e.addEventListener("focus",function(m){if(m&&
...
'"]'),x=s.offset(),k={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},E=0,y=0;x.left+=Math.ceil((s.outerWidth()-s.width())/2);x.top+=Math.ceil((s.outerHeight()-s.height())/2);if(l==="poly")for(E=q.length;E--;){y=[parseInt(q[--E],10),parseInt(q[E+1],10)];if(y[0]>k.offset.right)k.offset.right=y[0];if(y[0]<k.offset.left)k.offset.left=y[0];if(y[1]>k.offset.bottom)k.offset.bottom=y[1];if(y[1]<k.offset.top)k.offset.top=y[1];j.push(y)}else j=a.map(q,function(z){return parseInt(z,10)});switch(l){case "rect":k=
