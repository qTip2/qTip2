...
top:f[1]})}h.reposition(a);return h},reposition:function(a){var m=l.tip,v=["left","right"],e=j.offset,f,g;if(j.corner===FALSE||!m)return FALSE;a=a||h.corner;f=a.precedance;g=a[f==="y"?"x":"y"];v[f==="y"?"push":"unshift"]("top","bottom");e=Math.max(g==="center"?e:0,e);m.css({top:"",bottom:"",left:"",right:"",margin:""});switch(g){case "center":m.css(v[0],"50%").css("margin-"+v[0],-Math.floor(n[f==="y"?"width":"height"]/2)+e);break;case v[0]:m.css(v[0],e);break;case v[1]:m.css(v[1],e);break}e=n[f===
"x"?"width":"height"];if(z){o.toggleClass("ui-tooltip-accessible",!o.is(":visible"));e-=parseInt(w.css("border-"+a[f]+"-width"),10)||0;o.removeClass("ui-tooltip-accessible")}m.css(a[f],-e)},destroy:function(){l.tip&&l.tip.remove();o.unbind(".qtip-tip")}});h.init()}function S(d){var b=this,h=d.options.show.modal,j=d.elements,l=j.tooltip;b.checks={"^show.modal.(on|blur)$":function(){b.init();j.overlay.toggle(l.is(":visible"))}};c.extend(b,{init:function(){if(h.on){l.unbind(".qtipmodal").bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",
function(o,w,q){o=o.type.replace("tooltip","");c.isFunction(h[o])?h[o].call(j.overlay,q,w):b[o](q)});b.create();h.blur===TRUE&&j.overlay.unbind(".qtipmodal"+d.id).bind("click.qtipmodal"+d.id,function(){d.hide.call(d)})}},create:function(){var o=c("#qtip-overlay");if(o.length)j.overlay=o;else{j.overlay=c("<div />",{id:"#qtip-overlay".substr(1),css:{position:"absolute",top:0,left:0,display:"none",zIndex:parseInt(l.css("z-index"),10)}}).appendTo(document.body);c(F).bind("resize.qtipmodal",function(){j.overlay.css({height:Math.max(c(F).height(),
