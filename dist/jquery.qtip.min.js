...
k}}var g=this,q=c.options.style.tip,l=c.elements,v=l.tooltip,p=l.wrapper,z={top:0,left:0,corner:{string:function(){}}},t={width:q.width,height:q.height},y={},B=q.border||0,f=q.method||FALSE;g.corner=NULL;g.mimic=NULL;g.checks={"^position.my|style.tip.(corner|mimic|method|border)":function(){B=q.border;if(g.init())this.get("position.target")!=="mouse"&&this.reposition();else g.destroy()},"^style.tip.(height|width)":function(){t={width:q.width,height:q.height};g.create();g.update();c.reposition()}};
a.extend(g,{init:function(){var k=a.browser.msie,j=g.mimic&&/center/i.test(g.mimic.string()),b=g.detectCorner();if(b){if(f===TRUE)f=a("<canvas />")[0].getContext?"canvas":k&&(j||t.height!==t.width)?"vml":"polygon";else if(f==="canvas")f=k?"vml":!a("<canvas />")[0].getContext?"polygon":"canvas";else if(f==="polygon")f=k&&j?"vml":f;g.create();g.detectColours();g.update();v.bind("tooltipmove.tip",h)}return b},detectCorner:function(){var k=q.corner,j=q.mimic||k,b=c.options.position.at,e=c.options.position.my;
if(e.string)e=e.string();if(k===FALSE||e===FALSE&&b===FALSE)return FALSE;else{if(k===TRUE)g.corner=new a.fn.qtip.plugins.Corner(e);else if(!k.string)g.corner=new a.fn.qtip.plugins.Corner(k);if(j===TRUE)g.mimic=new a.fn.qtip.plugins.Corner(e);else if(!j.string){g.mimic=new a.fn.qtip.plugins.Corner(j);g.mimic.precedance=g.corner.precedance}}return g.corner.string()!=="centercenter"},detectColours:function(){var k=l.tip,j=g.mimic[g.mimic.precedance],b="border-"+j+"-color";y.fill=k.css("background-color",
...
w.x,Math.floor(e/2)+"px solid "+r).css("border-"+w.y,Math.floor(i/2)+"px solid "+r)});break}d(k);return g},destroy:function(){l.tip&&l.tip.remove();v.unbind("tooltipmove.tip")}})}function U(c,d){var h=this,g=c.elements,q=g.tooltip;g.overlay=a("#qtip-overlay");a.extend(h,{init:function(){q.bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(l,v,p){l=l.type.replace("tooltip","");a.isFunction(d[l])?d[l].call(g.overlay,p,v):h[l](p)});g.overlay.length||h.create();d.blur===TRUE&&g.overlay.bind("click.qtipmodal"+
c.id,function(){c.hide.call(c)})},create:function(){g.overlay=a("<div />",{id:"qtip-overlay",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(E).bind("resize.qtipmodal",function(){g.overlay.css({height:Math.max(a(E).height(),a(document).height()),width:Math.max(a(E).width(),a(document).width())})}).trigger("resize")},toggle:function(l){var v=g.overlay,p=c.options.show.modal.effect,z=l?"show":"hide";v.stop(TRUE,FALSE);if(a.isFunction(p))p.call(v,l);else p===FALSE?v[z]():
v.fadeTo(90,l?0.7:0,function(){l||a(this).hide()})},show:function(){h.toggle(TRUE)},hide:function(){h.toggle(FALSE)},destroy:function(){var l=TRUE;a("*").each(function(){var v=a(this).data("qtip");if(v&&v.id!==c.id&&v.options.show.modal)return l=FALSE});if(l){g.overlay.remove();a(E).unbind("scroll.qtipmodal resize.qtipmodal")}else g.overlay.unbind("click.qtipmodal"+c.id);q.unbind("tooltipshow.qtipmodal tooltiphide.qtipmodal")}});h.init()}function V(c){var d=this,h=c.elements,g=h.tooltip,q=".bgiframe-"+
c.id,l="tooltipmove"+q+" tooltipshow"+q;a.extend(d,{init:function(){h.bgiframe=a('<iframe class="ui-tooltip-bgiframe" frameborder="0" tabindex="-1" src="javascript:\'\';"  style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=0);"></iframe>');h.bgiframe.appendTo(g);g.bind(l,d.adjust)},adjust:function(){var v=c.get("dimensions"),p=c.plugins.tip,z=c.elements.tip,t;t=parseInt(g.css("border-left-width"),10)||0;t={left:-t,top:-t};if(p&&z){p=p.corner.precedance==="x"?["width","left"]:
["height","top"];t[p[1]]-=z[p[0]]()}h.bgiframe.css(t).css(v)},destroy:function(){d.iframe.remove();g.unbind(l)}});d.init()}a.fn.qtip=function(c,d,h){var g=String(c).toLowerCase(),q=NULL,l=g==="disable"?[TRUE]:a.makeArray(arguments).slice(1,10),v=l[l.length-1],p=a.extend(TRUE,{},c),z;if(!arguments.length&&this.data("qtip")||g==="api")return(p=this.data("qtip"))?p.hash():M;else if("string"===typeof c){this.each(function(){var t=a(this).data("qtip");if(!t)return TRUE;if(/option|set/.test(g)&&d)if(h!==
M)t.set(d,h);else q=t.get(d);else{if(!t.rendered&&(g==="show"||g==="toggle")){if(v&&v.timeStamp)t.cache.event=v;t.render(1)}else if(g==="enable"){g="disable";l=[FALSE]}t[g]&&t[g].apply(t[g],l)}});return q!==NULL?q:this}else if("object"===typeof c||!arguments.length){z=N(p,this);p=a.extend(TRUE,{},a.fn.qtip.defaults,p);return a.fn.qtip.bind.call(z,p,v)}};a.fn.qtip.bind=function(c,d){return this.each(function(h){function g(f){function k(){p.render(typeof f==="object"||z.show.ready);t.show.unbind(y.show);
t.hide.unbind(y.hide)}if(p.cache.disabled)return FALSE;p.cache.event=a.extend({},f);if(z.show.delay>0){clearTimeout(p.timers.show);p.timers.show=setTimeout(k,z.show.delay);y.show!==y.hide&&t.hide.bind(y.hide,function(){clearTimeout(p.timers.show)})}else k()}var q=a(this),l=c.id,v=c.content.text,p,z,t,y,B;c.id=l=l===FALSE||l.length<1||a("#ui-tooltip-"+l).length?a.fn.qtip.nextid++:l;B=".qtip-"+l+"-create";p=R.call(this,l,c);if(p===FALSE)return TRUE;z=p.options;if(a.isArray(v))z.content.text=v[h];q.attr("title")&&
q.data("oldtitle",q.attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(p)});t={show:z.show.target,hide:z.hide.target};y={show:String(z.show.event).replace(" ",B+" ")+B,hide:String(z.hide.event).replace(" ",B+" ")+B};t.show.bind(y.show,g);if(c.show.ready||c.prerender)g(d)})};a.each({attr:function(c){var d=this.data("qtip");return arguments.length===1&&c==="title"&&d&&d.rendered===TRUE?this.data("oldtitle"):NULL},remove:a.ui?NULL:function(c,d){this.each(function(){if(!d)if(!c||
...
d!=="c"&&h!=="c"?h+d:d+h}}};a.fn.qtip.defaults={prerender:FALSE,id:FALSE,overwrite:TRUE,metadata:{type:"class"},content:{text:TRUE,attr:"title",title:{text:FALSE,button:FALSE}},position:{my:"top left",at:"bottom right",target:FALSE,container:FALSE,adjust:{x:0,y:0,mouse:TRUE,screen:FALSE,resize:TRUE,container:FALSE},effect:TRUE},show:{target:FALSE,event:"mouseenter",effect:TRUE,delay:90,solo:FALSE,ready:FALSE},hide:{target:FALSE,event:"mouseleave",effect:TRUE,delay:0,fixed:FALSE,inactive:FALSE},style:{classes:"",
widget:FALSE},events:{render:a.noop,move:a.noop,show:a.noop,hide:a.noop,focus:a.noop,blur:a.noop}};a.fn.qtip.plugins.ajax=function(c){var d=c.plugins.ajax,h=c.options.content.ajax;if(h&&h.url)if(d)return d;else{c.plugins.ajax=new S(c);return c.plugins.ajax}};a.fn.qtip.plugins.ajax.initialize="render";a.fn.qtip.plugins.ajax.sanitize=function(c){try{var d=c.content.ajax;if(typeof d!=="object")d=c.content.ajax={url:d};if("boolean"!==typeof d.once&&d.once)d.once=!!d.once}catch(h){}};a.extend(TRUE,a.fn.qtip.defaults,
{content:{ajax:{once:TRUE}}});a.fn.qtip.plugins.tip=function(c){var d=c.plugins.tip,h=c.options.style.tip;if(h&&h.corner)if(d)return d;else{c.plugins.tip=new T(c);c.plugins.tip.init();return c.plugins.tip}};a.fn.qtip.plugins.tip.initialize="render";a.fn.qtip.plugins.tip.sanitize=function(c){try{var d=c.style.tip;if(typeof d!=="object")c.style.tip={corner:!!d};if(typeof d.method!=="string")d.method=TRUE;if(!/canvas|polygon/i.test(d.method))d.method=TRUE;if(typeof d.width!=="number")d.width=12;if(typeof d.height!==
"number")d.height=12;if(typeof d.border!=="number")d.border=0}catch(h){}};a.extend(TRUE,a.fn.qtip.defaults,{style:{tip:{corner:TRUE,method:TRUE,width:12,height:12,border:0,offset:0}}});a.fn.qtip.plugins.imagemap=function(c,d){function h(y,B){for(var f=0,k=1,j=1,b=0,e=0,i=y.width,n=y.height;i>0&&n>0&&k>0&&j>0;){i=Math.floor(i/2);n=Math.floor(n/2);if(d.x==="left")k=i;else if(d.x==="right")k=y.width-i;else k+=Math.floor(i/2);if(d.y==="top")j=n;else if(d.y==="bottom")j=y.height-n;else j+=Math.floor(n/
...
t=0;if(g==="poly")for(z=q.length;z--;){t=[parseInt(q[--z],10),parseInt(q[z+1],10)];if(t[0]>p.offset.right)p.offset.right=t[0];if(t[0]<p.offset.left)p.offset.left=t[0];if(t[1]>p.offset.bottom)p.offset.bottom=t[1];if(t[1]<p.offset.top)p.offset.top=t[1];l.push(t)}else l=a.map(q,function(y){return parseInt(y,10)});switch(g){case "rect":p={width:Math.abs(l[2]-l[0]),height:Math.abs(l[3]-l[1]),offset:{left:l[0],top:l[1]}};break;case "circle":p={width:l[2]+2,height:l[2]+2,offset:{left:l[0],top:l[1]}};break;
case "poly":a.extend(p,{width:Math.abs(p.offset.right-p.offset.left),height:Math.abs(p.offset.bottom-p.offset.top)});p.offset=d.string()==="centercenter"?{left:p.offset.left+p.width/2,top:p.offset.top+p.height/2}:h(p,l.slice());p.width=p.height=0;break}p.offset.left+=v.left;p.offset.top+=v.top;return p};a.fn.qtip.plugins.modal=function(c){var d=c.plugins.modal,h=c.options.show.modal;if(d)return d;else if(h&&h.on===TRUE){c.plugins.modal=new U(c,h);return c.plugins.modal}};a.fn.qtip.plugins.modal.initialize=
