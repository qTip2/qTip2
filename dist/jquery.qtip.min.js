...
"-icon",!f)}function s(){var b=d.elements;if(b.title){b.titlebar.remove();b.titlebar=b.title=b.button=D;e.removeAttr("aria-labelledby");d.reposition()}}function r(){var b=d.elements,f=a.content.title.button;b.button&&b.button.remove();b.button=f.jquery?f:c("<a />",{"class":"ui-state-default "+(a.style.widget?"":H+"-icon"),title:"Close"}).prepend(c("<span />",{"class":"ui-icon ui-icon-close",html:"&times;"}));b.button.prependTo(b.titlebar).attr("role","button").hover(function(i){c(this).toggleClass("ui-state-hover",
...
c(this).bind(F,function(){clearTimeout(d.timers.img[o]);l(this)});(function x(){if(C.height)return l(C);d.timers.img[o]=setTimeout(x,20)})();return u});n.length===0&&l(n)});return d}function G(b,f,i,l){function n(v){if(g.hasClass(M))return m;E.show.trigger("qtip-"+p+"-inactive");clearTimeout(d.timers.show);clearTimeout(d.timers.hide);var t=function(){d.show(v)};if(a.show.delay>0)d.timers.show=setTimeout(t,a.show.delay);else t()}function o(v){if(g.hasClass(M))return m;var t=c(v.relatedTarget||v.target).parents(R)[0]===
g[0],Q=c(v.relatedTarget||v.target)[0]===E.show[0];clearTimeout(d.timers.show);clearTimeout(d.timers.hide);if(a.hide.fixed&&(A.target==="mouse"&&t||/mouse(out|leave|move)/.test(v.type)&&(t||Q))){v.stopPropagation();v.preventDefault();return m}g.stop(1,1);if(a.hide.delay>0)d.timers.hide=setTimeout(function(){d.hide(v)},a.hide.delay);else d.hide(v)}function C(v){if(g.hasClass(M))return m;clearTimeout(d.timers.inactive);d.timers.inactive=setTimeout(function(){d.hide(v)},a.hide.inactive)}function F(v){if(!d.cache.processing){d.cache.processing=
1;j()&&d.reposition(v);d.cache.processing=0}}var x=".qtip-"+p,A=a.position,E={show:a.show.target,hide:a.hide.target,container:A.container[0]===y?document:A.container},L={show:String(a.show.event).split(" "),hide:String(a.hide.event).split(" ")},N=c.browser.msie&&parseInt(c.browser.version,10)===6;if(i){if(a.hide.fixed){E.hide=E.hide.add(g);g.bind("mouseover"+x,function(){g.hasClass(M)||clearTimeout(d.timers.hide)})}g.bind("mouseenter"+x+" mouseleave"+x,function(v){d[v.type==="mouseenter"?"focus":
"blur"](v)})}if(f){if("number"===typeof a.hide.inactive){E.show.bind("qtip-"+p+"-inactive",C);c.each(c.fn.qtip.inactiveEvents,function(v,t){E.hide.add(d.elements.tooltip).bind(t+x+"-inactive",C)})}c.each(L.hide,function(v,t){var Q=c.inArray(t,L.show);if(Q>-1&&c(E.hide).add(E.show).length===c(E.hide).length||t==="unfocus"){E.show.bind(t+x,function(I){j()?o(I):n(I)});delete L.show[Q]}else E.hide.bind(t+x,o)})}b&&c.each(L.show,function(v,t){E.show.bind(t+x,n)});if(l){if(A.adjust.resize||A.viewport)c(c.event.special.resize?
...
D,hide:f?a.hide.target:D,tooltip:i?d.elements.tooltip:D,content:i?d.elements.content:D,container:l?a.position.container[0]===y?document:a.position.container:D,window:l?K:D};if(d.rendered)c([]).pushStack(c.grep([f.show,f.hide,f.tooltip,f.container,f.content,f.window],function(){return this!==null})).unbind(n);else b&&f.show.unbind(n+"-create")}var d=this,y=document.body,g;d.id=p;d.rendered=m;d.elements={target:e};d.timers={img:[]};d.options=a;d.plugins={};d.cache={event:{},target:D,disabled:m,lastFocus:y};
...
d.cache.event=c.extend({},f)}o=c.Event("tooltip"+l);o.originalEvent=f?d.cache.event:D;g.trigger(o,[d,90]);if(o.isDefaultPrevented())return d;if(b){g.hide().css({visibility:""});d.focus(f);d.reposition(f,0);n.solo&&c(R).not(g).qtip("hide")}else{clearTimeout(d.timers.show);d.blur(f)}g.attr("aria-hidden",Boolean(!b));g.stop(1,1);if(c.isFunction(n.effect)){n.effect.call(g,d);g.queue("fx",function(C){i.call(this,C);C()})}else if(n.effect===m){g[l]();i.call(g)}else g.fadeTo(90,b?1:0,i);b&&n.target.trigger("qtip-"+
...
l}return d},blur:function(b){b=c.extend({},b);var f;g.removeClass(T);f=c.Event("tooltipblur");f.originalEvent=b;g.trigger(f,[d]);d.cache.lastFocus.focus()},reposition:function(b,f){if(d.rendered===m)return m;var i=a.position.target,l=a.position,n=l.my,o=l.at,C=l.adjust,F=g.outerWidth(),x=g.outerHeight(),A=0,E=0,L=c.Event("tooltipmove"),N=g.css("position")==="fixed",v=l.viewport.jquery?l.viewport:c(K),t={left:0,top:0},Q={left:function(I){var J=v.scrollLeft,O=o.x==="left"?A:o.x==="right"?-A:-A/2,S=
J-I;J=I+F-v.width-J;O=(n.x==="left"?F:n.x==="right"?-F:-F/2)-(n.precedance==="x"||n.x===n.y?O:0);if(S>0&&(n.x!=="left"||J>0))t.left-=O;else if(J>0&&(n.x!=="right"||S>0))t.left-=(n.x==="center"?-1:1)*O+2*C.x;if(t.left<0&&-t.left>J)t.left=I;return t.left-I},top:function(I){var J=v.scrollTop,O=o.y==="top"?E:o.y==="bottom"?-E:-E/2,S=J-I;J=I+x-v.height-J;O=(n.y==="top"?x:n.y==="bottom"?-x:-x/2)-(n.precedance==="y"||n.x===n.y?O:0);if(S>0&&(n.y!=="top"||J>0))t.top-=O;else if(J>0&&(n.y!=="bottom"||S>0))t.top-=
...
c(b.target):d.cache.target;i=c(i).eq(0);if(i.length===0)return d;else if(i[0]===document||i[0]===K){A=i.width();E=i.height();if(i[0]===K)t={top:N?0:v.scrollTop,left:N?0:v.scrollLeft}}else if(i.is("area")&&c.fn.qtip.plugins.imagemap){t=c.fn.qtip.plugins.imagemap(i,o);A=t.width;E=t.height;t=t.offset}else{A=i.outerWidth();E=i.outerHeight();t=q(i[0],l.container[0])}t.left+=o.x==="right"?A:o.x==="center"?A/2:0;t.top+=o.y==="bottom"?E:o.y==="center"?E/2:0}t.left+=C.x+(n.x==="right"?-F:n.x==="center"?-F/
...
w=this===document?j:q;h=q.metadata?q.metadata(a.metadata):D;var s=c.extend(u,{},c.fn.qtip.defaults,a,U((h&&a.metadata.type==="html5"?h[a.metadata.name]:D)||h));q.removeData("metadata");h=s.position;if("boolean"===typeof s.content.text)if(s.content.attr!==m&&q.attr(s.content.attr))s.content.text=q.attr(s.content.attr);else return m;if(h.container===m)h.container=j;if(h.target===m)h.target=w;if(s.show.target===m)s.show.target=w;if(s.hide.target===m)s.hide.target=w;if(s.position.viewport===u)s.position.viewport=
...
{init:function(){if(h&&h.url){a.load();p.one("tooltipshow",a.once)}},once:function(){h.once?a.destroy():p.bind("tooltipshow.qtip-ajax",a.load)},load:function(){c.ajax(c.extend({success:function(q){e.set("content.text",q)},error:function(q,j,w){e.set("content.text",j+": "+w)},context:e},h));return a},destroy:function(){p.unbind(".qtip-ajax")}});a.init()}function X(e,a,p){var h=Math.ceil(a/2),q=Math.ceil(p/2);a={bottomright:[[0,0],[a,p],[a,0]],bottomleft:[[0,0],[a,0],[0,p]],topright:[[0,p],[a,0],[a,
p]],topleft:[[0,0],[0,p],[a,p]],topcenter:[[0,p],[h,0],[a,p]],bottomcenter:[[0,0],[a,0],[h,p]],rightcenter:[[0,0],[a,q],[0,p]],leftcenter:[[a,0],[a,p],[0,q]]};a.lefttop=a.bottomright;a.righttop=a.bottomleft;a.leftbottom=a.topright;a.rightbottom=a.topleft;return a[e.string()]}function aa(e){function a(k,d,y){if(j.tip){k=c.extend({},h.corner);d=y.adjusted;var g;if(h.corner.fixed!==u){if(d.left)k.x=k.x==="center"?d.left>0?"left":"right":k.x==="left"?"right":"left";if(d.top)k.y=k.y==="center"?d.top>0?
"top":"bottom":k.y==="top"?"bottom":"top";if(k.string()!==s.corner&&(s.top!==d.top||s.left!==d.left))g=h.update(k)}g||(g=h.position(k,0));if(g.right!==P)g.left=g.right;if(g.bottom!==P)g.top=g.bottom;g.option=Math.max(0,q.offset);y.left-=g.left.charAt?g.option:(g.right?-1:1)*g.left;y.top-=g.top.charAt?g.option:(g.bottom?-1:1)*g.top;s.left=d.left;s.top=d.top;s.corner=k.string()}}function p(k,d,y){d=!d?k[k.precedance]:d;d="border-"+d+"-width";k=parseInt((j.titlebar&&k.y==="top"?j.titlebar:j.content).css(d),
10);return(y?k||parseInt(w.css(d),10):k)||0}var h=this,q=e.options.style.tip,j=e.elements,w=j.tooltip,s={top:0,left:0,corner:""},r={width:q.width,height:q.height},z={},B=q.border||0,G=q.method||m;h.corner=D;h.mimic=D;h.checks={"^position.my|style.tip.(corner|mimic|method|border)$":function(){h.init()||h.destroy();e.reposition()},"^style.tip.(height|width)$":function(){r={width:q.width,height:q.height};h.create();h.update();e.reposition()},"^content.title.text|style.(classes|widget)$":function(){j.tip&&
...
"x"?["width","left"]:["height","top"];z[s[1]]-=r[s[0]]()}p.bgiframe.css(z).css(w)},destroy:function(){a.iframe.remove();h.unbind(j)}});a.init()}var u=true,m=false,D=null,H="ui-tooltip",W="ui-widget",M="ui-state-disabled",R=".qtip."+H,T=H+"-focus",V="-31000px";c.fn.qtip=function(e,a,p){var h=String(e).toLowerCase(),q=D,j=h==="disable"?[u]:c.makeArray(arguments).slice(1,10),w=j[j.length-1],s;if(!arguments.length&&this.data("qtip")||h==="api")return(s=this.data("qtip"))?s:P;else if("string"===typeof e){this.each(function(){var r=
c(this).data("qtip");if(!r)return u;if(/option|set/.test(h)&&a)if(p!==P)r.set(a,p);else q=r.get(a);else{if(!r.rendered&&(h==="show"||h==="toggle")){if(w&&w.timeStamp)r.cache.event=w;r.render(1)}else if(h==="enable"){h="disable";j=[m]}r[h]&&r[h].apply(r[h],j)}});return q!==D?q:this}else if("object"===typeof e||!arguments.length){s=U(c.extend(u,{},e));return c.fn.qtip.bind.call(this,s,w)}};c.fn.qtip.bind=function(e,a){return this.each(function(){function p(z){function B(){r.render(typeof z==="object"||
