...
(function(a,B,E){function K(b){if(!b)return FALSE;try{if("metadata"in b&&"object"!==typeof b.metadata)b.metadata={type:b.metadata};if("content"in b){if("object"!==typeof b.content||b.content.jquery)b.content={text:b.content};var c=b.content.text||FALSE;if((c.length<1||!c&&!c.attr||"object"===typeof c&&!c.jquery)&&!a.isFunction(c))b.content.text=FALSE;if("title"in b.content&&"object"!==typeof b.content.title)b.content.title={text:b.content.title}}if("position"in b){if("object"!==typeof b.position)b.position=
...
return j}function o(){var d=e.elements;if(d.title){d.titlebar.remove();d.titlebar=d.title=d.button=NULL;d.tooltip.removeAttr("aria-labelledby")}}function v(){var d=e.elements,i=c.content.title.button;d.titlebar&&o();d.titlebar=a("<div />",{"class":"ui-tooltip-titlebar "+(c.style.widget?"ui-widget-header":"")}).append(d.title=a("<div />",{id:"ui-tooltip-"+k+"-title","class":"ui-tooltip-title",html:c.content.title.text})).prependTo(d.wrapper);if(i){d.button=i.jquery?i:"string"===typeof i?a("<a />",
{html:i}):a("<a />",{"class":"ui-state-default"}).append(a("<span />",{"class":"ui-icon ui-icon-close"}));d.button.prependTo(d.titlebar).attr("role","button").addClass("ui-tooltip-"+(i===TRUE?"close":"button")).hover(function(n){a(this).toggleClass("ui-state-hover",n.type==="mouseenter")}).click(function(){d.tooltip.hasClass("ui-state-disabled")||e.hide();return FALSE}).bind("mousedown keydown mouseup keyup mouseout",function(n){a(this).toggleClass("ui-state-active ui-state-focus",/down$/i.test(n.type))})}}
...
a(document).bind("mousedown"+t,function(y){var z=e.elements.tooltip;a(y.target).parents(".qtip.ui-tooltip").length===0&&a(y.target).add(b).length>1&&z.is(":visible")&&!z.hasClass("ui-state-disabled")&&e.hide()});c.position.target==="mouse"&&a(document).bind("mousemove"+t,function(y){c.position.adjust.mouse&&!r.tooltip.hasClass("ui-state-disabled")&&r.tooltip.is(":visible")&&e.reposition(y)})}}function u(d,i,n,j){j=parseInt(j,10)!==0;var f=".qtip-"+k,h={show:d?c.show.target:a("<div/>"),hide:i?c.hide.target:
a("<div/>"),tooltip:n?e.elements.tooltip:a("<div/>")};i={show:String(c.show.event).split(" "),hide:String(c.hide.event).split(" ")};if(e.rendered){a.each(i.show,function(l,m){h.show.unbind(m+f)});h.show.unbind("mousemove"+f).unbind("mouseout"+f).unbind("qtip-"+k+"-inactive");a.each(i.hide,function(l,m){h.hide.add(h.tooltip).unbind(m+f)});a.each(a.fn.qtip.inactiveEvents,function(l,m){h.hide.add(n?e.elements.content:NULL).unbind(m+f+"-inactive")});h.hide.unbind("mouseout"+f);h.tooltip.unbind("mouseover"+
f);if(j){a(B).unbind("resize"+f);a(document).unbind("mousedown"+f+" mousemove"+f)}}else d&&h.show.unbind(i.show+f+"-create")}var e=this;e.id=k;e.rendered=FALSE;e.elements={target:b};e.cache={event:{},target:NULL,disabled:FALSE};e.timers={};e.options=c;e.plugins={};a.extend(e,{render:function(d){var i=e.elements;if(e.rendered)return FALSE;e.rendered=d?-2:-1;i.tooltip=a("<div/>").attr({id:"ui-tooltip-"+k,role:"tooltip"}).addClass("qtip ui-tooltip ui-helper-reset "+c.style.classes).toggleClass("ui-widget",
c.style.widget).toggleClass("ui-state-disabled",e.cache.disabled).css("z-index",a.fn.qtip.zindex+a("div.qtip.ui-tooltip").length).data("qtip",e).appendTo(c.position.container);i.wrapper=a("<div />").addClass("ui-tooltip-wrapper").appendTo(i.tooltip);i.content=a("<div />").addClass("ui-tooltip-content").attr("id","ui-tooltip-"+k+"-content").addClass("ui-tooltip-content").toggleClass("ui-widget-content",c.style.widget).appendTo(i.wrapper);c.content.title.text&&v();a.each(a.fn.qtip.plugins,function(){this.initialize===
"render"&&this(e)});w(1,1,1,1);a.each(c.events,function(n,j){i.tooltip.bind("tooltip"+n,j)});b.bind("remove.qtip",function(){e.destroy()});q(c.content.text);i.tooltip.trigger("tooltiprender",[e.hash()]);return e},get:function(d){switch(d.toLowerCase()){case "offset":d=p("position");break;case "dimensions":d=p("dimensions");break;default:d=g(d.toLowerCase());d=d[0].precedance?d[0].string():d[0].jquery?d[0]:d[0][d[1]];break}return d},set:function(d,i){var n=g(d.toLowerCase()),j,f,h,l={builtin:{"^content.text":function(){q(i)},
"^content.title.text":function(){if(e.rendered)if(!e.elements.title&&i){v();e.reposition()}else i?e.elements.title.html(i):o()},"^position.container$":function(){if(e.rendered===true){e.elements.tooltip.appendTo(i);e.reposition()}},"^position.(my|at)$":function(){var m=/my$/i.test(d)?"my":"at";if("string"===typeof i)c.position[m]=new a.fn.qtip.plugins.Corner(i)},"^position.(my|at|adjust|target)":function(){e.rendered&&e.reposition()},"^(show|hide).(event|target|fixed)":function(m,t,r,x){var s=d.search(/fixed/i)>
-1?[0,[0,1,1,1]]:d.search(/hide/i)<0?["show",[1,0,0,0]]:["hide",[0,1,0,0]];if(s[0])m[t]=x;u.apply(e,s[1]);if(s[0])m[t]=r;w.apply(e,s[1])}}};a.each(e.plugins,function(m){if("object"===typeof this.checks)l[m]=this.checks});j=n[0][n[1]];n[0][n[1]]=i;K(c);for(f in l)for(h in l[f])RegExp(h,"i").test(d)&&l[f][h].call(e,n[0],n[1],i,j);return e},toggle:function(d,i){function n(){var t=d?"attr":"removeAttr",r=/^1|0$/.test(a(this).css("opacity"));e.elements.title&&b[t]("aria-labelledby","ui-tooltip-"+k+"-title");
b[t]("aria-describedby","ui-tooltip-"+k+"-content");if(d){if(a.browser.msie&&a(this).get(0).style&&r){m=a(this).get(0).style;m.removeAttribute("filter");m.removeAttribute("opacity")}}else r&&a(this).hide()}if(!e.rendered)return FALSE;var j=d?"show":"hide",f=e.elements.tooltip,h=c[j],l=f.is(":visible"),m;if((typeof d).search("boolean|number"))d=!f.is(":visible");if(!l&&!d||f.is(":animated"))return e;if(i){if(e.cache.event&&/over|enter/.test(i.type)&&/out|leave/.test(e.cache.event.type)&&a(i.target).add(c.show.target).length<
2&&a(i.relatedTarget).parents(".qtip.ui-tooltip").length>0)return e;e.cache.event=a.extend({},i)}l=a.Event("tooltip"+j);l.originalEvent=a.extend({},i);f.trigger(l,[e.hash(),90]);if(l.isDefaultPrevented())return e;if(d){if(e.rendered===TRUE){e.focus();e.reposition(i)}h.solo&&a(":not(.qtip.ui-tooltip)").qtip("hide")}else clearTimeout(e.timers.show);f.attr("aria-hidden",Boolean(!d));f.stop(TRUE,FALSE);if(a.isFunction(h.effect)){h.effect.call(f);f.queue(function(){n.call(this);a(this).dequeue()})}else if(h.effect===
FALSE){f[j]();n.call(f)}else f.fadeTo(90,d?100:0,n);d&&h.target.trigger("qtip-"+k+"-inactive");return e},show:function(d){e.toggle(TRUE,d)},hide:function(d){e.toggle(FALSE,d)},focus:function(d){if(!e.rendered)return FALSE;var i=e.elements.tooltip,n=parseInt(i.css("z-index"),10),j=a.fn.qtip.zindex+a(".qtip.ui-tooltip").length,f=a.extend({},d);if(!i.hasClass("ui-tooltip-focus")&&n!==j){a(".qtip.ui-tooltip").each(function(){var h=a(this).qtip(),l=a.Event("tooltipblur"),m,t;if(!h||!h.rendered)return TRUE;
m=h.elements.tooltip;t=parseInt(m.css("z-index"),10);isNaN(t)||m.css({zIndex:t-1});m.removeClass("ui-tooltip-focus");l.originalEvent=f;m.trigger(l,[h,j])});d=a.Event("tooltipfocus");d.originalEvent=f;i.trigger(d,[e.hash(),j]);d.isDefaultPrevented()||i.css({zIndex:j}).addClass("ui-tooltip-focus")}return e},reposition:function(d){if(e.rendered===FALSE)return FALSE;var i=c.position.target,n=e.elements.tooltip,j=c.position,f=j.my,h=j.at,l=e.elements.tooltip.width(),m=e.elements.tooltip.height(),t=a(j.container)[0],
r=0,x=0,s={left:0,top:0},y=a.Event("tooltipmove"),z={left:function(A){var C=i==="mouse"?d.pageX:i.offset().left,G=a(B).scrollLeft(),F=a(B).width(),D=h.x==="left"?r:h.x==="right"?-r:r/2,H=D+(f.x==="left"?-l:f.x==="right"?l:l/2)+-2*j.adjust.x;F=A+l-F-G;if(G-A>0&&!(A>=C&&A<C+r))s.left+=H-D;else if(F>0&&A+l>C)s.left+=f.x==="center"?-H+D:H-D;return s.left-A},top:function(A){var C=a(B).scrollTop(),G=a(B).height(),F=h.y==="top"?x:h.y==="bottom"?-x:0,D=F+(f.y==="top"?-m:f.y==="bottom"?m:-m/2)+-2*j.adjust.y;
G=A+m-G-C;if(C-A>0)s.top+=f.y==="center"?-D+F:D;else if(G>0)s.top+=D-F;return s.top-A}};if(i==="mouse"){h={x:"left",y:"top"};if(!d)d=e.cache.event;s={top:d.pageY,left:d.pageX}}else{if(i==="event")i=d&&d.target&&d.type!=="scroll"&&d.type!=="resize"?e.cache.target=a(d.target):e.cache.target;if(i[0]===document||i[0]===B){r=i.width();x=i.height();s={top:n.css("position")==="fixed"?0:i.scrollTop(),left:i.scrollLeft()}}else if(i.is("area")&&a.fn.qtip.plugins.imagemap){s=a.fn.qtip.plugins.imagemap(i,h);
...
j=j||g.corner;f.css({top:"",bottom:"",left:"",right:"",margin:""});h[j.precedance==="y"?"push":"unshift"]("top","bottom");if(a.browser.msie)l={top:j.precedance==="y"?0:0,left:0,bottom:j.precedance==="y"?0:0,right:0};switch(j[j.precedance==="y"?"x":"y"]){case "center":f.css(h[0],"50%").css("margin-"+h[0],-(u[j.precedance==="y"?"width":"height"]/2));break;case h[0]:f.css(h[0],l[h[0]]+i);break;case h[1]:f.css(h[1],l[h[1]]+i);break}m=u[j.precedance==="x"?"width":"height"];if(d)m-=parseInt(q.css("border-"+
...
'" stroked="'+!!d+'"  style="behavior:url(#default#VML); display:inline-block; antialias:TRUE; position: absolute;  top:0; left:0; width:'+j+"px; height:"+f+"px; vertical-align:"+g.corner.y+';"><vml:stroke weight="'+(d-2)+'px" joinstyle="miter" miterlimit="10"  style="behavior:url(#default#VML); display:inline-block;" /></vml:shape>');break;case "polygon":o.tip.append('<div class="ui-tooltip-tip-inner" />').append(d?'<div class="ui-tooltip-tip-border" />':"");break}return g},update:function(j,f){var h=
o.tip,l=u.width,m=u.height,t=d>0?0:1,r=Math.ceil(d/2+0.5),x;f||(f=j?j:g.mimic);if(!j)j=g.corner;if(f.x==="false")f.x=j.x;if(f.y==="false")f.y=j.y;h=h.children();switch(n){case "canvas":h=h.get(0).getContext("2d");h.clearRect(0,0,3E3,3E3);h.restore();if(d){x=J(f.string(),l*2,m*2);h.strokeStyle=e.border;h.lineWidth=d+1;h.lineJoin="miter";h.miterLimit=100;h.save();h.translate(f.x==="left"?0:f.x==="right"?-l:-l/2,f.y==="top"?0:f.y==="bottom"?-m:-m/2)}else x=J(f.string(),l,m);h.fillStyle=e.fill;for(h.miterLimit=
0;t<2;t++){h.globalCompositeOperation=t&&d?"destination-in":"source-over";h.beginPath();h.moveTo(x[0][0],x[0][1]);h.lineTo(x[1][0],x[1][1]);h.lineTo(x[2][0],x[2][1]);h.closePath();h.fill();t||h.stroke()}break;case "vml":x=J(f.string(),l,m);t="m"+x[0][0]+","+x[0][1]+" l"+x[1][0]+","+x[1][1]+" "+x[2][0]+","+x[2][1]+" xe";h.attr({path:t,fillcolor:e.fill});if(d){h.children().attr("color",e.border);if(f.precedance==="y"){h.css("top",(f.y==="top"?1:-1)*(d-2));h.css("left",f.x==="left"?1:-2)}else{h.css("left",
(f.x==="left"?1:-1)*(d-2));h.css("top",f.y==="top"?1:-2)}}break;case "polygon":if(f.precedance==="y"){t=l>m?1.5:l<m?5:2.2;r=[f.x==="left"?r:f.x==="right"?-r:0,Math.floor(t*r*(f.y==="bottom"?-1:1)*(f.x==="center"?0.8:1))]}else{t=l<m?1.5:l>m?5:2.2;r=[Math.floor(t*r*(f.x==="right"?-1:1)*(f.y==="center"?0.9:1)),f.y==="top"?r:f.y==="bottom"?-r:0]}h.removeAttr("style").each(function(s){var y={x:f.precedance==="x"?f.x==="left"?"right":"left":f.x,y:f.precedance==="y"?f.y==="top"?"bottom":"top":f.y},z=f.x===
