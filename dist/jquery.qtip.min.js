...
b.call(c);b.jquery&&b.length>0?e.content.empty().append(b.css({display:"block"})):e.content.html(b);e.tooltip.queue("fx",function(i){function n(p){m=m.not(p);if(m.length===0){p=void 0;var x=f.elements.tooltip,w,u;if(f.rendered&&a.browser.msie&&parseInt(a.browser.version.charAt(0),10)<9){x.css({width:"auto",maxWidth:"none"});p=l("dimensions").width;x.css({maxWidth:""});w=parseInt(x.css("max-width"),10)||0;u=parseInt(x.css("min-width"),10)||0;p=Math.min(Math.max(p,u),w);x.width(p)}f.rendered===TRUE&&
f.reposition(f.cache.event);i()}}var m=a("img:not([height]):not([width])",f.elements.content);m.each(function(p,x){var w=["abort","error","load","unload",""].join(".qtip-image ");a(this).bind(w,function(){clearTimeout(f.timers.img[p]);n(this)});(function u(){if(x.height)return n(x);f.timers.img[p]=setTimeout(u,20)})();return true});m.length===0&&n(m)});return f}function y(b,e,i,n){function m(C){if(r.tooltip.hasClass("ui-state-disabled"))return FALSE;r.show.trigger("qtip-"+h+"-inactive");clearTimeout(f.timers.show);
clearTimeout(f.timers.hide);var A=function(){f.show(d.position.target!=="mouse"?C:NULL)};if(d.show.delay>0)f.timers.show=setTimeout(A,d.show.delay);else A()}function p(C){if(r.tooltip.hasClass("ui-state-disabled"))return FALSE;var A=a(C.relatedTarget).parents(j)[0]==r.tooltip[0];clearTimeout(f.timers.show);clearTimeout(f.timers.hide);if(d.hide.fixed&&(d.position.target==="mouse"&&A||/mouse(out|leave|move)/.test(C.type)&&A)){C.stopPropagation();C.preventDefault();return FALSE}r.tooltip.stop(TRUE);
...
z();o()}else r.remove()},"^position.(my|at)$":function(){var r=/my$/i.test(b)?"my":"at";if("string"===typeof e)d.position[r]=new a.fn.qtip.plugins.Corner(e)},"^position.(my|at|adjust|target)":function(){f.rendered&&f.reposition()},"^position.container$":function(){if(f.rendered===TRUE){m.appendTo(e);f.reposition()}},"^(show|hide).(event|target|fixed|delay|inactive)":function(r,s,F,C){var A=b.search(/fixed/i)>-1?[0,[0,1,1,1]]:[b.substr(0,3),b.charAt(0)==="s"?[1,0,0,0]:[0,1,0,0]];if(A[0])r[s]=C;B.apply(f,
...
if(b){f.focus();f.reposition(e);p.solo&&a(j).qtip("hide")}else clearTimeout(f.timers.show);m.attr("aria-hidden",Boolean(!b));m.stop(TRUE,FALSE);if(a.isFunction(p.effect)){p.effect.call(m,f.hash());m.queue(function(){i.call(this);a(this).dequeue()})}else if(p.effect===FALSE){m[n]();i.call(m)}else m.fadeTo(90,b?1:0,i);b&&p.target.trigger("qtip-"+h+"-inactive");return f},show:function(b){f.toggle(TRUE,b)},hide:function(b){f.toggle(FALSE,b)},focus:function(b){if(f.rendered===false)return FALSE;var e=
f.elements.tooltip,i=a(j),n=parseInt(e.css("z-index"),10),m=a.fn.qtip.zindex+i.length,p=k+"-focus",x=a.extend({},b);if(!e.hasClass(p)&&n!==m){i.css("z-index",function(w,u){return u-1});a(j+"."+p).each(function(){var w=a(this),u=w.qtip(),r;if(!u||u.rendered===FALSE)return TRUE;w.removeClass(p);r=a.Event("tooltipblur");r.originalEvent=x;w.trigger(r,[u,m])});b=a.Event("tooltipfocus");b.originalEvent=x;e.trigger(b,[f.hash(),m]);b.isDefaultPrevented()||e.css({zIndex:m}).addClass(p)}return f},reposition:function(b){if(f.rendered===
FALSE)return FALSE;var e=d.position.target,i=f.elements.tooltip,n=d.position,m=n.my,p=n.at,x=f.elements.tooltip.width(),w=f.elements.tooltip.height();a(n.container);var u=0,r=0,s={left:0,top:0},F=a.Event("tooltipmove"),C=i.css("position")==="fixed",A=a(E),H={left:function(D){var I=A.scrollLeft(),J=A.width(),G=p.x==="left"?u:p.x==="right"?-u:u/2,K=m.x!==p.x&&p.x!=="center"?u:0,L=G+(m.x==="left"?-x:m.x==="right"?x:x/2)+-2*n.adjust.x,Q=I-D;if(D+x-J-I>0)s.left+=(m.x==="center"?-1:1)*(L-G-K);else if(Q>
0)s.left+=L-G+K;return s.left-D},top:function(D){var I=A.scrollTop(),J=A.height(),G=p.y==="top"?r:p.y==="bottom"?-r:0,K=m.y!==p.y&&p.y!=="center"?r:0,L=G+(m.y==="top"?-w:m.y==="bottom"?w:-w/2)+-2*n.adjust.y;J=D+w-J-I;if(I-D>0)s.top+=(m.y==="center"?-1:1)*(L-G-K);else if(J>0)s.top+=L-G-K;return s.top-D}};if(e==="mouse"){p={x:"left",y:"top"};b=a.extend({},a.fn.qtip.mouse);s={top:b.pageY,left:b.pageX}}else{if(e==="event")e=b&&b.target&&b.type!=="scroll"&&b.type!=="resize"?f.cache.target=a(b.target):
f.cache.target;e=a(e).eq(0);if(e.length===0)return f;else if(e[0]===document||e[0]===E){u=e.width();r=e.height();if(e[0]===E)s={top:C?0:A.scrollTop(),left:C?0:A.scrollLeft()}}else if(e.is("area")&&a.fn.qtip.plugins.imagemap){s=a.fn.qtip.plugins.imagemap(e,p);u=s.width;r=s.height;s=s.offset}else{u=e.outerWidth();r=e.outerHeight();s=q(e)}s.left+=p.x==="right"?u:p.x==="center"?u/2:0;s.top+=p.y==="bottom"?r:p.y==="center"?r/2:0}s.left+=n.adjust.x+(m.x==="right"?-x:m.x==="center"?-x/2:0);s.top+=n.adjust.y+
...
!v.is(":visible"));e-=parseInt(o.css("border-"+k[i]+"-width"),10)||0;v.removeClass("ui-tooltip-accessible")}j.css(k[i],-e)}function h(k,j,b){if(l.tip){k=a.extend({},g.corner);j=g.mimic.adjust?a.extend({},g.mimic):NULL;var e=k.precedance==="y"?["y","top","left","height","x"]:["x","left","top","width","y"],i=b.adjusted,n=[parseInt(o.css("border-"+k[e[0]]+"-width"),10)||0,0];a.each([k,j],function(){if(i.left)this.x=this.x==="center"?i.left>0?"left":"right":this.x==="left"?"right":"left";if(i.top)this.y=
...
'<div class="ui-tooltip-tip-border" />':"");break}return g},update:function(k,j){var b=l.tip,e=t.width,i=t.height,n=B>0?0:1,m=Math.ceil(B/2+0.5),p;j||(j=k?k:g.mimic);if(!k)k=g.corner;if(j.x==="false")j.x=k.x;if(j.y==="false")j.y=k.y;b=b.children();switch(f){case "canvas":b=b.get(0).getContext("2d");b.restore();b.clearRect(0,0,3E3,3E3);for(p=O(j.string(),e,i);n<2;n++){if(n){b.save();b.translate(Math.floor((j.x==="left"?1:j.x==="right"?-1:0)*(B+1)*(j.precedance==="y"?0.5:1)),Math.floor((j.y==="top"?
1:j.y==="bottom"?-1:0)*(B+1)*(j.precedance==="x"?0.5:1)))}b.beginPath();b.moveTo(p[0][0],p[0][1]);b.lineTo(p[1][0],p[1][1]);b.lineTo(p[2][0],p[2][1]);b.closePath();b.fillStyle=y[n?"fill":"border"];b.fill()}break;case "vml":p=O(j.string(),e,i);n="m"+p[0][0]+","+p[0][1]+" l"+p[1][0]+","+p[1][1]+" "+p[2][0]+","+p[2][1]+" xe";b.attr({path:n,fillcolor:y.fill});if(B){b.children().attr("color",y.border);if(j.precedance==="y"){b.css("top",(j.y==="top"?1:-1)*(B-2));b.css("left",j.x==="left"?1:-2)}else{b.css("left",
...
a("<div />",{id:"qtip-overlay",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(E).bind("resize.qtipmodal",function(){g.overlay.css({height:Math.max(a(E).height(),a(document).height()),width:Math.max(a(E).width(),a(document).width())})}).trigger("resize")},toggle:function(l){var v=g.overlay,o=c.options.show.modal.effect,z=l?"show":"hide";v.stop(TRUE,FALSE);if(a.isFunction(o))o.call(v,l);else o===FALSE?v[z]():v.fadeTo(90,l?0.7:0,function(){l||a(this).hide()})},show:function(){h.toggle(TRUE)},
...
a('<iframe class="ui-tooltip-bgiframe" frameborder="0" tabindex="-1" src="javascript:\'\';"  style="display:block; position:absolute; z-index:-1; filter:alpha(opacity=0);"></iframe>');h.bgiframe.appendTo(g);g.bind(l,d.adjust)},adjust:function(){var v=c.get("dimensions"),o=c.plugins.tip,z=c.elements.tip,t;t=parseInt(g.css("border-left-width"),10);t={left:-t,top:-t};if(o&&z){o=o.corner.precedance==="x"?["width","left"]:["height","top"];t[o[1]]-=z[o[0]]()}h.bgiframe.css(t).css(v)},destroy:function(){d.iframe.remove();
g.unbind(l)}});d.init()}a.fn.qtip=function(c,d,h){var g=String(c).toLowerCase(),q=NULL,l=g==="disable"?[TRUE]:a.makeArray(arguments).slice(1,10),v=l[l.length-1],o=a.extend(TRUE,{},c),z;if(!arguments.length&&this.data("qtip")||g==="api")return(o=this.data("qtip"))?o.hash():M;else if("string"===typeof c){this.each(function(){var t=a(this).data("qtip");if(!t)return TRUE;if(/option|set/.test(g)&&d)if(h!==M)t.set(d,h);else q=t.get(d);else{if(!t.rendered&&(g==="show"||g==="toggle")){if(v&&v.timeStamp)t.cache.event=
v;t.render()}else if(g==="enable"){g="disable";l=[FALSE]}t[g]&&t[g].apply(t[g],l)}});return q!==NULL?q:this}else if("object"===typeof c){z=N(o,this);o=a.extend(TRUE,{},a.fn.qtip.defaults,o);return a.fn.qtip.bind.call(z,o,v)}};a.fn.qtip.bind=function(c,d){return this.each(function(h){function g(f){function k(){o.render(typeof f==="object"||z.show.ready);t.show.unbind(y.show);t.hide.unbind(y.hide)}if(o.cache.disabled)return FALSE;o.cache.event=a.extend({},f);if(z.show.delay>0){clearTimeout(o.timers.show);
o.timers.show=setTimeout(k,z.show.delay);y.show!==y.hide&&t.hide.bind(y.hide,function(){clearTimeout(o.timers.show)})}else k()}var q=a(this),l=c.id,v=c.content.text,o,z,t,y,B;c.id=l=l===FALSE||l.length<1||a("#ui-tooltip-"+l).length?a.fn.qtip.nextid++:l;B=".qtip-"+l+"-create";o=R.call(this,l,c);if(o===FALSE)return TRUE;z=o.options;if(a.isArray(v))z.content.text=v[h];q.attr("title")&&q.data("oldtitle",q.attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&
this(o)});t={show:z.show.target,hide:z.hide.target};y={show:String(z.show.event).replace(" ",B+" ")+B,hide:String(z.hide.event).replace(" ",B+" ")+B};t.show.bind(y.show,g);if(c.show.ready||c.prerender||c.show.event===FALSE)g(d)})};a.each({attr:function(c){var d=a(this).data("qtip");return arguments.length===1&&c==="title"&&d&&d.rendered===TRUE?a(this).data("oldtitle"):NULL},remove:a.ui?NULL:function(c,d){this.each(function(){if(!d)if(!c||a.filter(c,[this]).length)a("*",this).add(this).each(function(){a(this).triggerHandler("remove")})})}},
...
a.fn.qtip.plugins.ajax=function(c){var d=c.plugins.ajax,h=c.options.content.ajax;if(h&&h.url)if(d)return d;else{c.plugins.ajax=new S(c);return c.plugins.ajax}};a.fn.qtip.plugins.ajax.initialize="render";a.fn.qtip.plugins.ajax.sanitize=function(c){try{var d=c.content.ajax;if(typeof d!=="object")c.content.ajax={url:d};d.once=!!d.once}catch(h){}};a.extend(TRUE,a.fn.qtip.defaults,{content:{ajax:{once:TRUE}}});a.fn.qtip.plugins.tip=function(c){var d=c.plugins.tip,h=c.options.style.tip;if(h&&h.corner)if(d)return d;
else{c.plugins.tip=new T(c);c.plugins.tip.init();return c.plugins.tip}};a.fn.qtip.plugins.tip.initialize="render";a.fn.qtip.plugins.tip.sanitize=function(c){try{var d=c.style.tip;if(typeof d!=="object")c.style.tip={corner:!!d};if(typeof d.method!=="string")d.method=TRUE;if(!/canvas|polygon/i.test(d.method))d.method=TRUE;if(typeof d.width!=="number")d.width=12;if(typeof d.height!=="number")d.height=12;if(typeof d.border!=="number")d.border=0}catch(h){}};a.extend(TRUE,a.fn.qtip.defaults,{style:{tip:{corner:TRUE,
method:TRUE,width:12,height:12,border:0,offset:0}}});a.fn.qtip.plugins.imagemap=function(c,d){function h(y,B){for(var f=0,k=1,j=1,b=0,e=0,i=y.width,n=y.height;i>0&&n>0&&k>0&&j>0;){i=Math.floor(i/2);n=Math.floor(n/2);if(d.x==="left")k=i;else if(d.x==="right")k=y.width-i;else k+=Math.floor(i/2);if(d.y==="top")j=n;else if(d.y==="bottom")j=y.height-n;else j+=Math.floor(n/2);for(f=B.length;f--;){if(B.length<2)break;b=B[f][0]-y.offset.left;e=B[f][1]-y.offset.top;if(d.x==="left"&&b>=k||d.x==="right"&&b<=
k||d.x==="center"&&(b<k||b>y.width-k)||d.y==="top"&&e>=j||d.y==="bottom"&&e<=j||d.y==="center"&&(e<j||e>y.height-j))B.splice(f,1)}}return{left:B[0][0],top:B[0][1]}}var g=c.attr("shape").toLowerCase(),q=c.attr("coords").split(","),l=[],v=a('img[usemap="#'+c.parent("map").attr("name")+'"]').offset(),o={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},z=0,t=0;if(g==="poly")for(z=q.length;z--;){t=[parseInt(q[--z],10),parseInt(q[z+1],10)];if(t[0]>o.offset.right)o.offset.right=t[0];if(t[0]<
o.offset.left)o.offset.left=t[0];if(t[1]>o.offset.bottom)o.offset.bottom=t[1];if(t[1]<o.offset.top)o.offset.top=t[1];l.push(t)}else l=a.map(q,function(y){return parseInt(y,10)});switch(g){case "rect":o={width:Math.abs(l[2]-l[0]),height:Math.abs(l[3]-l[1]),offset:{left:l[0],top:l[1]}};break;case "circle":o={width:l[2]+2,height:l[2]+2,offset:{left:l[0],top:l[1]}};break;case "poly":a.extend(o,{width:Math.abs(o.offset.right-o.offset.left),height:Math.abs(o.offset.bottom-o.offset.top)});o.offset=d.string()===
"centercenter"?{left:o.offset.left+o.width/2,top:o.offset.top+o.height/2}:h(o,l.slice());o.width=o.height=0;break}o.offset.left+=v.left;o.offset.top+=v.top;return o};a.fn.qtip.plugins.modal=function(c){var d=c.plugins.modal,h=c.options.show.modal;if(d)return d;else if(h&&h.on===TRUE){c.plugins.modal=new U(c,h);return c.plugins.modal}};a.fn.qtip.plugins.modal.initialize="render";a.fn.qtip.plugins.modal.sanitize=function(c){if(c.show)if(typeof c.show.modal!=="object")c.show.modal={on:!!c.show.modal};
else if(typeof c.show.modal.on==="undefined")c.show.modal.on=TRUE};a.extend(TRUE,a.fn.qtip.defaults,{show:{modal:{on:FALSE,effect:TRUE,blur:TRUE}}});a.fn.qtip.plugins.bgiframe=function(c){if(!(a.browser.msie&&/^6\.[0-9]/.test(a.browser.version)&&a("select, object").length))return FALSE;var d=c.plugins.bgiframe;if(d)return d;else{c.plugins.bgiframe=new V(c);return c.plugins.bgiframe}};a.fn.qtip.plugins.bgiframe.initialize="render"})(jQuery,window);
