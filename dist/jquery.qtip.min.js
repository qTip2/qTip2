...
y:"top"};b=j.adjust.mouse||!b||!b.pageX?a.extend({},a.fn.qtip.mouse):b;A={top:b.pageY,left:b.pageX}}else{if(e==="event")e=b&&b.target&&b.type!=="scroll"&&b.type!=="resize"?f.cache.target=a(b.target):f.cache.target;e=a(e).eq(0);if(e.length===0)return f;else if(e[0]===document||e[0]===G){p=e.width();y=e.height();if(e[0]===G)A={top:D?0:v.scrollTop,left:D?0:v.scrollLeft}}else if(e.is("area")&&a.fn.qtip.plugins.imagemap){A=a.fn.qtip.plugins.imagemap(e,m);p=A.width;y=A.height;A=A.offset}else{p=e.outerWidth();
y=e.outerHeight();A=o(e)}A.left+=m.x==="right"?p:m.x==="center"?p/2:0;A.top+=m.y==="bottom"?y:m.y==="center"?y/2:0}A.left+=j.adjust.x+(i.x==="right"?-r:i.x==="center"?-r/2:0);A.top+=j.adjust.y+(i.y==="bottom"?-x:i.y==="center"?-x/2:0);A.adjusted=j.adjust.screen&&e[0]!==G&&e[0]!==document.body?{left:w.left(A.left),top:w.top(A.top)}:{left:0,top:0};k.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(C+"-pos-"+i.abbreviation());E.originalEvent=a.extend({},
b);k.trigger(E,[f.hash(),A,v.elem]);if(E.isDefaultPrevented())return f;delete A.adjusted;if(j.container[0]==document.body){if(A.top+v.scrollTop<1)A.top=0;if(A.left+v.scrollLeft<1)A.left=0}if(k.is(":visible")&&a.isFunction(j.effect)){j.effect.call(k,f.hash(),A);k.queue(function(){var F=a(this);F.css({opacity:"",height:""});a.browser.msie&&this.style&&this.style.removeAttribute("filter");F.dequeue()})}else isNaN(A.left,A.top)||k.css(A);return f},disable:function(b){var e=f.elements.tooltip;if(f.rendered)e.toggleClass("ui-state-disabled",
