...
if(f==="mouse"){q={x:"left",y:"top"};c=x.mouse||!c||!c.pageX?a.extend({},a.fn.qtip.mouse):c;y={top:c.pageY,left:c.pageX}}else{if(f==="event")f=c&&c.target&&c.type!=="scroll"&&c.type!=="resize"?e.cache.target=a(c.target):e.cache.target;f=a(f).eq(0);if(f.length===0)return e;else if(f[0]===document||f[0]===F){r=f.width();E=f.height();if(f[0]===F)y={top:C?0:w.scrollTop,left:C?0:w.scrollLeft}}else if(f.is("area")&&a.fn.qtip.plugins.imagemap){y=a.fn.qtip.plugins.imagemap(f,q);r=y.width;E=y.height;y=y.offset}else{r=
...
c);i.trigger(I,[e.hash(),y,w.elem]);if(I.isDefaultPrevented())return e;delete y.adjusted;if(i.is(":visible")&&a.isFunction(l.effect)){l.effect.call(i,e.hash(),y);i.queue(function(){var G=a(this);G.css({opacity:"",height:""});a.browser.msie&&this.style&&this.style.removeAttribute("filter");G.dequeue()})}else isNaN(y.left,y.top)||i.css(y);return e},redraw:function(){if(!e.rendered||!(a.browser.msie&&parseInt(a.browser.version.charAt(0),10)<9))return FALSE;var c=e.elements.tooltip;c.attr("style");var f;
c.css({width:"auto",height:"auto"});f=m("dimensions",1);a.each(["width","height"],function(i,l){var g=parseInt(c.css("max-"+l),10)||0,q=parseInt(c.css("min-"+l),10)||0;f[l]=g+q?Math.min(Math.max(f[l],q),g):f[l]});c.css(f)},disable:function(c){var f=e.elements.tooltip;if(e.rendered)f.toggleClass("ui-state-disabled",c);else e.cache.disabled=!!c;return e},destroy:function(){var c=e.elements,f=c.target.data("oldtitle");e.rendered&&a.each(e.plugins,function(){this.initialize==="render"&&this.destroy()});
z(1,1,1,1);d.removeData("qtip");e.rendered&&c.tooltip.remove();f&&d.attr("title",f);d.removeAttr("aria-describedby");return d},hash:function(){var c=a.extend({},e);delete c.cache;delete c.timers;delete c.options;delete c.plugins;delete c.render;delete c.hash;return c}})}function Q(d,b){var k,h,p=a(this);k=a(document.body);var m=this===document?k:p;h=p.metadata?p.metadata(b.metadata):NULL;var s=a.extend(TRUE,{},a.fn.qtip.defaults,b,M((h&&b.metadata.type==="html5"?h[b.metadata.name]:NULL)||h));p.removeData("metadata");
h=s.position;if("boolean"===typeof s.content.text)if(s.content.attr!==FALSE&&p.attr(s.content.attr))s.content.text=p.attr(s.content.attr);else return FALSE;if(h.container===FALSE)h.container=k;if(h.target===FALSE)h.target=m;if(s.show.target===FALSE)s.show.target=m;if(s.hide.target===FALSE)s.hide.target=m;h.at=new a.fn.qtip.plugins.Corner(h.at);h.my=new a.fn.qtip.plugins.Corner(h.my);if(p.data("qtip"))if(s.overwrite)p.qtip("destroy");else if(s.overwrite===FALSE)return FALSE;p.attr("title")&&p.data("oldtitle",
p.attr("title")).removeAttr("title");k=new P(p,s,d);p.data("qtip",k);return k}function R(d){var b=this;b.checks={"^content.ajax":function(){this.plugins.ajax.load(this.options.content.ajax)}};a.extend(b,{init:function(){var k=d.options.content.ajax;d.elements.tooltip.bind("tooltiprender.ajax",function(){b.load(k);d.elements.tooltip.bind("tooltipshow.ajax",function(){k.once===FALSE&&d.rendered===TRUE&&b.load(k)})})},load:function(k){a.ajax(a.extend(TRUE,{},k,{success:function(h,p){if(a.isFunction(k.success))if(k.success.call(d.hash(),
h,p)===FALSE)return;d.set("content.text",h)},error:function(h,p,m){var s=p||m;if(a.isFunction(k.error)){h=k.error.call(d.hash(),h,p,m);if(h===FALSE)return}d.set("content.text",s)}}));return b},destroy:function(){d.elements.tooltip.unbind("tooltipshow.ajax")}});b.init()}function O(d,b,k){var h=Math.floor(b/2),p=Math.floor(k/2);b={bottomright:[[0,0],[b,k],[b,0]],bottomleft:[[0,0],[b,0],[0,k]],topright:[[0,k],[b,0],[b,k]],topleft:[[0,0],[0,k],[b,k]],topcenter:[[0,k],[h,0],[b,k]],bottomcenter:[[0,0],
...
g.precedance=j.precedance;if(g.x==="inherit")g.x=j.x;else if(g.y==="inherit")g.y=j.y;else if(g.x===g.y)g[j.precedance]=j[j.precedance]}x=Math[/b|r/.test(g[g.precedance==="y"?"x":"y"])?"ceil":"floor"];n=n.children();switch(e){case "canvas":n=n.get(0).getContext("2d");n.restore&&n.restore();n.clearRect(0,0,3E3,3E3);for(q=O(g.string(),c,f);i<2;i++){if(i){n.save();n.translate(x((g.x==="left"?1:g.x==="right"?-1:0)*(z+1)*(g.precedance==="y"?0.5:1)),x((g.y==="top"?1:g.y==="bottom"?-1:0)*(z+1)*(g.precedance===
...
if(!arguments.length&&this.data("qtip")||h==="api")return(v=this.data("qtip"))?v.hash():L;else if("string"===typeof d){this.each(function(){var o=a(this).data("qtip");if(!o)return TRUE;if(/option|set/.test(h)&&b)if(k!==L)o.set(b,k);else p=o.get(b);else{if(!o.rendered&&(h==="show"||h==="toggle")){if(s&&s.timeStamp)o.cache.event=s;o.render(1)}else if(h==="enable"){h="disable";m=[FALSE]}o[h]&&o[h].apply(o[h],m)}});return p!==NULL?p:this}else if("object"===typeof d||!arguments.length){v=M(a.extend(TRUE,
...
-1?"y":"x";this.string=function(){return this.precedance==="y"?this.y+this.x:this.x+this.y};this.abbreviation=function(){var b=this.x.substr(0,1),k=this.y.substr(0,1);return b===k?b:b==="c"||b!=="c"&&k!=="c"?k+b:b+k}},iOS:parseFloat((/CPU.+OS ([0-9_]{3}).*AppleWebkit.*Mobile/i.exec(navigator.userAgent)||[0,"4_2"])[1].replace("_","."))<4.1};a.fn.qtip.defaults={prerender:FALSE,id:FALSE,overwrite:TRUE,metadata:{type:"class"},content:{text:TRUE,attr:"title",title:{text:FALSE,button:FALSE}},position:{my:"top left",
at:"bottom right",target:FALSE,container:FALSE,adjust:{x:0,y:0,mouse:TRUE,screen:FALSE,resize:TRUE},effect:TRUE},show:{target:FALSE,event:"mouseenter",effect:TRUE,delay:90,solo:FALSE,ready:FALSE},hide:{target:FALSE,event:"mouseleave",effect:TRUE,delay:0,fixed:FALSE,inactive:FALSE},style:{classes:"",widget:FALSE},events:{render:NULL,move:NULL,show:NULL,hide:NULL,focus:NULL,blur:NULL}};a.fn.qtip.plugins.ajax=function(d){var b=d.plugins.ajax,k=d.options.content.ajax;if(k&&k.url)if(b)return b;else{d.plugins.ajax=
new R(d);return d.plugins.ajax}};a.fn.qtip.plugins.ajax.initialize="render";a.fn.qtip.plugins.ajax.sanitize=function(d){var b=d.content;if(b&&"ajax"in b){b=b.ajax;if(typeof b!=="object")b=d.content.ajax={url:b};if("boolean"!==typeof b.once&&b.once)b.once=!!b.once}};a.extend(TRUE,a.fn.qtip.defaults,{content:{ajax:{once:TRUE}}});a.fn.qtip.plugins.tip=function(d){var b=d.plugins.tip,k=d.options.style.tip;if(k&&k.corner)if(b)return b;else{d.plugins.tip=new S(d);d.plugins.tip.init();return d.plugins.tip}};
a.fn.qtip.plugins.tip.initialize="render";a.fn.qtip.plugins.tip.sanitize=function(d){var b=d.style;if(b&&"tip"in b){b=d.style.tip;if(typeof b!=="object")d.style.tip={corner:b};if(!/string|boolean/i.test(typeof b.corner))b.corner=true;if(typeof b.method!=="string")b.method=TRUE;if(!/canvas|polygon/i.test(b.method))b.method=TRUE;typeof b.width!=="number"&&delete b.width;typeof b.height!=="number"&&delete b.height;typeof b.border!=="number"&&delete b.border;typeof b.offset!=="number"&&delete b.offset}};
a.extend(TRUE,a.fn.qtip.defaults,{style:{tip:{corner:TRUE,mimic:FALSE,method:TRUE,width:9,height:9,border:0,offset:0}}});a.fn.qtip.plugins.imagemap=function(d,b){function k(z,e){for(var j=0,n=1,c=1,f=0,i=0,l=z.width,g=z.height;l>0&&g>0&&n>0&&c>0;){l=Math.floor(l/2);g=Math.floor(g/2);if(b.x==="left")n=l;else if(b.x==="right")n=z.width-l;else n+=Math.floor(l/2);if(b.y==="top")c=g;else if(b.y==="bottom")c=z.height-g;else c+=Math.floor(g/2);for(j=e.length;j--;){if(e.length<2)break;f=e[j][0]-z.offset.left;
i=e[j][1]-z.offset.top;if(b.x==="left"&&f>=n||b.x==="right"&&f<=n||b.x==="center"&&(f<n||f>z.width-n)||b.y==="top"&&i>=c||b.y==="bottom"&&i<=c||b.y==="center"&&(i<c||i>z.height-c))e.splice(j,1)}}return{left:e[0][0],top:e[0][1]}}var h=d.attr("shape").toLowerCase(),p=d.attr("coords").split(","),m=[],s=a('img[usemap="#'+d.parent("map").attr("name")+'"]'),v=s.offset(),o={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},A=0,B=0;v.left+=Math.ceil((s.outerWidth()-s.width())/2);v.top+=Math.ceil((s.outerHeight()-
s.height())/2);if(h==="poly")for(A=p.length;A--;){B=[parseInt(p[--A],10),parseInt(p[A+1],10)];if(B[0]>o.offset.right)o.offset.right=B[0];if(B[0]<o.offset.left)o.offset.left=B[0];if(B[1]>o.offset.bottom)o.offset.bottom=B[1];if(B[1]<o.offset.top)o.offset.top=B[1];m.push(B)}else m=a.map(p,function(z){return parseInt(z,10)});switch(h){case "rect":o={width:Math.abs(m[2]-m[0]),height:Math.abs(m[3]-m[1]),offset:{left:m[0],top:m[1]}};break;case "circle":o={width:m[2]+2,height:m[2]+2,offset:{left:m[0],top:m[1]}};
break;case "poly":a.extend(o,{width:Math.abs(o.offset.right-o.offset.left),height:Math.abs(o.offset.bottom-o.offset.top)});o.offset=b.string()==="centercenter"?{left:o.offset.left+o.width/2,top:o.offset.top+o.height/2}:k(o,m.slice());o.width=o.height=0;break}o.offset.left+=v.left;o.offset.top+=v.top;return o};a.fn.qtip.plugins.modal=function(d){var b=d.plugins.modal,k=d.options.show.modal;if(b)return b;else if(k&&k.on===TRUE){d.plugins.modal=new T(d,k);return d.plugins.modal}};a.fn.qtip.plugins.modal.initialize=
"render";a.fn.qtip.plugins.modal.sanitize=function(d){if(d.show)if(typeof d.show.modal!=="object")d.show.modal={on:!!d.show.modal};else if(typeof d.show.modal.on==="undefined")d.show.modal.on=TRUE};a.extend(TRUE,a.fn.qtip.defaults,{show:{modal:{on:FALSE,effect:TRUE,blur:TRUE}}});a.fn.qtip.plugins.bgiframe=function(d){if(!(a.browser.msie&&/^6\.[0-9]/.test(a.browser.version)&&a("select, object").length))return FALSE;var b=d.plugins.bgiframe;if(b)return b;else{d.plugins.bgiframe=new U(d);return d.plugins.bgiframe}};
