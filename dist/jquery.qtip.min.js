...
if(h.container===l)h.container=j;if(h.target===l)h.target=v;if(s.show.target===l)s.show.target=v;if(s.hide.target===l)s.hide.target=v;if(s.position.viewport===t)s.position.viewport=h.container;h.at=new d.fn.qtip.plugins.Corner(h.at);h.my=new d.fn.qtip.plugins.Corner(h.my);if(n.data("qtip"))if(s.overwrite)n.qtip("destroy");else if(s.overwrite===l)return l;n.attr("title")&&n.data("oldtitle",n.attr("title")).removeAttr("title");q=new X(n,s,e);n.data("qtip",q);n.bind("remove.qtip",function(){q.destroy()});
return q}function Z(e){var a=this,q=e.elements.tooltip,h=e.options.content.ajax;a.checks={"^content.ajax":function(n,j){if(j==="once")a.once();else h&&h.url&&a.load()}};d.extend(a,{init:function(){if(h&&h.url){a.load();a.once()}},once:function(){h.once?a.destroy():q.bind("tooltipshow.ajax",function(){a.load()})},load:function(){d.ajax(d.extend({success:function(n){e.set("content.text",n)},error:function(n,j,v){e.set("content.text",j+": "+v)},context:e},h));return a},destroy:function(){q.unbind(".ajax")}});
