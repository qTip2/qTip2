...
width:d[q?1:0]}}var r=this,h=c.options.style.tip,l=c.elements,x=l.tooltip,o={top:0,left:0,corner:""},A={width:h.width,height:h.height},z={},B=h.border||0,P=a("<canvas />")[0].getContext;r.corner=E;r.mimic=E;a.fn.qtip.checks.tip={"^position.my|style.tip.(corner|mimic|border)$":function(){r.init()||r.destroy();c.reposition()},"^style.tip.(height|width)$":function(){A={width:h.width,height:h.height};r.create();r.update();c.reposition()},"^content.title.text|style.(classes|widget)$":function(){l.tip&&
r.update()}};a.extend(r,{init:function(){var m=r.detectCorner()&&(P||a.browser.msie);if(m){r.create();r.update();x.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",b)}return m},detectCorner:function(){var m=h.corner,q=c.options.position,d=q.at;q=q.my.string?q.my.string():q.my;if(m===p||q===p&&d===p)return p;else if(m===y)r.corner=new a.fn.qtip.plugins.Corner(q);else if(!m.string){r.corner=new a.fn.qtip.plugins.Corner(m);r.corner.fixed=y}return r.corner.string()!=="centercenter"},detectColours:function(){var m=
