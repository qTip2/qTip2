...
M;if("boolean"!==typeof b)b=!(g.hasClass(f)||d.cache.disabled);if(d.rendered)g.toggleClass(f,b);else d.cache.disabled=!!b;return d},destroy:function(){var b=e.data("oldtitle");if(d.rendered){g.remove();c.each(d.plugins,function(){this.initialize==="render"&&this.destroy()})}clearTimeout(d.timers.show);clearTimeout(d.timers.hide);k(1,1,1,1);e.removeData("qtip");b&&e.attr("title",b);e.removeAttr("aria-describedby");return e}})}function Z(e,a){var p,h,q=c(this),j=c(document.body),w=this===document?j:
q;h=q.metadata?q.metadata(a.metadata):E;var s=c.extend(u,{},c.fn.qtip.defaults,a,U((h&&a.metadata.type==="html5"?h[a.metadata.name]:E)||h));q.removeData("metadata");h=s.position;if("boolean"===typeof s.content.text)if(s.content.attr!==m&&q.attr(s.content.attr))s.content.text=q.attr(s.content.attr);else return m;if(h.container===m)h.container=j;if(h.target===m)h.target=w;if(s.show.target===m)s.show.target=w;if(s.hide.target===m)s.hide.target=w;if(s.position.viewport===u)s.position.viewport=h.container;
h.at=new c.fn.qtip.plugins.Corner(h.at);h.my=new c.fn.qtip.plugins.Corner(h.my);if(q.data("qtip"))if(s.overwrite)q.qtip("destroy");else if(s.overwrite===m)return m;q.attr("title")&&q.data("oldtitle",q.attr("title")).removeAttr("title");p=new Y(q,s,e);q.data("qtip",p);q.bind("remove.qtip",function(){p.destroy()});return p}function $(e){var a=this,p=e.elements.tooltip,h=e.options.content.ajax;a.checks={"^content.ajax":function(q,j){if(j==="once")a.once();else h&&h.url&&a.load()}};c.extend(a,{init:function(){if(h&&
h.url){a.load();a.once()}},once:function(){h.once?a.destroy():p.bind("tooltipshow.ajax",function(){a.load()})},load:function(){c.ajax(c.extend({success:function(q){e.set("content.text",q)},error:function(q,j,w){e.set("content.text",j+": "+w)},context:e},h));return a},destroy:function(){p.unbind(".ajax")}});a.init()}function X(e,a,p){var h=Math.ceil(a/2),q=Math.ceil(p/2);a={bottomright:[[0,0],[a,p],[a,0]],bottomleft:[[0,0],[a,0],[0,p]],topright:[[0,p],[a,0],[a,p]],topleft:[[0,0],[0,p],[a,p]],topcenter:[[0,
p],[h,0],[a,p]],bottomcenter:[[0,0],[a,0],[h,p]],rightcenter:[[0,0],[a,q],[0,p]],leftcenter:[[a,0],[a,p],[0,q]]};a.lefttop=a.bottomright;a.righttop=a.bottomleft;a.leftbottom=a.topright;a.rightbottom=a.topleft;return a[e.string()]}function aa(e){function a(k,d,y){if(j.tip){k=c.extend({},h.corner);d=y.adjusted;var g;if(h.corner.fixed!==u){if(d.left)k.x=k.x==="center"?d.left>0?"left":"right":k.x==="left"?"right":"left";if(d.top)k.y=k.y==="center"?d.top>0?"top":"bottom":k.y==="top"?"bottom":"top";if(k.string()!==
s.corner&&(s.top!==d.top||s.left!==d.left))g=h.update(k)}g||(g=h.position(k,0));if(g.right!==P)g.left=g.right;if(g.bottom!==P)g.top=g.bottom;g.option=Math.max(0,q.offset);y.left-=g.left.charAt?g.option:(g.right?-1:1)*g.left;y.top-=g.top.charAt?g.option:(g.bottom?-1:1)*g.top;s.left=d.left;s.top=d.top;s.corner=k.string()}}function p(k,d,y){d=!d?k[k.precedance]:d;d="border-"+d+"-width";k=parseInt((j.titlebar&&k.y==="top"?j.titlebar:j.content).css(d),10);return(y?k||parseInt(w.css(d),10):k)||0}var h=
this,q=e.options.style.tip,j=e.elements,w=j.tooltip,s={top:0,left:0,corner:""},r={width:q.width,height:q.height},z={},B=q.border||0,G=q.method||m;h.corner=E;h.mimic=E;h.checks={"^position.my|style.tip.(corner|mimic|method|border)$":function(){h.init()||h.destroy();e.reposition()},"^style.tip.(height|width)$":function(){r={width:q.width,height:q.height};h.create();h.update();e.reposition()},"^content.title.text|style.(classes|widget)$":function(){j.tip&&h.update()}};c.extend(h,{init:function(){var k=
h.detectCorner();if(k){if(G!=="polygon")G=c("<canvas />")[0].getContext?"canvas":c.browser.msie?"vml":"polygon";h.create();h.update();w.unbind(".qtip-tip").bind("tooltipmove.qtip-tip",a)}return k},detectCorner:function(){var k=q.corner,d=e.options.position,y=d.at;d=d.my.string?d.my.string():d.my;if(k===m||d===m&&y===m)return m;else if(k===u)h.corner=new c.fn.qtip.plugins.Corner(d);else if(!k.string){h.corner=new c.fn.qtip.plugins.Corner(k);h.corner.fixed=u}return h.corner.string()!=="centercenter"},
detectColours:function(){var k=j.tip.css({backgroundColor:"",border:""}),d=h.corner,y=d[d.precedance],g="border-"+y+"-color";y="border"+y.charAt(0)+y.substr(1)+"Color";var b=/rgba?\(0, 0, 0(, 0)?\)|transparent/i;d=j.titlebar&&(d.y==="top"||d.y==="center"&&k.position().top+r.height/2+q.offset<j.titlebar.outerHeight(1))?j.titlebar:j.content;z.fill=k.css("background-color")||"transparent";z.border=k[0].style[y];if(!z.fill||b.test(z.fill)){z.fill=d.css("background-color");if(b.test(z.fill))z.fill=w.css("background-color")}if(!z.border||
b.test(z.border)){z.border=w.css(g);if(b.test(z.border)||z.border===c(document.body).css("color"))z.border=d.css(g)||z.fill}c("*",k).add(k).css("background-color","transparent").css("border",0)},create:function(){var k=r.width,d=r.height;j.tip&&j.tip.remove();j.tip=c("<div />",{"class":"ui-tooltip-tip"}).css(r).prependTo(w);switch(G){case "canvas":c('<canvas height="'+d+'" width="'+k+'" />').appendTo(j.tip)[0].getContext("2d").save();break;case "vml":k='<vml:shape coordorigin="0,0" coordsize="'+k+
" "+d+'" stroked="false"  style="behavior:url(#default#VML); display:inline-block; position:absolute; antialias:false; left: 0; top: 0; width:'+k+"px; height:"+d+'px;"></vml:shape>';j.tip.html(B?k+=k:k);break;case "polygon":j.tip.append('<div class="ui-tooltip-tip-inner" />').append(B?'<div class="ui-tooltip-tip-border" />':"");break}},update:function(k){var d=j.tip.children(),y=r.width,g=r.height,b=q.mimic,f,i,l,n,o;if(!k)k=h.corner;if(b===m)b=k;else{b=new c.fn.qtip.plugins.Corner(b);b.precedance=
k.precedance;if(b.x==="inherit")b.x=k.x;else if(b.y==="inherit")b.y=k.y;else if(b.x===b.y)b[k.precedance]=k[k.precedance]}B=q.border===u?p(k,E,u):q.border;f=B>0?0:1;n=b.string().indexOf("center")>-1;o=Math[/b|r/.test(b[b.precedance==="y"?"x":"y"])?"ceil":"floor"];k=h.position(k,1);h.detectColours();switch(G){case "canvas":i=d[0].getContext("2d");i.restore&&i.restore();i.clearRect(0,0,3E3,3E3);for(l=X(b,y,g);f<2;f++){if(f){i.save();i.translate(o((b.x==="left"?1:b.x==="right"?-1:0)*(B+1)*(b.precedance===
"y"?0.5:1)),o((b.y==="top"?1:b.y==="bottom"?-1:0)*(B+1)*(b.precedance==="x"?0.5:1)))}i.beginPath();i.moveTo(l[0][0],l[0][1]);i.lineTo(l[1][0],l[1][1]);i.lineTo(l[2][0],l[2][1]);i.closePath();i.fillStyle=z[f?"fill":"border"];i.fill()}break;case "vml":l=X(b,y,g);l="m"+l[0][0]+","+l[0][1]+" l"+l[1][0]+","+l[1][1]+" "+l[2][0]+","+l[2][1]+" xe";d.each(function(C){c(this).attr({path:l,fillcolor:z[C||!B?"fill":"border"]}).css("antialias",""+n)});break;case "polygon":d.removeAttr("style").css({position:"absolute",
