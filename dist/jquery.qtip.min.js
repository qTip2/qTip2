...
q=parseInt(k.css("max-width"),10)||0;s=parseInt(k.css("min-width"),10)||0;j=Math.min(Math.max(j,s),q);k.width(j)}e.rendered===TRUE&&e.reposition(e.cache.event)}}var h;if(!e.rendered||!b)return FALSE;if(a.isFunction(b))b=b.call(c);b.jquery&&b.length>0?e.elements.content.append(b.css({display:"block"})):e.elements.content.html(b);h=a("img",e.elements.content).each(function(){a(this).load(f);var j=this.src;this.src="";this.src=j}).length;h===0&&f();return e}function r(b,f,h,j){function k(B){if(o.tooltip.hasClass("ui-state-disabled"))return FALSE;
