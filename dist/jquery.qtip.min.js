...
[];d.each(function(){var l=h.call(this);if(l){c.content.text.push(l);i=i.add(a(this))}})}else i=d;a.each(a.fn.qtip.plugins,function(){this.sanitize&&this.sanitize(c)});return d?i:c}function O(c,d,h){function i(b){var f,k=b.split("."),n=d[k[0]];for(b=1;b<k.length;b+=1){f=n[k[b]];if(typeof f==="object"&&!f.jquery&&!f.precedance)n=n[k[b]];else break}return k[b]!==K?[n,k[b]]:[d,k[0]]}function q(b){b=b[0];var f={left:0,top:0},k=!d.position.adjust.container;if(b.offsetParent){do{f.left+=b.offsetLeft;f.top+=
b.offsetTop}while(b=k?b.offsetParent:0)}return f}function l(b,f){var k=e.elements.tooltip,n=j+"-accessible "+(f?j+"-accessible-fluid":""),g=!k.is(":visible"),p=FALSE;if(!e.rendered)return FALSE;g&&k.addClass(n);switch(b){case "dimensions":p={height:k.outerHeight(),width:k.outerWidth()};break;case "position":p=q(k);break}g&&k.removeClass(n);return p}function s(){var b=e.elements;if(b.title){b.titlebar.remove();b.titlebar=b.title=b.button=NULL;b.tooltip.removeAttr("aria-labelledby")}}function u(){var b=
...
e.hide();return FALSE}).bind("mousedown keydown mouseup keyup mouseout",function(k){a(this).toggleClass("ui-state-active ui-state-focus",/down$/i.test(k.type))});e.redraw()}function r(){var b=e.elements;b.titlebar&&s();b.titlebar=a("<div />",{"class":j+"-titlebar "+(d.style.widget?"ui-widget-header":"")}).append(b.title=a("<div />",{id:j+"-"+h+"-title","class":j+"-title",html:d.content.title.text})).prependTo(b.wrapper);if(d.content.title.button)u();else e.rendered===TRUE&&e.redraw()}function t(b){var f=
...
TRUE){g.appendTo(f);e.reposition()}},"^(show|hide).(event|target|fixed|delay|inactive)":function(o,A,F,C){var D=b.search(/fixed/i)>-1?[0,[0,1,1,1]]:[b.substr(0,3),b.charAt(0)==="s"?[1,0,0,0]:[0,1,0,0]];if(D[0])o[A]=C;w.apply(e,D[1]);if(D[0])o[A]=F;x.apply(e,D[1])},"^show.ready$":function(){e.rendered===FALSE&&e.show()},"^style.classes$":function(){e.elements.tooltip.attr("class",j+" qtip ui-helper-reset "+f)},"^style.widget$":function(){var o=!!f;g.toggleClass("ui-widget",o);n.titlebar.toggleClass("ui-widget-header",
o);n.content.toggleClass("ui-widget-content",o);n.button.children("span").toggleClass(j+"-icon",!o).toggleClass("ui-icon",o)},"^events.(render|show|move|hide|focus|blur)":function(o,A,F){n.tooltip[(a.isFunction(f)?"":"un")+"bind"]("tooltip"+A,F)}}};a.each(e.plugins,function(o){if("object"===typeof this.checks)v[o]=this.checks});p=k[0][k[1]];k[0][k[1]]=f.nodeType?a(f):f;L(d,c);for(y in v)for(z in v[y])RegExp(z,"i").test(b)&&v[y][z].call(e,k[0],k[1],f,p);return e},toggle:function(b,f){function k(){var v=
a(this),o=b?"attr":"removeAttr",A=/^1|0$/.test(v.css("opacity"));e.elements.title&&c[o]("aria-labelledby",j+"-"+h+"-title");c[o]("aria-describedby",j+"-"+h+"-content");if(b){if(a.browser.msie&&this.style&&A){z=this.style;z.removeAttribute("filter");z.removeAttribute("opacity")}}else A&&v.hide()}if(e.rendered===FALSE)return FALSE;var n=b?"show":"hide",g=e.elements.tooltip,p=d[n],y=g.is(":visible"),z;if((typeof b).search("boolean|number"))b=!g.is(":visible");if(!y&&!b||g.is(":animated"))return e;if(f){if(e.cache.event&&
/over|enter/.test(f.type)&&/out|leave/.test(e.cache.event.type)&&a(f.target).add(d.show.target).length<2&&a(f.relatedTarget).parents(m).length>0)return e;e.cache.event=a.extend({},f)}y=a.Event("tooltip"+n);y.originalEvent=a.extend({},f);g.trigger(y,[e.hash(),90]);if(y.isDefaultPrevented())return e;if(b){e.focus();e.reposition(f);p.solo&&a(m).qtip("hide")}else clearTimeout(e.timers.show);g.attr("aria-hidden",Boolean(!b));g.stop(1,1);if(a.isFunction(p.effect)){p.effect.call(g,e.hash());g.queue(function(){k.call(this);
a(this).dequeue()})}else if(p.effect===FALSE){g[n]();k.call(g)}else g.fadeTo(90,b?1:0,k);b&&p.target.trigger("qtip-"+h+"-inactive");return e},show:function(b){e.toggle(TRUE,b)},hide:function(b){e.toggle(FALSE,b)},focus:function(b){if(e.rendered===false)return FALSE;var f=e.elements.tooltip,k=a(m),n=parseInt(f.css("z-index"),10),g=a.fn.qtip.zindex+k.length,p=j+"-focus",y=a.extend({},b);if(!f.hasClass(p)&&n!==g){k.css("z-index",function(z,v){return v-1});a(m+"."+p).each(function(){var z=a(this),v=z.qtip(),
...
-v/2:0);B.adjusted=y.screen&&f[0]!==G&&f[0]!==document.body?{left:o.left(B.left),top:o.top(B.top)}:{left:0,top:0};k.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(j+"-pos-"+g.abbreviation());C.originalEvent=a.extend({},b);k.trigger(C,[e.hash(),B,E.elem]);if(C.isDefaultPrevented())return e;delete B.adjusted;if(k.is(":visible")&&a.isFunction(n.effect)){n.effect.call(k,e.hash(),B);k.queue(function(){var H=a(this);H.css({opacity:"",height:""});a.browser.msie&&
this.style&&this.style.removeAttribute("filter");H.dequeue()})}else isNaN(B.left,B.top)||k.css(B);return e},redraw:function(){if(!e.rendered||!(a.browser.msie&&parseInt(a.browser.version.charAt(0),10)<9))return FALSE;var b=e.elements.tooltip;b.attr("style");var f;b.css({width:"auto",height:"auto"});f=l("dimensions",1);a.each(["width","height"],function(k,n){var g=parseInt(b.css("max-"+n),10)||0,p=parseInt(b.css("min-"+n),10)||0;f[n]=g+p?Math.min(Math.max(f[n],p),g):f[n]});b.css(f)},disable:function(b){var f=
...
2)break;f=e[j][0]-w.offset.left;k=e[j][1]-w.offset.top;if(d.x==="left"&&f>=m||d.x==="right"&&f<=m||d.x==="center"&&(f<m||f>w.width-m)||d.y==="top"&&k>=b||d.y==="bottom"&&k<=b||d.y==="center"&&(k<b||k>w.height-b))e.splice(j,1)}}return{left:e[0][0],top:e[0][1]}}var i=c.attr("shape").toLowerCase(),q=c.attr("coords").split(","),l=[],s=a('img[usemap="#'+c.parent("map").attr("name")+'"]'),u=s.offset(),r={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},t=0,x=0;u.left+=Math.ceil((s.outerWidth()-
s.width())/2);u.top+=Math.ceil((s.outerHeight()-s.height())/2);if(i==="poly")for(t=q.length;t--;){x=[parseInt(q[--t],10),parseInt(q[t+1],10)];if(x[0]>r.offset.right)r.offset.right=x[0];if(x[0]<r.offset.left)r.offset.left=x[0];if(x[1]>r.offset.bottom)r.offset.bottom=x[1];if(x[1]<r.offset.top)r.offset.top=x[1];l.push(x)}else l=a.map(q,function(w){return parseInt(w,10)});switch(i){case "rect":r={width:Math.abs(l[2]-l[0]),height:Math.abs(l[3]-l[1]),offset:{left:l[0],top:l[1]}};break;case "circle":r={width:l[2]+
2,height:l[2]+2,offset:{left:l[0],top:l[1]}};break;case "poly":a.extend(r,{width:Math.abs(r.offset.right-r.offset.left),height:Math.abs(r.offset.bottom-r.offset.top)});r.offset=d.string()==="centercenter"?{left:r.offset.left+r.width/2,top:r.offset.top+r.height/2}:h(r,l.slice());r.width=r.height=0;break}r.offset.left+=u.left;r.offset.top+=u.top;return r};a.fn.qtip.plugins.modal=function(c){var d=c.plugins.modal,h=c.options.show.modal;if(d)return d;else if(h&&h.on===TRUE){c.plugins.modal=new S(c,h);
