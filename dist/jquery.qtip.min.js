...
typeof d.hide.inactive){o.show.bind("qtip-"+h+"-inactive",w);a.each(a.fn.qtip.inactiveEvents,function(y,A){o.hide.add(e.elements.tooltip).bind(A+v+"-inactive",w)})}a.each(z.hide,function(y,A){var C=a.inArray(A,z.show);if(C>-1&&a(o.hide).add(o.show).length===a(o.hide).length||A==="unfocus"){o.show.bind(A+v,function(K){o.tooltip.is(":visible")?l(K):j(K)});delete z.show[C]}else o.hide.bind(A+v,l)})}if(f){a.each(z.show,function(y,A){o.show.bind(A+v,j)});o.tooltip.bind("mouseover"+v,function(){e.focus()})}if(k){if(d.position.adjust.resize||
...
new a.fn.qtip.plugins.Corner(c)},"^position.(my|at|adjust|target)":function(){e.rendered&&e.reposition()},"^(show|hide).(event|target|fixed)":function(t,v,o,z){var r=f.search(/fixed/i)>-1?[0,[0,1,1,1]]:f.search(/hide/i)<0?["show",[1,0,0,0]]:["hide",[0,1,0,0]];if(r[0])t[v]=z;p.apply(e,r[1]);if(r[0])t[v]=o;x.apply(e,r[1])}}};a.each(e.plugins,function(t){if("object"===typeof this.checks)w[t]=this.checks});k=g[0][g[1]];g[0][g[1]]=c;N(d,b);for(j in w)for(l in w[j])RegExp(l,"i").test(f)&&w[j][l].call(e,
g[0],g[1],c,k);return e},toggle:function(f,c){function g(){var v=f?"attr":"removeAttr",o=/^1|0$/.test(a(this).css("opacity"));e.elements.title&&b[v]("aria-labelledby",u+"-"+h+"-title");b[v]("aria-describedby",u+"-"+h+"-content");if(f){if(a.browser.msie&&a(this).get(0).style&&o){t=a(this).get(0).style;t.removeAttribute("filter");t.removeAttribute("opacity")}}else o&&a(this).hide()}if(e.rendered===FALSE)return FALSE;var k=f?"show":"hide",j=e.elements.tooltip,l=d[k],w=j.is(":visible"),t;if((typeof f).search("boolean|number"))f=
!j.is(":visible");if(!w&&!f||j.is(":animated"))return e;if(c){if(e.cache.event&&/over|enter/.test(c.type)&&/out|leave/.test(e.cache.event.type)&&a(c.target).add(d.show.target).length<2&&a(c.relatedTarget).parents(B).length>0)return e;e.cache.event=a.extend({},c)}w=a.Event("tooltip"+k);w.originalEvent=a.extend({},c);j.trigger(w,[e.hash(),90]);if(w.isDefaultPrevented())return e;if(f){e.focus();e.reposition(c);l.solo&&a(B).qtip("hide")}else clearTimeout(e.timers.show);j.attr("aria-hidden",Boolean(!f));
j.stop(TRUE,FALSE);if(a.isFunction(l.effect)){l.effect.call(j,e.hash());j.queue(function(){g.call(this);a(this).dequeue()})}else if(l.effect===FALSE){j[k]();g.call(j)}else j.fadeTo(90,f?1:0,g);f&&l.target.trigger("qtip-"+h+"-inactive");return e},show:function(f){e.toggle(TRUE,f)},hide:function(f){e.toggle(FALSE,f)},focus:function(f){if(!e.rendered)return FALSE;var c=e.elements.tooltip,g=a(B),k=parseInt(c.css("z-index"),10),j=a.fn.qtip.zindex+g.length,l=u+"-focus",w=a.extend({},f);if(!c.hasClass(l)&&
k!==j){g.each(function(){var t=a(this).qtip(),v=a.Event("tooltipblur"),o,z;if(!t||t.rendered===FALSE)return TRUE;o=t.elements.tooltip;z=parseInt(o.css("z-index"),10);isNaN(z)||o.css({zIndex:z-1});o.removeClass(l);v.originalEvent=w;o.trigger(v,[t,j])});f=a.Event("tooltipfocus");f.originalEvent=w;c.trigger(f,[e.hash(),j]);f.isDefaultPrevented()||c.css({zIndex:j}).addClass(l)}return e},reposition:function(f){if(e.rendered===FALSE)return FALSE;var c=d.position.target,g=e.elements.tooltip,k=d.position,
j=k.my,l=k.at,w=e.elements.tooltip.width(),t=e.elements.tooltip.height(),v=a(k.container)[0],o=0,z=0,r={left:0,top:0},y=a.Event("tooltipmove"),A=g.css("position")==="fixed",C=a(D),K={left:function(E){var H=c==="mouse"?f.pageX:c.offset().left,I=C.scrollLeft(),G=C.width(),L=l.x==="left"?o:l.x==="right"?-o:o/2,J=j.x!==l.x&&l.x!=="center"?o:0,O=L+(j.x==="left"?-w:j.x==="right"?w:w/2)+-2*k.adjust.x;G=E+w-G-I;if(I-E>0&&!(E>=H&&E<H+o))r.left+=O-L+J;else if(G>0&&E+w>H)r.left+=(j.x==="center"?-1:1)*(O-L-J);
return r.left-E},top:function(E){var H=C.scrollTop(),I=C.height(),G=l.y==="top"?z:l.y==="bottom"?-z:0,L=j.y!==l.y&&l.y!=="center"?z:0,J=G+(j.y==="top"?-t:j.y==="bottom"?t:-t/2)+-2*k.adjust.y;I=E+t-I-H;if(H-E>0)r.top+=j.y==="center"?-J+G:J;else if(I>0)r.top+=J-G-L;return r.top-E}};if(c==="mouse"){l={x:"left",y:"top"};f=a.extend({},a.fn.qtip.mouse);r={top:f.pageY,left:f.pageX}}else{if(c==="event")c=f&&f.target&&f.type!=="scroll"&&f.type!=="resize"?e.cache.target=a(f.target):e.cache.target;if(c[0]===
document||c[0]===D){o=c.width();z=c.height();if(c[0]===D)r={top:A?0:C.scrollTop(),left:A?0:C.scrollLeft()}}else if(c.is("area")&&a.fn.qtip.plugins.imagemap){r=a.fn.qtip.plugins.imagemap(c,l);o=r.width;z=r.height;r=r.offset}else{o=c.outerWidth();z=c.outerHeight();r=c.offset();if(k.adjust.offset){do{r.left-=v.offsetLeft-v.scrollLeft;r.top-=v.offsetTop-v.scrollTop}while(v=v.offsetParent)}}r.left+=l.x==="right"?o:l.x==="center"?o/2:0;r.top+=l.y==="bottom"?z:l.y==="center"?z/2:0}r.left+=k.adjust.x+(j.x===
"right"?-w:j.x==="center"?-w/2:0);r.top+=k.adjust.y+(j.y==="bottom"?-t:j.y==="center"?-t/2:0);r.adjusted=k.adjust.screen&&c[0]!==D&&c[0]!==document.body?{left:K.left(r.left),top:K.top(r.top)}:{left:0,top:0};if(k.container[0]==document.body){if(r.top+C.scrollTop()<1)r.top=0;if(r.left+C.scrollLeft()<1)r.left=0}g.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(u+"-pos-"+j.abbreviation());y.originalEvent=a.extend({},f);g.trigger(y,[e.hash(),r]);if(y.isDefaultPrevented())return e;
delete r.adjust;if(g.is(":visible")&&a.isFunction(k.effect)){k.effect.call(g,r);g.queue(function(){a(this).css({opacity:"",height:""});a.browser.msie&&a(this).get(0).style&&a(this).get(0).style.removeAttribute("filter");a(this).dequeue()})}else isNaN(r.left,r.top)||g.css(r);return e},disable:function(f){var c=e.elements.tooltip;if(e.rendered)c.toggleClass("ui-state-disabled",f);else e.cache.disabled=!!f;return e},destroy:function(){var f=e.elements,c=f.target.data("oldtitle");e.rendered&&a.each(e.plugins,
function(){this.initialize==="render"&&this.destroy()});p(1,1,1,1);b.removeData("qtip");e.rendered&&f.tooltip.remove();c&&b.attr("title",c);b.removeAttr("aria-describedby");return b},hash:function(){var f=a.extend({},e);delete f.cache;delete f.timers;delete f.options;delete f.plugins;delete f.render;delete f.hash;return f}})}function R(b,d){var h;h=a(this).metadata?a(this).metadata(d.metadata):{};h=a.extend(TRUE,{},d,h);var i=h.position,q=a(this)[0]===document?a(document.body):a(this);if("boolean"===
typeof h.content.text)if(h.content.attr!==FALSE&&a(this).attr(h.content.attr))h.content.text=a(this).attr(h.content.attr);else return FALSE;if(i.container===FALSE)i.container=a(document.body);if(i.target===FALSE)i.target=q;if(h.show.target===FALSE)h.show.target=q;if(h.hide.target===FALSE)h.hide.target=q;i.at=new a.fn.qtip.plugins.Corner(i.at);i.my=new a.fn.qtip.plugins.Corner(i.my);if(a(this).data("qtip"))if(h.overwrite)a(this).qtip("destroy");else if(h.overwrite===FALSE)return FALSE;h=new Q(a(this),
h,b);a(this).data("qtip",h);return h}function S(b){var d=this;d.checks={"^content.ajax":function(){this.plugins.ajax.load(this.options.content.ajax)}};a.extend(d,{init:function(){var h=b.options.content.ajax;b.elements.tooltip.bind("tooltiprender.ajax",function(){d.load(h);b.elements.tooltip.bind("tooltipshow.ajax",function(){h.once===FALSE&&b.rendered===TRUE&&d.load(h)})})},load:function(h){a.ajax(a.extend(TRUE,{},h,{success:function(i,q){if(a.isFunction(h.success))if(h.success.call(b.hash(),i,q)===
FALSE)return;b.set("content.text",i);M.remove()},error:function(i,q,m){var s=q||m;if(a.isFunction(h.error)){i=h.error.call(b.hash(),i,q,m);if(i===FALSE)return}b.set("content.text",s)}}));return d},destroy:function(){b.elements.tooltip.unbind("tooltipshow.ajax")}});d.init()}function P(b,d,h){var i=Math.floor(d/2),q=Math.floor(h/2);d={bottomright:[[0,0],[d,h],[d,0]],bottomleft:[[0,0],[d,0],[0,h]],topright:[[0,h],[d,0],[d,h]],topleft:[[0,0],[0,h],[d,h]],topcenter:[[0,h],[i,0],[d,h]],bottomcenter:[[0,
0],[d,0],[i,h]],rightcenter:[[0,0],[d,q],[0,h]],leftcenter:[[d,0],[d,h],[0,q]]};d.lefttop=d.bottomright;d.righttop=d.bottomleft;d.leftbottom=d.topright;d.rightbottom=d.topleft;return d[b]}function T(b){function d(c){var g=m.tip,k=["left","right"],j={left:0,right:0,top:0,bottom:0},l=0;if(q.corner===FALSE||!g)return FALSE;c=c||i.corner;g.css({top:"",bottom:"",left:"",right:"",margin:""});k[c.precedance==="y"?"push":"unshift"]("top","bottom");if(a.browser.msie)j={top:c.precedance==="y"?0:0,left:0,bottom:c.precedance===
"y"?0:0,right:0};switch(c[c.precedance==="y"?"x":"y"]){case "center":g.css(k[0],"50%").css("margin-"+k[0],-(p[c.precedance==="y"?"width":"height"]/2));break;case k[0]:g.css(k[0],j[k[0]]+B);break;case k[1]:g.css(k[1],j[k[1]]+B);break}l=p[c.precedance==="x"?"width":"height"];if(u){s.toggleClass("ui-tooltip-accessible",!s.is(":visible"));l-=parseInt(n.css("border-"+c[c.precedance]+"-width"),10)||0;s.removeClass("ui-tooltip-accessible")}g.css(c[c.precedance],-1*j[c[c.precedance]]-l)}function h(c,g,k){if(m.tip){c=
a.extend({},i.corner);g=i.mimic.adjust?a.extend({},i.mimic):NULL;var j=c.precedance==="y"?["y","top","left","height"]:["x","left","top","width"],l=k.adjusted,w=parseInt(n.css("border-"+c[j[0]]+"-width"),10)||0;a.each([c,g],function(){if(l.left)this.x=this.x==="center"?l.left>0?"left":"right":this.x==="left"?"right":"left";if(l.top)this.y=this.y==="center"?l.top>0?"top":"bottom":this.y==="top"?"bottom":"top"});k[j[1]]+=(c[j[0]]===j[1]?1:-1)*(p[j[3]]-w);k[j[2]]-=B;if(c.string()!==x.corner.string()&&
(x.top!==l.top||x.left!==l.left))i.update(c,g);x.left=l.left;x.top=l.top;x.corner=c}}var i=this,q=b.options.style.tip,m=b.elements,s=m.tooltip,n=m.wrapper,x={top:0,left:0,corner:{string:function(){}}},p={width:q.width,height:q.height},e={},u=q.border||0,B=q.adjust||0,f=q.method||FALSE;i.corner=NULL;i.mimic=NULL;i.checks={"^position.my|style.tip.(corner|mimic|method|border)":function(){u=q.border;if(i.detectCorner()){i.create();i.detectColours();i.update()}else i.tip.remove();this.get("position.target")!==
...
"transparent").css("border",0)},create:function(){var c=p.width,g=p.height;m.tip&&m.tip.remove();m.tip=a('<div class="ui-tooltip-tip" />').toggleClass("ui-widget-content",b.options.style.widget).css(p).prependTo(s);switch(f){case "canvas":m.tip.append('<canvas height="'+g+'" width="'+c+'" />');break;case "vml":m.tip.html('<vml:shape coordorigin="0 0" coordsize="'+c+" "+g+'" stroked="'+!!u+'"  style="behavior:url(#default#VML); display:inline-block; antialias:TRUE; position: absolute;  top:0; left:0; width:'+
c+"px; height:"+g+"px; vertical-align:"+i.corner.y+';"><vml:stroke weight="'+(u-2)+'px" joinstyle="miter" miterlimit="10"  style="behavior:url(#default#VML); display:inline-block;" /></vml:shape>');break;case "polygon":m.tip.append('<div class="ui-tooltip-tip-inner" />').append(u?'<div class="ui-tooltip-tip-border" />':"");break}return i},update:function(c,g){var k=m.tip,j=p.width,l=p.height,w=u>0?0:1,t=Math.ceil(u/2+0.5),v;g||(g=c?c:i.mimic);if(!c)c=i.corner;if(g.x==="false")g.x=c.x;if(g.y==="false")g.y=
c.y;k=k.children();switch(f){case "canvas":k=k.get(0).getContext("2d");k.restore();k.clearRect(0,0,3E3,3E3);for(v=P(g.string(),j,l);w<2;w++){if(w){k.save();k.translate(Math.floor((g.x==="left"?1:g.x==="right"?-1:0)*(u+1)*(g.precedance==="y"?0.5:1)),Math.floor((g.y==="top"?1:g.y==="bottom"?-1:0)*(u+1)*(g.precedance==="x"?0.5:1)))}k.beginPath();k.moveTo(v[0][0],v[0][1]);k.lineTo(v[1][0],v[1][1]);k.lineTo(v[2][0],v[2][1]);k.closePath();k.fillStyle=e[w?"fill":"border"];k.fill()}break;case "vml":v=P(g.string(),
j,l);w="m"+v[0][0]+","+v[0][1]+" l"+v[1][0]+","+v[1][1]+" "+v[2][0]+","+v[2][1]+" xe";k.attr({path:w,fillcolor:e.fill});if(u){k.children().attr("color",e.border);if(g.precedance==="y"){k.css("top",(g.y==="top"?1:-1)*(u-2));k.css("left",g.x==="left"?1:-2)}else{k.css("left",(g.x==="left"?1:-1)*(u-2));k.css("top",g.y==="top"?1:-2)}}break;case "polygon":if(g.precedance==="y"){w=j>l?1.5:j<l?5:2.2;t=[g.x==="left"?t:g.x==="right"?-t:0,Math.floor(w*t*(g.y==="bottom"?-1:1)*(g.x==="center"?0.8:1))]}else{w=
j<l?1.5:j>l?5:2.2;t=[Math.floor(w*t*(g.x==="right"?-1:1)*(g.y==="center"?0.9:1)),g.y==="top"?t:g.y==="bottom"?-t:0]}k.removeAttr("style").each(function(o){var z={x:g.precedance==="x"?g.x==="left"?"right":"left":g.x,y:g.precedance==="y"?g.y==="top"?"bottom":"top":g.y},r=g.x==="center"?["left","right",z.y,l,j]:["top","bottom",z.x,j,l],y=e[!o&&u?"border":"fill"];o&&a(this).css({position:"absolute","z-index":1,left:t[0],top:t[1]});g.x==="center"||g.y==="center"?a(this).css("border-"+r[2],r[3]+"px solid "+
y).css("border-"+r[0],Math.floor(r[4]/2)+"px dashed transparent").css("border-"+r[1],Math.floor(r[4]/2)+"px dashed transparent"):a(this).css("border-width",Math.floor(l/2)+"px "+Math.floor(j/2)+"px").css("border-"+z.x,Math.floor(j/2)+"px solid "+y).css("border-"+z.y,Math.floor(l/2)+"px solid "+y)});break}d(c);return i},destroy:function(){m.tip&&m.tip.remove();s.unbind("tooltipmove.tip")}})}function U(b,d){var h=this,i=b.elements,q=i.tooltip;i.overlay=a("#qtip-overlay");a.extend(h,{init:function(){d=
a.extend(TRUE,a.fn.qtip.plugins.modal.defaults,d);q.bind("tooltipshow.qtipmodal tooltiphide.qtipmodal",function(m,s,n){m=m.type.replace("tooltip","");a.isFunction(d[m])?d[m].call(i.overlay,n,s):h[m](n)});i.overlay.length||h.create();d.blur===TRUE&&i.overlay.bind("click.qtipmodal"+b.id,function(){b.hide.call(b)})},create:function(){i.overlay=a("<div />",{id:"qtip-overlay",css:{position:"absolute",top:0,left:0,display:"none"}}).appendTo(document.body);a(D).bind("resize.qtipmodal",function(){i.overlay.css({height:Math.max(a(D).height(),
a(document).height()),width:Math.max(a(D).width(),a(document).width())})}).trigger("resize")},toggle:function(m){var s=i.overlay,n=b.options.show.modal.effect,x=m?"show":"hide";s.stop(TRUE,FALSE);if(a.isFunction(n))n.call(s,m);else n===FALSE?s[x]():s.fadeTo(90,m?0.7:0,function(){m||a(this).hide()})},show:function(){h.toggle(TRUE)},hide:function(){h.toggle(FALSE)},destroy:function(){var m=TRUE;a("*").each(function(){var s=a(this).data("qtip");if(s&&s.id!==b.id&&s.options.show.modal)return m=FALSE});
...
h.bgiframe.appendTo(i);i.bind(m,d.adjust)},adjust:function(){var s=b.calculate("dimensions"),n=b.plugins.tip,x=b.elements.tip,p;p=parseInt(i.css("border-left-width"),10);p={left:-p,top:-p};if(n&&x){n=n.corner.precedance==="x"?["width","left"]:["height","top"];p[n[1]]-=x[n[0]]()}h.bgiframe.css(p).css(s)},destroy:function(){d.iframe.remove();i.unbind(m)}});d.init()}a.fn.qtip=function(b,d,h){var i=String(b).toLowerCase(),q=FALSE,m=i==="disable"?[TRUE]:a.makeArray(arguments).slice(1,10),s=m[m.length-
1],n=a.extend(TRUE,{},b),x;if(!arguments.length&&a(this).data("qtip")||i==="api")return(n=a(this).eq(0).data("qtip"))?n.hash():F;else if("string"===typeof b){a(this).each(function(){var p=a(this).data("qtip");if(!p)return TRUE;if(/option|set/.test(i)&&d)if(h!==F)p.set(d,h);else q=p.get(d);else{if(!p.rendered&&(i==="show"||i==="toggle")){if(s&&s.timeStamp)p.cache.event=s;p.render()}else if(i==="enable"){i="disable";m=[FALSE]}p[i]&&p[i].apply(p[i],m)}});return q!==FALSE?q:a(this)}else if("object"===
typeof b){x=N(n,this);n=a.extend(TRUE,{},a.fn.qtip.defaults,n);return a.fn.qtip.bind.call(x,n,s)}};a.fn.qtip.bind=function(b,d){return a(this).each(function(h){function i(u){function B(){m.render(typeof u==="object"||s.show.ready);n.show.unbind(x.show);n.hide.unbind(x.hide)}if(m.cache.disabled)return FALSE;m.cache.event=a.extend({},u);if(s.show.delay>0){clearTimeout(m.timers.show);m.timers.show=setTimeout(B,s.show.delay);x.show!==x.hide&&n.hide.bind(x.hide,function(){clearTimeout(m.timers.show)})}else B()}
var q,m,s,n,x,p,e=b.content.text;q=b.id===FALSE||b.id.length<1||a("#ui-tooltip-"+b.id).length?a.fn.qtip.nextid++:b.id;p=".qtip-"+q+"-create";m=R.call(a(this),q,b);if(m===FALSE)return TRUE;s=m.options;if(a.isArray(e))s.content.text=e[h];a(this).attr("title")&&a(this).data("oldtitle",a(this).attr("title")).removeAttr("title");a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(m)});n={show:s.show.target,hide:s.hide.target};x={show:String(s.show.event).replace(" ",p+" ")+p,hide:String(s.hide.event).replace(" ",
...
"object")b.style.tip={corner:!!b.style.tip};if(typeof b.style.tip.method!=="string")b.style.tip.method=TRUE;if(!/canvas|polygon/i.test(b.style.tip.method))b.style.tip.method=TRUE;if(typeof b.style.tip.width!=="number")b.style.tip.width=12;if(typeof b.style.tip.height!=="number")b.style.tip.height=12;if(typeof b.style.tip.border!=="number")b.style.tip.border=0};a.fn.qtip.plugins.imagemap=function(b,d){function h(e,u){for(var B=0,f=1,c=1,g=0,k=0,j=e.width,l=e.height;j>0&&l>0&&f>0&&c>0;){j=Math.floor(j/
2);l=Math.floor(l/2);if(d.x==="left")f=j;else if(d.x==="right")f=e.width-j;else f+=Math.floor(j/2);if(d.y==="top")c=l;else if(d.y==="bottom")c=e.height-l;else c+=Math.floor(l/2);for(B=u.length;B--;){if(u.length<2)break;g=u[B][0]-e.offset.left;k=u[B][1]-e.offset.top;if(d.x==="left"&&g>=f||d.x==="right"&&g<=f||d.x==="center"&&(g<f||g>e.width-f)||d.y==="top"&&k>=c||d.y==="bottom"&&k<=c||d.y==="center"&&(k<c||k>e.height-c))u.splice(B,1)}}return{left:u[0][0],top:u[0][1]}}var i=b.attr("shape").toLowerCase(),
q=b.attr("coords").split(","),m=[],s=a('img[usemap="#'+b.parent("map").attr("name")+'"]').offset(),n={width:0,height:0,offset:{top:1E10,right:0,bottom:0,left:1E10}},x=0,p=0;if(i==="poly")for(x=q.length;x--;){p=[parseInt(q[--x],10),parseInt(q[x+1],10)];if(p[0]>n.offset.right)n.offset.right=p[0];if(p[0]<n.offset.left)n.offset.left=p[0];if(p[1]>n.offset.bottom)n.offset.bottom=p[1];if(p[1]<n.offset.top)n.offset.top=p[1];m.push(p)}else m=a.map(q,function(e){return parseInt(e,10)});switch(i){case "rect":n=
{width:Math.abs(m[2]-m[0]),height:Math.abs(m[3]-m[1]),offset:{left:m[0],top:m[1]}};break;case "circle":n={width:m[2]+2,height:m[2]+2,offset:{left:m[0],top:m[1]}};break;case "poly":a.extend(n,{width:Math.abs(n.offset.right-n.offset.left),height:Math.abs(n.offset.bottom-n.offset.top)});n.offset=d.string()==="centercenter"?{left:n.offset.left+n.width/2,top:n.offset.top+n.height/2}:h(n,m.slice());n.width=n.height=0;break}n.offset.left+=s.left;n.offset.top+=s.top;return n};a.fn.qtip.plugins.modal=function(b){var d=
