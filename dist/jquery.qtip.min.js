...
if(i&&b.hide.fixed){r.hide=r.hide.add(r.tooltip);r.tooltip.bind("mouseover"+w,function(){r.tooltip.hasClass("ui-state-disabled")||clearTimeout(e.timers.hide)})}if(f){if("number"===typeof b.hide.inactive){r.show.bind("qtip-"+k+"-inactive",A);a.each(a.fn.qtip.inactiveEvents,function(v,u){r.hide.add(e.elements.tooltip).bind(u+w+"-inactive",A)})}a.each(F.hide,function(v,u){var I=a.inArray(u,F.show);if(I>-1&&a(r.hide).add(r.show).length===a(r.hide).length||u==="unfocus"){r.show.bind(u+w,function(D){r.tooltip.is(":visible")?
...
r,F,J){var v=d.search(/fixed/i)>-1?[0,[0,1,1,1]]:[d.substr(0,3),d.charAt(0)==="s"?[1,0,0,0]:[0,1,0,0]];if(v[0])t[r]=J;y.apply(e,v[1]);if(v[0])t[r]=F;B.apply(e,v[1])},"^show.ready$":function(){e.rendered===FALSE&&e.show()},"^style.classes$":function(){e.elements.tooltip.attr("class",j+" qtip ui-helper-reset "+f)},"^style.widget$":function(){var t=!!f;g.toggleClass("ui-widget",t);l.titlebar.toggleClass("ui-widget-header",t);l.content.toggleClass("ui-widget-content",t);l.button.children("span").toggleClass(j+
"-icon",!t).toggleClass("ui-icon",t)},"^events.(render|show|move|hide|focus|blur)":function(t,r,F){l.tooltip[(a.isFunction(f)?"":"un")+"bind"]("tooltip"+r,F)}}};a.each(e.plugins,function(t){if("object"===typeof this.checks)w[t]=this.checks});q=i[0][i[1]];i[0][i[1]]=f.nodeType?a(f):f;M(b);for(A in w)for(C in w[A])RegExp(C,"i").test(d)&&w[A][C].call(e,i[0],i[1],f,q);return e},toggle:function(d,f){function i(){var C=a(this),w=d?"attr":"removeAttr",t=/^1|0$/.test(C.css("opacity")),r=this.style;e.elements.title&&
...
d.originalEvent=A;f.trigger(d,[e,g]);if(!d.isDefaultPrevented())f.addClass(q)[0].style.zIndex=g}return e},reposition:function(d){if(e.rendered===FALSE)return FALSE;var f=b.position.target,i=e.elements.tooltip,l=b.position,g=l.my,q=l.at,A=l.adjust,C=e.elements.tooltip.width(),w=e.elements.tooltip.height(),t=0,r=0,F=a.Event("tooltipmove"),J=i.css("position")==="fixed",v=l.container[0]!==document.body?l.container:a(E),u={left:0,top:0,adjusted:{left:0,top:0}},I={left:function(D){var G=v.scrollLeft,H=
q.x==="left"?t:q.x==="right"?-t:-t/2,K=G-D;G=D+C-v.width-G;H=(g.x==="left"?C:g.x==="right"?-C:-C/2)-(g.precedance==="x"||g.x===g.y?H:0);if(K>0&&(g.x!=="left"||G>0))u.left-=H;else if(G>0&&(g.x!=="right"||K>0))u.left-=(g.x==="center"?-1:1)*H+2*A.x;if(u.left<0)u.left=D;return u.left-D},top:function(D){var G=v.scrollTop,H=q.y==="top"?r:q.y==="bottom"?-r:-r/2,K=G-D;G=D+w-v.height-G;H=(g.y==="top"?w:g.y==="bottom"?-w:-w/2)-(g.precedance==="y"||g.x===g.y?H:0);if(K>0&&(g.y!=="top"||G>0))u.top-=H;else if(G>
0&&(g.y!=="bottom"||K>0))u.top-=(g.y==="center"?-1:1)*H+2*A.y;if(u.top<0)u.top=D;return u.top-D}};v={elem:v,height:v[(v[0]===E?"h":"outerH")+"eight"](),width:v[(v[0]===E?"w":"outerW")+"idth"](),scrollLeft:v.scrollLeft(),scrollTop:v.scrollTop()};if(f==="mouse"){q={x:"left",y:"top"};d=d&&(d.type==="resize"||d.type==="scroll")?e.cache.event:A.mouse||!d||!d.pageX?a.extend({},a.fn.qtip.mouse):d;u={top:d.pageY,left:d.pageX}}else{if(f==="event")f=d&&d.target&&d.type!=="scroll"&&d.type!=="resize"?e.cache.target=
a(d.target):e.cache.target;f=a(f).eq(0);if(f.length===0)return e;else if(f[0]===document||f[0]===E){t=f.width();r=f.height();if(f[0]===E)u={top:J?0:v.scrollTop,left:J?0:v.scrollLeft}}else if(f.is("area")&&a.fn.qtip.plugins.imagemap){u=a.fn.qtip.plugins.imagemap(f,q);t=u.width;r=u.height;u=u.offset}else{t=f.outerWidth();r=f.outerHeight();u=p(f[0],l.container[0])}u.left+=q.x==="right"?t:q.x==="center"?t/2:0;u.top+=q.y==="bottom"?r:q.y==="center"?r/2:0}u.left+=A.x+(g.x==="right"?-C:g.x==="center"?-C/
2:0);u.top+=A.y+(g.y==="bottom"?-w:g.y==="center"?-w/2:0);if(A.screen&&f[0]!==E&&f[0]!==document.body)u.adjusted={left:I.left(u.left),top:I.top(u.top)};i.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(j+"-pos-"+g.abbreviation());F.originalEvent=a.extend({},d);i.trigger(F,[e,u,v.elem]);if(F.isDefaultPrevented())return e;delete u.adjusted;if(i.is(":visible")&&a.isFunction(l.effect)){l.effect.call(i,e,u);i.queue(function(){var D=a(this);D.css({opacity:"",
...
j+" "+n+'" stroked="'+!!y+'"  style="behavior:url(#default#VML); display:inline-block; antialias:TRUE; position: absolute;  top:0; left:0; width:'+j+"px; height:"+n+"px; vertical-align:"+h.corner.y+';"><vml:stroke weight="'+(y-2)+'px" joinstyle="miter" miterlimit="10"  style="behavior:url(#default#VML); display:inline-block;" /></vml:shape>');break;case "polygon":m.tip.append('<div class="ui-tooltip-tip-inner" />').append(y?'<div class="ui-tooltip-tip-border" />':"");break}return h},update:function(j){var n=
...
g.x==="left"?"right":"left":g.x,y:g.precedance==="y"?g.y==="top"?"bottom":"top":g.y},t=g.x==="center"?["left","right",w.y,f,d]:["top","bottom",w.x,d,f],r=B[!C&&y?"border":"fill"];C&&a(this).css({position:"absolute","z-index":1,left:l[0],top:l[1]});g.x==="center"||g.y==="center"?a(this).css("border-"+t[2],t[3]+"px solid "+r).css("border-"+t[0],Math.floor(t[4]/2)+"px dashed transparent").css("border-"+t[1],Math.floor(t[4]/2)+"px dashed transparent"):a(this).css("border-width",Math.floor(f/2)+"px "+
...
x.corner.precedance==="x"?["width","left"]:["height","top"];z[x[1]]-=o[x[0]]()}k.bgiframe.css(z).css(s)},destroy:function(){b.iframe.remove();h.unbind(m)}});b.init()}a.fn.qtip=function(c,b,k){var h=String(c).toLowerCase(),p=NULL,m=h==="disable"?[TRUE]:a.makeArray(arguments).slice(1,10),s=m[m.length-1],x;if(!arguments.length&&this.data("qtip")||h==="api")return(x=this.data("qtip"))?x:L;else if("string"===typeof c){this.each(function(){var o=a(this).data("qtip");if(!o)return TRUE;if(/option|set/.test(h)&&
b)if(k!==L)o.set(b,k);else p=o.get(b);else{if(!o.rendered&&(h==="show"||h==="toggle")){if(s&&s.timeStamp)o.cache.event=s;o.render(1)}else if(h==="enable"){h="disable";m=[FALSE]}o[h]&&o[h].apply(o[h],m)}});return p!==NULL?p:this}else if("object"===typeof c||!arguments.length){x=M(a.extend(TRUE,{},c));return a.fn.qtip.bind.call(this,x,s)}};a.fn.qtip.bind=function(c,b){return this.each(function(){function k(z){function B(){o.render(typeof z==="object"||h.show.ready);p.show.unbind(m.show);p.hide.unbind(m.hide)}
if(o.cache.disabled)return FALSE;o.cache.event=a.extend({},z);if(h.show.delay>0){clearTimeout(o.timers.show);o.timers.show=setTimeout(B,h.show.delay);m.show!==m.hide&&p.hide.bind(m.hide,function(){clearTimeout(o.timers.show)})}else B()}var h,p,m,s=c.id=!c.id||c.id===FALSE||c.id.length<1||a("#ui-tooltip-"+c.id).length?a.fn.qtip.nextid++:c.id,x=".qtip-"+s+"-create",o=P.call(this,s,c);if(o===FALSE)return TRUE;h=o.options;a.each(a.fn.qtip.plugins,function(){this.initialize==="initialize"&&this(o)});p=
