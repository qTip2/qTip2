...
r,E){l.tooltip[(a.isFunction(f)?"":"un")+"bind"]("tooltip"+r,E)}}};a.each(e.plugins,function(t){if("object"===typeof this.checks)u[t]=this.checks});q=i[0][i[1]];i[0][i[1]]=f.nodeType?a(f):f;M(b);for(x in u)for(D in u[x])RegExp(D,"i").test(c)&&u[x][D].call(e,i[0],i[1],f,q);return e},toggle:function(c,f){function i(){var D=a(this),u=c?"attr":"removeAttr",t=/^1|0$/.test(D.css("opacity")),r=this.style;e.elements.title&&d[u]("aria-labelledby",j+"-"+k+"-title");d[u]("aria-describedby",j+"-"+k+"-content");
if(c){if(a.browser.msie&&r&&t){r.removeAttribute("filter");r.removeAttribute("opacity")}}else t&&D.hide()}if(e.rendered===FALSE)return FALSE;var l=c?"show":"hide",g=e.elements.tooltip,q=b[l],x=g.is(":visible");if((typeof c).search("boolean|number"))c=!g.is(":visible");if(!x&&!c||g.is(":animated"))return e;if(f){if(/over|enter/.test(f.type)&&/out|leave/.test(e.cache.event.type)&&f.target===b.show.target[0]&&g.has(f.relatedTarget).length)return e;e.cache.event=a.extend({},f)}x=a.Event("tooltip"+l);
