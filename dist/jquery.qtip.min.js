...
p.show.unbind(e.show+j+"-create")}var f=this,z="ui-tooltip",B=".qtip."+z;f.id=i;f.rendered=FALSE;f.elements={target:c};f.cache={event:{},target:NULL,disabled:FALSE};f.timers={};f.options=d;f.plugins={};a.extend(f,{render:function(b){var e=f.elements;if(f.rendered)return FALSE;f.rendered=b?-2:-1;e.tooltip=a("<div/>").attr({id:z+"-"+i,role:"tooltip","class":z+" qtip ui-tooltip-accessible ui-helper-reset "+d.style.classes}).css("z-index",a.fn.qtip.zindex+a(B).length).toggleClass("ui-widget",d.style.widget).toggleClass("ui-state-disabled",
...
z+" qtip ui-helper-reset "+e)},"^style.widget$":function(){j.toggleClass("ui-widget",!!e);k.titlebar.toggleClass("ui-widget-header",!!e);k.content.toggleClass("ui-widget-content",!!e)},"^events.(render|show|move|hide|focus|blur)":function(n,u,s,A){a.isFunction(e)?k.tooltip.bind("tooltip"+u,s):k.tooltip.unbind("tooltip"+u,A)}}};a.each(f.plugins,function(n){if("object"===typeof this.checks)y[n]=this.checks});p=h[0][h[1]];h[0][h[1]]=e;O(d,c);for(v in y)for(r in y[v])RegExp(r,"i").test(b)&&y[v][r].call(f,
h[0],h[1],e,p);return f},toggle:function(b,e){function h(){var y=a(this),n=b?"attr":"removeAttr",u=/^1|0$/.test(y.css("opacity"));f.elements.title&&c[n]("aria-labelledby",z+"-"+i+"-title");c[n]("aria-describedby",z+"-"+i+"-content");if(b){if(a.browser.msie&&this.style&&u){r=this.style;r.removeAttribute("filter");r.removeAttribute("opacity")}}else u&&y.hide()}if(f.rendered===FALSE)return FALSE;var k=b?"show":"hide",j=f.elements.tooltip,p=d[k],v=j.is(":visible"),r;if((typeof b).search("boolean|number"))b=
!j.is(":visible");if(!v&&!b||j.is(":animated"))return f;if(e){if(f.cache.event&&/over|enter/.test(e.type)&&/out|leave/.test(f.cache.event.type)&&a(e.target).add(d.show.target).length<2&&a(e.relatedTarget).parents(B).length>0)return f;f.cache.event=a.extend({},e)}v=a.Event("tooltip"+k);v.originalEvent=a.extend({},e);j.trigger(v,[f.hash(),90]);if(v.isDefaultPrevented())return f;if(b){f.focus();f.reposition(e);p.solo&&a(B).qtip("hide")}else clearTimeout(f.timers.show);j.attr("aria-hidden",Boolean(!b));
...
k!==j){h.each(function(){var r=a(this).qtip(),y=a.Event("tooltipblur"),n,u;if(!r||r.rendered===FALSE)return TRUE;n=r.elements.tooltip;u=parseInt(n.css("z-index"),10);isNaN(u)||n.css({zIndex:u-1});n.removeClass(p);y.originalEvent=v;n.trigger(y,[r,j])});b=a.Event("tooltipfocus");b.originalEvent=v;e.trigger(b,[f.hash(),j]);b.isDefaultPrevented()||e.css({zIndex:j}).addClass(p)}return f},reposition:function(b){if(f.rendered===FALSE)return FALSE;var e=d.position.target,h=f.elements.tooltip,k=d.position,
j=k.my,p=k.at,v=f.elements.tooltip.width(),r=f.elements.tooltip.height(),y=a(k.container)[0],n=0,u=0,s={left:0,top:0},A=a.Event("tooltipmove"),C=h.css("position")==="fixed",E=a(F),K={left:function(D){var H=e==="mouse"?b.pageX:e.offset().left,I=E.scrollLeft(),G=E.width(),L=p.x==="left"?n:p.x==="right"?-n:n/2,J=j.x!==p.x&&p.x!=="center"?n:0,P=L+(j.x==="left"?-v:j.x==="right"?v:v/2)+-2*k.adjust.x;G=D+v-G-I;if(I-D>0&&!(D>=H&&D<H+n))s.left+=P-L+J;else if(G>0&&D+v>H)s.left+=(j.x==="center"?-1:1)*(P-L-J);
return s.left-D},top:function(D){var H=E.scrollTop(),I=E.height(),G=p.y==="top"?u:p.y==="bottom"?-u:0,L=j.y!==p.y&&p.y!=="center"?u:0,J=G+(j.y==="top"?-r:j.y==="bottom"?r:-r/2)+-2*k.adjust.y;I=D+r-I-H;if(H-D>0)s.top+=j.y==="center"?-J+G:J;else if(I>0)s.top+=J-G-L;return s.top-D}};if(e==="mouse"){p={x:"left",y:"top"};b=a.extend({},a.fn.qtip.mouse);s={top:b.pageY,left:b.pageX}}else{if(e==="event")e=b&&b.target&&b.type!=="scroll"&&b.type!=="resize"?f.cache.target=a(b.target):f.cache.target;if(e[0]===
document||e[0]===F){n=e.width();u=e.height();if(e[0]===F)s={top:C?0:E.scrollTop(),left:C?0:E.scrollLeft()}}else if(e.is("area")&&a.fn.qtip.plugins.imagemap){s=a.fn.qtip.plugins.imagemap(e,p);n=s.width;u=s.height;s=s.offset}else{n=e.outerWidth();u=e.outerHeight();s=e.offset();if(k.adjust.offset){do{s.left-=y.offsetLeft-y.scrollLeft;s.top-=y.offsetTop-y.scrollTop}while(y=y.offsetParent)}}s.left+=p.x==="right"?n:p.x==="center"?n/2:0;s.top+=p.y==="bottom"?u:p.y==="center"?u/2:0}s.left+=k.adjust.x+(j.x===
"right"?-v:j.x==="center"?-v/2:0);s.top+=k.adjust.y+(j.y==="bottom"?-r:j.y==="center"?-r/2:0);s.adjusted=k.adjust.screen&&e[0]!==F&&e[0]!==document.body?{left:K.left(s.left),top:K.top(s.top)}:{left:0,top:0};if(k.container[0]==document.body){if(s.top+E.scrollTop()<1)s.top=0;if(s.left+E.scrollLeft()<1)s.left=0}h.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass(z+"-pos-"+j.abbreviation());A.originalEvent=a.extend({},b);h.trigger(A,[f.hash(),s]);if(A.isDefaultPrevented())return f;
delete s.adjusted;if(h.is(":visible")&&a.isFunction(k.effect)){k.effect.call(h,f.hash(),s);h.queue(function(){var D=a(this);D.css({opacity:"",height:""});a.browser.msie&&this.style&&this.style.removeAttribute("filter");D.dequeue()})}else isNaN(s.left,s.top)||h.css(s);return f},disable:function(b){var e=f.elements.tooltip;if(f.rendered)e.toggleClass("ui-state-disabled",b);else f.cache.disabled=!!b;return f},destroy:function(){var b=f.elements,e=b.target.data("oldtitle");f.rendered&&a.each(f.plugins,
function(){this.initialize==="render"&&this.destroy()});w(1,1,1,1);c.removeData("qtip");f.rendered&&b.tooltip.remove();e&&c.attr("title",e);c.removeAttr("aria-describedby");return c},hash:function(){var b=a.extend({},f);delete b.cache;delete b.timers;delete b.options;delete b.plugins;delete b.render;delete b.hash;return b}})}function S(c,d){var i,g=a(this);i=a(document.body);var o=g.metadata?g.metadata(d.metadata):{};o=a.extend(TRUE,{},d,o);var l=o.position,t=this===document?i:g;if("boolean"===typeof o.content.text)if(o.content.attr!==
FALSE&&g.attr(o.content.attr))o.content.text=g.attr(o.content.attr);else return FALSE;if(l.container===FALSE)l.container=i;if(l.target===FALSE)l.target=t;if(o.show.target===FALSE)o.show.target=t;if(o.hide.target===FALSE)o.hide.target=t;l.at=new a.fn.qtip.plugins.Corner(l.at);l.my=new a.fn.qtip.plugins.Corner(l.my);if(g.data("qtip"))if(o.overwrite)g.qtip("destroy");else if(o.overwrite===FALSE)return FALSE;i=new R(g,o,c);g.data("qtip",i);return i}function T(c){var d=this;d.checks={"^content.ajax":function(){this.plugins.ajax.load(this.options.content.ajax)}};
a.extend(d,{init:function(){var i=c.options.content.ajax;c.elements.tooltip.bind("tooltiprender.ajax",function(){d.load(i);c.elements.tooltip.bind("tooltipshow.ajax",function(){i.once===FALSE&&c.rendered===TRUE&&d.load(i)})})},load:function(i){a.ajax(a.extend(TRUE,{},i,{success:function(g,o){if(a.isFunction(i.success))if(i.success.call(c.hash(),g,o)===FALSE)return;c.set("content.text",g);N.remove()},error:function(g,o,l){var t=o||l;if(a.isFunction(i.error)){g=i.error.call(c.hash(),g,o,l);if(g===FALSE)return}c.set("content.text",
t)}}));return d},destroy:function(){c.elements.tooltip.unbind("tooltipshow.ajax")}});d.init()}function Q(c,d,i){var g=Math.floor(d/2),o=Math.floor(i/2);d={bottomright:[[0,0],[d,i],[d,0]],bottomleft:[[0,0],[d,0],[0,i]],topright:[[0,i],[d,0],[d,i]],topleft:[[0,0],[0,i],[d,i]],topcenter:[[0,i],[g,0],[d,i]],bottomcenter:[[0,0],[d,0],[g,i]],rightcenter:[[0,0],[d,o],[0,i]],leftcenter:[[d,0],[d,i],[0,o]]};d.lefttop=d.bottomright;d.righttop=d.bottomleft;d.leftbottom=d.topright;d.rightbottom=d.topleft;return d[c]}
