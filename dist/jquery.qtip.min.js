...
"^content.title.text":function(){if(d.rendered)if(!d.elements.title&&i){v();d.reposition()}else i?d.elements.title.html(i):o()},"^position.container$":function(){if(d.rendered===true){d.elements.tooltip.appendTo(i);d.reposition()}},"^position.(my|at)$":function(){var m=/my$/i.test(e)?"my":"at";if("string"===typeof i)c.position[m]=new a.fn.qtip.plugins.Corner(i)},"^position.(my|at|adjust|target)":function(){d.rendered&&d.reposition()},"^(show|hide).(event|target|fixed)":function(m,t,r,x){var s=e.search(/fixed/i)>
-1?[0,[0,1,1,1]]:e.search(/hide/i)<0?["show",[1,0,0,0]]:["hide",[0,1,0,0]];if(s[0])m[t]=x;u.apply(d,s[1]);if(s[0])m[t]=r;w.apply(d,s[1])}}};a.each(d.plugins,function(m){if("object"===typeof this.checks)l[m]=this.checks});j=n[0][n[1]];n[0][n[1]]=i;K(c);for(f in l)for(h in l[f])RegExp(h,"i").test(e)&&l[f][h].call(d,n[0],n[1],i,j);return d},toggle:function(e,i){function n(){var t=e?"attr":"removeAttr",r=/^1|0$/.test(a(this).css("opacity"));d.elements.title&&b[t]("aria-labelledby","ui-tooltip-"+k+"-title");
b[t]("aria-describedby","ui-tooltip-"+k+"-content");if(e){if(a.browser.msie&&a(this).get(0).style&&r){m=a(this).get(0).style;m.removeAttribute("filter");m.removeAttribute("opacity")}}else r&&a(this).hide()}if(!d.rendered)return FALSE;var j=e?"show":"hide",f=d.elements.tooltip,h=c[j],l=f.is(":visible"),m;if((typeof e).search("boolean|number"))e=!f.is(":visible");if(!l&&!e||f.is(":animated"))return d;if(i){if(d.cache.event&&/over|enter/.test(i.type)&&/out|leave/.test(d.cache.event.type)&&a(i.target).add(c.show.target).length<
2&&a(i.relatedTarget).parents(".qtip.ui-tooltip").length>0)return d;d.cache.event=a.extend({},i)}l=a.Event("tooltip"+j);l.originalEvent=a.extend({},i);f.trigger(l,[d.hash(),90]);if(l.isDefaultPrevented())return d;if(e){if(d.rendered===TRUE){d.focus();d.reposition(i)}h.solo&&a(":not(.qtip.ui-tooltip)").qtip("hide")}else clearTimeout(d.timers.show);f.attr("aria-hidden",Boolean(!e));f.stop(TRUE,FALSE);if(a.isFunction(h.effect)){h.effect.call(f);f.queue(function(){n.call(this);a(this).dequeue()})}else if(h.effect===
FALSE){f[j]();n.call(f)}else f.fadeTo(90,e?100:0,n);e&&h.target.trigger("qtip-"+k+"-inactive");return d},show:function(e){d.toggle(TRUE,e)},hide:function(e){d.toggle(FALSE,e)},focus:function(e){if(!d.rendered)return FALSE;var i=d.elements.tooltip,n=parseInt(i.css("z-index"),10),j=a.fn.qtip.zindex+a(".qtip.ui-tooltip").length,f=a.extend({},e);if(!i.hasClass("ui-tooltip-focus")&&n!==j){a(".qtip.ui-tooltip").each(function(){var h=a(this).qtip(),l=a.Event("tooltipblur"),m,t;if(!h||!h.rendered)return TRUE;
m=h.elements.tooltip;t=parseInt(m.css("z-index"),10);isNaN(t)||m.css({zIndex:t-1});m.removeClass("ui-tooltip-focus");l.originalEvent=f;m.trigger(l,[h,j])});e=a.Event("tooltipfocus");e.originalEvent=f;i.trigger(e,[d.hash(),j]);e.isDefaultPrevented()||i.css({zIndex:j}).addClass("ui-tooltip-focus")}return d},reposition:function(e){if(d.rendered===FALSE)return FALSE;var i=c.position.target,n=d.elements.tooltip,j=c.position,f=j.my,h=j.at,l=d.elements.tooltip.width(),m=d.elements.tooltip.height(),t=a(j.container)[0],
...
r=s.width;x=s.height;s=s.offset}else{r=i.outerWidth();x=i.outerHeight();s=i.offset();if(j.adjust.offset){do{s.left-=t.offsetLeft-t.scrollLeft;s.top-=t.offsetTop-t.scrollTop}while(t=t.offsetParent)}}s.left+=h.x==="right"?r:h.x==="center"?r/2:0;s.top+=h.y==="bottom"?x:h.y==="center"?x/2:0}s.left+=j.adjust.x+(f.x==="right"?-l:f.x==="center"?-l/2:0);s.top+=j.adjust.y+(f.y==="bottom"?-m:f.y==="center"?-m/2:0);s.adjusted=j.adjust.screen?{left:z.left(s.left),top:z.top(s.top)}:{left:0,top:0};if(j.container[0]==
document.body){if(s.top<1)s.top=0;if(s.left<1)s.left=0}n.attr("class",function(){return a(this).attr("class").replace(/ui-tooltip-pos-\w+/i,"")}).addClass("ui-tooltip-pos-"+f.abbreviation());y.originalEvent=a.extend({},e);n.trigger(y,[d.hash(),s]);if(y.isDefaultPrevented())return d;delete s.adjust;if(n.is(":visible")&&a.isFunction(j.adjust.effect)){j.adjust.effect.call(n,s);n.queue(function(){a(this).css({opacity:"",height:""});a.browser.msie&&a(this).get(0).style&&a(this).get(0).style.removeAttribute("filter");
