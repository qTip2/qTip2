...
a.each(a.fn.qtip.plugins,function(){this.sanitize&&this.sanitize(c)});return c}function aa(c,b,i,k){function q(e){var f,j=e.split("."),s=b[j[0]];for(e=1;e<j.length;e+=1){f=s[j[e]];if(typeof f==="object"&&!f.jquery&&!f.precedance)s=s[j[e]];else break}return j[e]!==T?[s,j[e]]:[b,j[0]]}function h(e,f){var j={left:0,top:0},s=!a.fn.qtip.plugins.iOS,p,u;if(f){if(f.offsetParent!==v){j=h(f);j.left*=-1;j.top*=-1;j.left+=parseInt(a.css(f,"margin-left"),10);j.top+=parseInt(a.css(f,"margin-top"),10)}else{j.left+=
